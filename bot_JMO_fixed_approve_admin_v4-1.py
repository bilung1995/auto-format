...
async def forward_wa_to_telegram(bot, wa_text: str, wa_group: str, wa_sender: str, parsed_kab: str, parsed_kec: str):
    """Teruskan pesan WA ke Telegram user yang cocok dengan KAB + KEC."""
    nl = "\n"
    ...
    if not parsed_kab or not parsed_kec:
        print(f"⚠️ Parse KAB/KEC gagal, text: {wa_text[:200]}")
        return 0

    user_ids = find_users_by_kota_kec(parsed_kab, parsed_kec)
    if not user_ids:
        print(f"⏭️ Tidak ada user cocok: {parsed_kab} + {parsed_kec}")
        return 0

    # WA bebas berisi < > &. Karena bot memakai ParseMode.HTML,
    # escape semua isi dari WA agar pesan tidak ditolak Telegram.
    safe_text = html.escape(wa_text[:3500])
    safe_group = html.escape(wa_group[:120])
    safe_sender = html.escape(wa_sender[:120])
    safe_kab = html.escape(str(parsed_kab))
    safe_kec = html.escape(str(parsed_kec))

    clean_text = safe_text
    if "PERHATIAN" not in wa_text.upper():
        clean_text = f"{clean_text}{nl}{nl}{html.escape(FOOTER_PERINGATAN)}"

    final_text = (
        f"📢 <b>INFO MASUK KEYWORD</b>{nl}{nl}"
        f"📍 <b>KAB:</b> {safe_kab}{nl}"
        f"📍 <b>KEC:</b> {safe_kec}{nl}"
        f"👥 <b>GRUP:</b> {safe_group}{nl}"
        f"👤 <b>PENGIRIM:</b> {safe_sender}{nl}{nl}"
        f"{clean_text}"
    )
    ...
    for tid in set(user_ids):
        try:
            ...
            count += 1
            print(f"✅ Forwarded WA -> Telegram {tid} ({parsed_kab}/{parsed_kec})")
        except Exception as e:
            # User bisa memblokir bot / chat tidak tersedia; jangan sampai webhook crash.
            print(f"⚠️ Gagal forward ke Telegram {tid}: {e}")
    return count
...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🔎 CARI LAINNYA", callback_data="kota_search_lain"), InlineKeyboardButton(text="📢 KEYWORD LAIN", callback_data="keyword_lain")],
...
async def keyword_lain_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "📢 <b>KEYWORD LAIN</b>\n\n"
        "Silakan ketikan <b>keyword lain</b> yang ingin kamu pantau.\n\n"
        "🔔 Jika keyword tersebut muncul di pesan WhatsApp yang masuk, bot akan langsung mengirim notifikasi ke Telegram kamu.\n\n"
        "⚠️ Keyword <b>tidak boleh mengandung nama provinsi, kota/kabupaten, atau kecamatan</b>.\n\n"
        "💡 Contoh keyword yang diperbolehkan:\n"
...
async def callback_fallback(callback: CallbackQuery):
    """Jangan biarkan tombol yang belum tertangkap handler membuat UI seolah macet."""
    try:
        await callback.answer("⚠️ Menu sedang diperbaiki. Silakan coba lagi.", show_alert=True)
    except Exception:
        pass
