...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🏙️ KOTA SAYA", callback_data="kota_saya")],
        [InlineKeyboardButton(text="➕ TAMBAH KOTA", callback_data="kota_add")],
        [InlineKeyboardButton(text="🔎 CARI DATA LAINNYA", callback_data="kota_search_lain")],
...
async def kota_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    for prov in provinces:
        row.append(InlineKeyboardButton(text=prov["name"][:20], callback_data=f"prov_{prov['id']}_{prov['name'][:12]}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
...
async def provinsi_selected(callback: CallbackQuery, state: FSMContext):
    ...
    buttons = []
    row = []
    for reg in regencies[:50]:
        row.append(InlineKeyboardButton(text=reg["name"][:20], callback_data=f"kab_{reg['id']}_{reg['name'][:12]}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
...
async def kabupaten_selected(callback: CallbackQuery, state: FSMContext):
    ...
    buttons = []
    row = []
    for dist in districts[:50]:
        row.append(InlineKeyboardButton(text=dist["name"][:20], callback_data=f"kec_{dist['id']}_{dist['name'][:12]}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
        buttons.append(row)
    buttons.append([InlineKeyboardButton(text=f"⬅️ KEMBALI KE {kab_name[:10]}", callback_data=f"prov_{prov_id}_{prov_name}")])
    await callback.message.edit_text(
        f"🏘️ <b>{kab_name}, {prov_name} - PILIH KECAMATAN ({len(districts)})</b>\n\nKlik kecamatan untuk simpan:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await callback.answer()
...
async def kecamatan_selected(callback: CallbackQuery, state: FSMContext):
    ...
    conn.execute(
        ...
        (callback.from_user.id, prov_name, prov_id, kab_name, kab_id, kec_name, kec_id, datetime.now().isoformat())
    )
    conn.commit()
    conn.close()
    # Kurangi quota 1 (admin tidak dikurangi)
    if callback.from_user.id not in ADMIN_IDS and has_auto_format_access(callback.from_user.id) == False:
        use_kota_quota(callback.from_user.id)
    quota_sisa = get_kota_quota(callback.from_user.id)
    await callback.message.edit_text(
        f"✅ <b>KOTA BERHASIL DITAMBAHKAN</b>\n\n📍 Provinsi: <b>{prov_name}</b>\n🏛️ Kab/Kota: <b>{kab_name}</b>\n🏘️ Kecamatan: <b>{kec_name}</b>\n\nData tersimpan di KOTA SAYA!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
