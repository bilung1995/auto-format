...
def main_menu(user_id=None):
    ...
    # KOTAK BERWARNA BIRU/HIJAU/ABU - NAMA TETAP SAHABAT JHT ASLI
    keyboard = [
        [
            KeyboardButton(text="👤 PROFIL USER"),
            KeyboardButton(text="💳 TOP UP")
        ],
        [
            KeyboardButton(text="📊 CEK STATUS"),
            KeyboardButton(text="💡 SOLUSI JMO")
        ],
        [
            KeyboardButton(text="📝 AUTO FORMAT"),
            KeyboardButton(text="📞 HUBUNGI ADMIN")
        ],
        [
            KeyboardButton(text="✨ MENU EMAS"),
            KeyboardButton(text="ℹ️ INFO BOT")
        ],
    ]
    try:
        if is_admin(user_id):
...
async def hk_profil(message: Message):
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    # panggil handler profile yang lama
    await message.answer("👤 PROFIL USER - Ketik /profile atau klik di bawah:", reply_markup=main_menu_inline(message.from_user.id))

@dp.message(F.text == "💳 TOP UP")
async def hk_topup(message: Message):
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo
    await message.answer(
        "💳 TOP UP - Menu warna emas:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✨ BUKA TOP UP EMAS", web_app=WebAppInfo(url="https://golden-pastelito-feec0f.netlify.app"))],
            [InlineKeyboardButton(text="💰 50K", callback_data="topup_50"), InlineKeyboardButton(text="💰 100K", callback_data="topup_100")]
        ])
    )

@dp.message(F.text == "📊 CEK STATUS")
async def hk_status(message: Message):
...
async def hk_solusi(message: Message):
    await message.answer("💡 SOLUSI JMO - Pilih masalah", reply_markup=main_menu_inline(message.from_user.id))

@dp.message(F.text == "📝 AUTO FORMAT")
async def hk_auto(message: Message):
    await message.answer("📝 AUTO FORMAT - Kirim data", reply_markup=main_menu_inline(message.from_user.id))

@dp.message(F.text == "📞 HUBUNGI ADMIN")
async def hk_admin(message: Message):
    await message.answer("📞 HUBUNGI ADMIN - @admin")

@dp.message(F.text == "✨ MENU EMAS")
async def hk_emas(message: Message):
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo
    await message.answer("✨ MENU EMAS - Warna emas asli:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✨ BUKA MENU EMAS", web_app=WebAppInfo(url="https://golden-pastelito-feec0f.netlify.app"))]
    ]))

@dp.message(F.text == "ℹ️ INFO BOT")
async def hk_info(message: Message):
    await message.answer("ℹ️ SAHABAT JHT BOT - Layanan pencairan JHT")

@dp.message(F.text == "🏠 MENU UTAMA")
async def hk_menu_utama(message: Message):
