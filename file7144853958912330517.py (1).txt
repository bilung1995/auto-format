...
def main_menu(user_id=None):
    if user_id is None:
        user_id = 0
    # 2 KOLOM FULL LAYAR + FIX DUPLIKAT SOLUSI JMO
    rows = [
...
def main_menu_kota_aktif():
    # khusus setelah pilih kota aktif, ada tombol KEMBALI KE MENU UTAMA jelas
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏙️ LIHAT KOTA SAYA", callback_data="kota_saya")],
...
async def process_payment(callback: CallbackQuery, tx_id: int, approve: bool):
    ...
    if approve:
        ...
        if tx["package_code"]:
            ...
            elif code.startswith("kota_"):
                # Paket kota baru - semua dapat 3x kuota
                if code == "kota_1w":
                    expiry = datetime.now() + __import__('datetime').timedelta(days=7)
                elif code == "kota_1m":
            ...
            if quota_to_add > 0:
                conn.execute("""
                    INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0)
                    ON CONFLICT(telegram_id) DO UPDATE SET quota = quota + ?
                """, (tx["telegram_id"], quota_to_add, quota_to_add))
        else:
            conn.execute(
        ...
        expiry_text = ""
        quota_text = f"\n🎟️ Kuota: {quota_to_add}x TAMBAH KOTA" if quota_to_add>0 else ""
        if tx["package_code"]:
            expiry_text = (
                f"\n📅 Berakhir : "
        ...
        if quota_to_add > 0:
            user_text = (
                ...
                f"{expiry_text}\n\n"
                "🔓 TAMBAH KOTA SEKARANG BISA 3x PAKAI!\n"
                "Setelah 3x, klik ke-4 harus Top Up lagi!"
            )
        else:
            user_text = (
...
async def kota_add_start(callback: CallbackQuery, state: FSMContext):
    # ADMIN unlimited - langsung pilih provinsi
    if callback.from_user.id in ADMIN_IDS or is_admin(callback.from_user.id):
        conn = db()
        row = conn.execute("SELECT quota FROM kota_quota WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
        conn.close()
        # admin tetap bisa pilih paket atau langsung provinsi
        await state.clear()
        await callback.message.edit_text(
            "🏙️ <b>TAMBAH KOTA - PILIH PAKET (ADMIN)</b>\n\n"
            "Kamu admin, bisa langsung pilih provinsi atau beli paket lagi.\n\n"
            "💎 <b>Paket tersedia:</b>\n"
            ...
            "Klik paket untuk Top Up, atau langsung pilih provinsi:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="➡️ LANGSUNG PILIH PROVINSI", callback_data="kota_add_provinsi")],
    ...
    await state.clear()
    # ALUR BARU: TAMBAH KOTA HARUS PILIH PAKET DULU
    if quota > 0:
        # Masih ada quota, kasih 2 opsi: pakai sisa quota atau beli paket baru
        await callback.message.edit_text(
            ...
            "Klik paket untuk Top Up:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"✅ LANJUT PAKAI KUOTA ({quota}x) - PILIH PROVINSI", callback_data="kota_add_provinsi")],
        ...
        # Quota habis - WAJIB pilih paket dulu
        await callback.message.edit_text(
...
async def kota_add_provinsi(callback: CallbackQuery, state: FSMContext):
    # Lanjut pilih provinsi (hanya jika punya quota atau admin)
    quota = get_kota_quota(callback.from_user.id)
...
async def paket_kota_selected(callback: CallbackQuery, state: FSMContext):
    paket_map = {
        "paket_kota_1w": ("1 Minggu", 35000, "kota_1w"),
    ...
    await callback.message.edit_text(
        ...
        "🏦 <b>SEABANK</b>\n901040978290\nA/N HAMBALI\n\n"
        "💰 <b>DANA</b>\n083824101264\nA/N HAMBALI\n\n"
        f"Setelah transfer <b>{rupiah(harga)}</b>, klik SUDAH BAYAR.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ SUDAH BAYAR", callback_data="payment_done")],
            [InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
...
async def kecamatan_save(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🌆 LIHAT KOTA SAYA", callback_data="kota_saya")],
