...
PASADATA_URL_BARU = os.getenv("PASADATA_URL_BARU", "https://api.jsonbin.io/v3/b/6a8830e5f5f4af5e2930e232").strip()  # BIN BARU: 6a8830e5f5f4af5e2930e232 - 100% TERPISAH DARI BOT LAMA!
...
def db():
    ...
    # Pertama kali buka DB, coba restore dari PASADATA BARU (bukan lama!)
...
def init_database():
    # Restore dari PASADATA BARU dulu (bukan lama!)
    if PASADATA_URL_BARU:
        sync_from_pasadata_baru()
    conn = db()
    conn.executescript("""
...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🔎 CARI LAINNYA", callback_data="kota_search_lain"), InlineKeyboardButton(text="💡 SOLUSI JMO", callback_data="solusi_jmo")],
...
def auto_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        return
    buttons = []
    # 1 KOLOM FULL LAYAR - sesuai request
    for reg in regencies[:100]:
...
async def kabupaten_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(kab_id=kab_id, kab_name=kab_name, selected_kec=[])
    await callback.message.edit_text(
    ...
    if not districts:
        ...
        return
    await state.update_data(districts_cache=districts)
    # Build 1 kolom per baris
    buttons = []
    for dist in districts[:60]:  # limit 60 biar tidak terlalu panjang
        buttons.append([InlineKeyboardButton(text=f"⬜ {dist['name']}", callback_data=f"kec_toggle_{dist['id']}_{dist['name']}")])
    # Tombol aksi bawah - full width 1 per baris sesuai request
...
✅ Bisa pilih lebih dari 3 kecamatan sekaligus.
Klik untuk pilih/hapus pilihan, lalu klik SIMPAN:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await callback.answer()
...
async def kecamatan_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    # Rebuild buttons 1 kolom, tampilkan centang
    buttons = []
    ...
    if count > 0:
        daftar = ", ".join([x['name'] for x in selected[:5]])
        if count > 5:
            daftar += f" +{count-5} lainnya"
    else:
        daftar = "Belum ada yang dipilih"
...
async def kecamatan_save(callback: CallbackQuery, state: FSMContext):
    ...
    selected = data.get("selected_kec", [])
    if not selected:
        await callback.answer("❌ Belum ada kecamatan yang dipilih! Pilih minimal 1.", show_alert=True)
        return
    prov_name = data.get("provinsi_name", "")
    prov_id = data.get("provinsi_id", "")
    kab_name = data.get("kab_name", "")
    ...
    for kec in selected:
        conn.execute(
            "INSERT INTO user_kota(telegram_id,provinsi,provinsi_id,kab,kab_id,kec,kec_id,created_at) VALUES(?,?,?,?,?,?,?,?)",
    ...
    # Kurangi quota sesuai jumlah yang disimpan (admin tidak dikurangi)
    if callback.from_user.id not in ADMIN_IDS and not has_auto_format_access(callback.from_user.id):
...
async def kecamatan_selected_OLD(callback: CallbackQuery, state: FSMContext):
    # Handler lama single-select - dinonaktifkan, diarahkan ke toggle
    # Untuk kompatibilitas jika masih ada callback kec_xxx lama
...
async def kota_list(callback: CallbackQuery):
    conn = db()
    rows = conn.execute("SELECT * FROM user_kota WHERE telegram_id=? ORDER BY created_at DESC LIMIT 20", (callback.from_user.id,)).fetchall()
    ...
    text = f"🌆 <b>KOTA SAYA - {username}</b>\n\n"
    text += f"👤 User: <b>{username}</b>\n"
    text += f"🆔 ID: <code>{callback.from_user.id}</code>\n"
    text += f"💰 Saldo: <b>{rupiah(balance)}</b>\n"
    text += f"🎟️ Sisa Kuota: <b>{quota} kali</b> | Dipakai: {total_used}x\n"
    text += f"📍 Total Kota: <b>{len(rows)} kota</b>\n\n"
    text += "📋 <b>DAFTAR KOTA YANG DIPILIH:</b>\n"
    buttons = []
    for i, r in enumerate(rows, 1):
        text += f"{i}. <b>{r['provinsi']} > {r['kab']} > {r['kec']}</b>\n"
        text += f"   📅 {r['created_at'][:10]}\n"
        buttons.append([InlineKeyboardButton(text=f"🗑️ HAPUS {i}. {r['kec'][:12]} - {r['kab'][:8]}", callback_data=f"kota_del_{r['id']}")])
    
    buttons.append([InlineKeyboardButton(text="🏙️ TAMBAH KOTA", callback_data="kota_add")])
    buttons.append([InlineKeyboardButton(text="🔍 CARI DATA LAINNYA", callback_data="kota_search_lain")])
...
async def kota_search_lain_process(message: Message, state: FSMContext):
    ...
    # CARI DI HISTORY WA - format_results (hasil terkini)
    format_rows = conn.execute(
        "SELECT * FROM format_results WHERE telegram_id=? AND result_text LIKE ? ORDER BY created_at DESC LIMIT 20",
        (message.from_user.id, like)
    ).fetchall()
    
    # CARI DI RIWAYAT - format_history
    history_rows = conn.execute(
        "SELECT * FROM format_history WHERE telegram_id=? AND result_text LIKE ? ORDER BY created_at DESC LIMIT 20",
        (message.from_user.id, like)
    ).fetchall()
    
    # CARI DI KOTA SAYA
    kota_rows = conn.execute(
        "SELECT * FROM user_kota WHERE telegram_id=? AND (provinsi LIKE ? OR kab LIKE ? OR kec LIKE ? OR kel LIKE ?) ORDER BY created_at DESC LIMIT 10",
        (message.from_user.id, like, like, like, like)
    ).fetchall()
    ...
    if total == 0:
        await message.answer(
            ...
            f"❌ Tidak ada riwayat WA dengan kota <b>{query.upper()}</b>\n\n"
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                ...
                [InlineKeyboardButton(text="⬅️ MENU UTAMA", callback_data="auto_format")]
            ])
        )
        return
    ...
    if format_rows:
        ...
        for i, r in enumerate(format_rows[:10], 1):
            snippet = r['result_text'][:100].replace('\n', ' ')
            text += f"{i}. {snippet}...\n"
            text += f"   📅 {r['created_at'][:16]} | KODE: {r['kode'][:10]}\n\n"
    
    if history_rows:
        text += f"🕘 <b>RIWAYAT WA ({len(history_rows)}):</b>\n"
        for i, r in enumerate(history_rows[:10], 1):
            snippet = r['result_text'][:100].replace('\n', ' ')
            text += f"{i}. {snippet}...\n"
            text += f"   📅 {r['created_at'][:16]}\n\n"
    
    # Potong jika terlalu panjang untuk Telegram (max 4096)
    if len(text) > 3500:
        text = text[:3500] + "\n... (hasil dipotong, terlalu banyak)"
    
    await message.answer(
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔍 CARI LAGI", callback_data="kota_search_lain")],
            [InlineKeyboardButton(text="📄 LIHAT HASIL TERKINI", callback_data="format_results")],
...
async def admin_balance_process(message: Message, state: FSMContext):
    ...
    if data.get("action") == "add":
        ...
        try:
            conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+?, total_used=total_used", (user_id, 2, 2))
        except Exception as e:
...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🏙️ KOTA SAYA", callback_data="kota_saya"), InlineKeyboardButton(text="➕ TAMBAH KOTA", callback_data="kota_add")],
        [InlineKeyboardButton(text="🚫 NO BLACKLIST", callback_data="blacklist_view"), InlineKeyboardButton(text="💡 SOLUSI JMO", callback_data="solusi_jmo")],
        [InlineKeyboardButton(text="🔎 CARI LAINNYA", callback_data="kota_search_lain"), InlineKeyboardButton(text="📞 ADMIN", callback_data="contact_admin")],
        [InlineKeyboardButton(text="🆘 BANTUAN", callback_data="bantuan")],
    ]
    if is_admin(user_id):
        rows.append([InlineKeyboardButton(text="🔐 PANEL ADMIN", callback_data="admin_panel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📋 LIST BLACKLIST", callback_data="admin_blacklist_view"), InlineKeyboardButton(text="🗑️ HAPUS USER", callback_data="admin_delete_user")],
...
def add_kota_quota(telegram_id, amount=2):
    conn = db()
...
def use_kota_quota(telegram_id):
    conn = db()
    conn.execute("UPDATE kota_quota SET quota=quota-1, total_used=total_used+1 WHERE telegram_id=?", (telegram_id,))
    conn.commit()
    conn.close()
...
async def kota_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_admin and quota <= 0 and not has_access:
        # if balance <=0 check, but now use quota system
        if balance <= 0:
            await callback.message.edit_text(
                "🔒 <b>AKSES TERKUNCI - TAMBAH KOTA</b>\n\n"
                "Untuk membuka menu <b>TAMBAH KOTA</b>, kamu harus Top Up dulu ke rekening Admin.\n\n"
                f"💰 Saldo kamu saat ini: <b>{rupiah(balance)}</b>\n"
                f"🎟️ Sisa Kuota Kota: <b>{quota} kali</b>\n"
                "💳 Minimal Top Up Rp 10.000 untuk dapat 2x akses TAMBAH KOTA.\n\n"
                "Silakan Top Up dulu ya bos!",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            return
    # kalau quota habis tapi masih punya saldo? tetap suruh topup lagi sesuai request
    if not is_admin and quota <= 0:
        await callback.message.edit_text(
            "🔒 <b>KUOTA HABIS - TAMBAH KOTA</b>\n\n"
            f"🎟️ Sisa Kuota: <b>{quota} kali</b>\n"
            "Kamu sudah pakai 2x akses TAMBAH KOTA.\n"
            "Untuk lanjut, harus Top Up lagi!\n\n"
            "Top Up 1x = 2x akses TAMBAH KOTA",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💳 TOP UP LAGI", callback_data="topup")],
                [InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
                [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
            ])
        )
        await callback.answer("❌ Kuota habis, Top Up lagi!", show_alert=True)
        return
        await callback.message.edit_text(
            "🔒 <b>AKSES TERKUNCI - TAMBAH KOTA</b>\n\n"
            "Untuk membuka menu <b>TAMBAH KOTA</b>, kamu harus Top Up dulu ke rekening Admin.\n\n"
            f"💰 Saldo kamu saat ini: <b>{rupiah(balance)}</b>\n"
            "💳 Minimal Top Up Rp 10.000 untuk akses menu ini.\n\n"
            "Silakan Top Up dulu ya bos!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💳 TOP UP SEKARANG", callback_data="topup")],
                [InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
    ...
    await state.clear()
    await callback.message.edit_text(
        "🏙️ <b>TAMBAH KOTA - PILIH PROVINSI</b>\n\n⏳ Memuat 38 provinsi...",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]])
    )
    ...
    if not provinces:
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔄 COBA LAGI", callback_data="kota_add")],
                [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
            ])
        )
        await callback.answer()
    ...
    for prov in provinces:
        buttons.append([InlineKeyboardButton(text=prov["name"], callback_data=f"prov_{prov['id']}_{prov['name']}")])  # 1 KOLOM
    buttons.append([InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")])
    await callback.message.edit_text(
        f"🏙️ <b>PILIH PROVINSI ({len(provinces)} Provinsi)</b>\n\nKlik provinsi untuk lihat kab/kota:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await callback.answer()
