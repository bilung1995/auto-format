...
def find_users_from_wa_message(wa_text: str):
    ...
    Ini sengaja tidak bergantung pada label KAB/KEC. Contoh pesan:
    2017\nSORONG\nSORONG\nBANJAR AGUNG\nRp 34.096.72
    tetap akan cocok dengan user yang memilih SORONG -> SORONG.
    """
    text = str(wa_text or "").upper()
    if not text:
        return []
    normalized_text = _norm_region_name(text)
    try:
        conn = db()
        rows = conn.execute("SELECT DISTINCT telegram_id,kab,kec FROM user_kota").fetchall()
        conn.close()
        ...
        for r in rows:
            kab = (r['kab'] or '').strip()
            kec = (r['kec'] or '').strip()
            nkab = _norm_region_name(kab)
            nkec = _norm_region_name(kec)
            if not nkab or not nkec:
                continue
            # Ikuti logika bot lama: kota/kabupaten dan kecamatan harus muncul
            # berurutan pada baris berbeda. Contoh: 2017 / SORONG / SORONG.
            lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
            ok = False
            kota_indexes = [i for i, ln in enumerate(lines)
                            if re.search(r'(?<![A-Z0-9])' + re.escape(nkab) + r'(?![A-Z0-9])', ln)]
            for kota_idx in kota_indexes:
                for kec_idx in range(kota_idx + 1, len(lines)):
                    if re.search(r'(?<![A-Z0-9])' + re.escape(nkec) + r'(?![A-Z0-9])', lines[kec_idx]):
                        ok = True
                        break
                if ok:
                    break
...
def _extract_wa_payload(data):
    """Parse incoming Green-API webhook payload only."""
    if not isinstance(data, dict):
        return '', '', '', '', 'green_api'

    sender_data = data.get('senderData') or {}
    message_data = data.get('messageData') or {}
    ttype = str(data.get('typeMessage') or '')

    sender = sender_data.get('sender') or sender_data.get('chatId') or message_data.get('sender') or ''
    group = sender_data.get('chatName') or sender_data.get('chatId') or 'Grup WA'
    ...
    if ttype == 'textMessage':
        ...
        text = (message_data.get('imageMessageData') or {}).get('caption', '')
    elif ttype == 'documentMessage':
        text = (message_data.get('documentMessageData') or {}).get('caption', '')
        ...
        elif isinstance(message_data.get('extendedTextMessageData'), dict):
            text = message_data['extendedTextMessageData'].get('text', '')
        elif isinstance(message_data.get('caption'), str):
