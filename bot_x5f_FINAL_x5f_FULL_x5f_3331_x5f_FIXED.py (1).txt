...
async def blacklist_view_user(callback: CallbackQuery):
    ...
    if not rows:
        ...
        for i, r in enumerate(rows, 1):
            tgl = r['created_at'][:10] if r['created_at'] else "-"
            text += f"{i}. <code>{r['number']}</code> - {tgl}\n"
    await callback.message.edit_text(text[:4000], reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="back_main")]]))
    await callback.answer()
