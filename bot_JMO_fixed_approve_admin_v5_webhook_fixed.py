    ...
    try:
        ...
        for r in rows:
            ...
            if not nkab or not nkec:
                continue
            # Untuk pasangan yang sama (mis. SORONG -> SORONG), satu kecocokan
            # cukup karena secara lokasi keduanya memang identik.
            if nkab == nkec:
                ok = re.search(r'(?<![A-Z0-9])' + re.escape(nkab) + r'(?![A-Z0-9])', normalized_text)
            else:
                ok = (
                    re.search(r'(?<![A-Z0-9])' + re.escape(nkab) + r'(?![A-Z0-9])', normalized_text)
                    and re.search(r'(?<![A-Z0-9])' + re.escape(nkec) + r'(?![A-Z0-9])', normalized_text)
                )
            if ok:
...
async def fonnte_webhook(request):
    try:
        ...
        try:
            ...
            data = {}
    # Fonnte kadang kirim di field berbeda, cek semua
    # Fonnte dapat mengirim payload dengan beberapa nama field.
    nested = data.get('data') if isinstance(data.get('data'), dict) else {}
    wa_text = (
        data.get('message') or data.get('text') or data.get('msg') or
        data.get('message_text') or nested.get('message') or nested.get('text') or ''
    )
    wa_group = (
        data.get('group') or data.get('group_name') or data.get('name') or
        data.get('group_id') or nested.get('group') or nested.get('group_name') or
        'ZOLDYCK STORE'
    )
    wa_sender = (
        data.get('sender') or data.get('from') or data.get('pushname') or
        data.get('author') or data.get('sender_name') or nested.get('sender') or ''
    )
    wa_number = (
        data.get('phone') or data.get('number') or data.get('sender_number') or
        data.get('from') or nested.get('phone') or nested.get('number') or
        nested.get('sender_number') or ''
    )
    if not wa_text:
        # Jika masih kosong, log full data untuk debug
    ...
    try:
        ...
        print(f"log fail {e}")
    bot = request.app['bot']
    keyword_notif = await notify_custom_keywords(bot, wa_text, wa_group, wa_sender)
    forwarded = await forward_wa_to_telegram(bot, wa_text, wa_group, wa_sender, wa_number, kab, kec)
    return web.json_response({"status": True, "forwarded": forwarded, "kab": kab, "kec": kec, "parsed_kab": kab, "parsed_kec": kec, "admin_notif": True, "keyword_notif": keyword_notif, "footer_added": True})
...
async def blacklist_search_check(message: Message, state: FSMContext):
    ...
    if not data.get("is_blacklist_search"):
        ...
        # Cari juga pesan WhatsApp yang sudah masuk melalui webhook Fonnte.
...
async def main():
    ...
    app.router.add_post('/app-webhook', fonnte_webhook)
    app.router.add_post('/webhook/fonnte', fonnte_webhook)
    app.router.add_post('/webhook', fonnte_webhook)
    app.router.add_get('/app-webhook', lambda r: web.json_response({"status": True, "message": "WhatsApp webhook aktif"}))
