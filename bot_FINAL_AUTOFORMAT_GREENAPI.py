...
async def green_webhook(request):
    ...
    try:
        ...
        matched = {}
        if kab and kec:
            # Pilihan wilayah user disimpan di user_kota. Jangan mengambil dari
            # tabel users karena user_kota adalah sumber pilihan KOTA/KEC aktif.
            rows = conn.execute(
                "SELECT DISTINCT telegram_id FROM user_kota WHERE UPPER(TRIM(kab))=UPPER(TRIM(?)) AND UPPER(TRIM(kec))=UPPER(TRIM(?))",
                (kab, kec)
            ).fetchall()
            targets_kota = [r["telegram_id"] for r in rows]
            if not targets_kota:
                rows = conn.execute(
                    "SELECT DISTINCT telegram_id FROM user_kota WHERE UPPER(kab) LIKE UPPER(?) AND UPPER(kec) LIKE UPPER(?)",
                    (f"%{kab}%", f"%{kec}%")
                ).fetchall()
                targets_kota = [r["telegram_id"] for r in rows]

            # Hanya kirim ke user yang masih mempunyai akses, kecuali admin.
            active_targets = []
            for tid in targets_kota:
                try:
                    if is_admin(tid):
                        active_targets.append(tid)
                        continue
                    sub = conn.execute("SELECT status, expiry_date FROM subscriptions WHERE telegram_id=?", (tid,)).fetchone()
                    if not sub or sub["status"] == "unlimited":
                        active_targets.append(tid)
                    elif sub["status"] == "active":
                        exp = sub["expiry_date"]
                    ...
                    print(f"subscription check {tid}: {e}")
            targets_kota = active_targets
        try:
...
# ==================== CARI DATA LAINNYA - 1 PENGIRIM 1 KOTAK ====================
async def send_cari_hasil(bot, chat_id, keyword, rows):
    """
    Kirim hasil CARI DATA LAINNYA sebagai 1 pengirim = 1 kotak.
    Setiap kotak mempunyai tombol SALIN dan CHAT PENGIRIM.
    rows: iterable dict/Row dengan wa_group, wa_sender, message, parsed_kab, parsed_kec.
    """
    seen = set()
    sent = 0
    for r in rows:
        sender = str(r["wa_sender"] or "Tidak diketahui").strip()
        key = sender.casefold()
        if key in seen:
            continue
        seen.add(key)

        group = str(r["wa_group"] or "-").strip()
        message = str(r["message"] or "").strip()
        kab = str(r["parsed_kab"] or "").strip()
        kec = str(r["parsed_kec"] or "").strip()

        # Ambil nomor dari field pengirim jika berbentuk nomor/ID Green-API.
        raw = sender
        digits = "".join(re.findall(r"\d+", raw))
        if digits.startswith("0"):
            digits = "62" + digits[1:]
        elif digits and not digits.startswith("62") and len(digits) >= 10:
        ...
        try:
            conn = db()
            conn.execute(
                "INSERT OR REPLACE INTO salin_cache(id,content,created_at) VALUES(?,?,?)",
                (cache_id, message[:4000], datetime.now().isoformat())
            )
            conn.commit()
            conn.close()
        except Exception:
        ...
        box = (
            f"🛡️ <b>GRUP:</b> {html.escape(group)}\n"
            f"🧑‍💻 <b>PENGIRIM:</b> {html.escape(sender)}\n"
        )
        if kab or kec:
            box += f"📍 <b>WILAYAH:</b> {html.escape(kab or '-')} / {html.escape(kec or '-')}\n"
        box += f"\n💬 <b>CHAT:</b>\n{html.escape(message[:3500])}"

        buttons = [
            [InlineKeyboardButton(text="📋 SALIN", callback_data=f"salin_{cache_id}")]
        ]
        if digits and len(digits) >= 10:
            buttons.append([
                InlineKeyboardButton(text="💬 CHAT PENGIRIM", url=f"https://wa.me/{digits}")
            ])
        else:
            buttons.append([
                InlineKeyboardButton(text="💬 CHAT PENGIRIM", callback_data="noop")
            ])
...
async def format_create(callback: CallbackQuery, state: FSMContext):
    # BUAT FORMAT langsung masuk mode input agar tidak perlu bolak-balik menu.
    ...
    await state.update_data(edit_result_id=None)

    await callback.message.edit_text(
        "📝 <b>BUAT FORMAT</b>\n\n"
        "Silakan langsung ketik / paste data yang mau dibuat format.\n\n"
        "<b>Urutan data:</b>\n"
        "1️⃣ KAB\n2️⃣ KEC\n3️⃣ KEL\n4️⃣ SALDO\n5️⃣ KELAMIN\n"
        "6️⃣ KPJ\n7️⃣ SENSOR\n8️⃣ IT\n9️⃣ PT\n\n"
        "Contoh:\n"
        ...
        "➡️ Setelah kamu kirim, bot <b>langsung membalas hasil AUTO FORMAT</b>.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def format_manual_receive(message: Message, state: FSMContext):
    if not (message.text or '').strip():
        await message.answer("❌ Data kosong. Silakan kirim data untuk dibuat AUTO FORMAT.")
        return
    if not has_auto_format_access(message.from_user.id):
        await state.clear()
        await message.answer("🔒 AUTO FORMAT masih terkunci. Silakan pilih paket terlebih dahulu.")
        return
...
async def keyword_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "🔑 <b>TAMBAH KEYWORD</b>\n\nKirim format:\n<code>KEYWORD | KOTA | KECAMATAN</code>\n\nContoh:\n<code>JHT CAIR | SERANG | CIRUAS</code>\n<code>LASIK | JEMBER | KALIWATES</code>\n\nKetik /cancel untuk batal.",
        reply_markup=back_main()
    )
    await callback.answer()
