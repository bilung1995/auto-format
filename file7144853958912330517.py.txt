...
async def admin_user_aktif(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        ...
        return
    conn = db()
    # User aktif = pernah transaksi dalam 30 hari atau punya quota / subscription
    rows = conn.execute("""
        SELECT u.telegram_id, u.name, u.username, u.balance, u.created_at,
               s.package_name, s.status, s.expiry_date,
               COALESCE(q.quota,0) as quota, COALESCE(q.total_used,0) as total_used
        FROM users u
        LEFT JOIN subscriptions s ON s.telegram_id=u.telegram_id
        LEFT JOIN kota_quota q ON q.telegram_id=u.telegram_id
        ORDER BY u.created_at DESC LIMIT 50
    """).fetchall()
    total_users = conn.execute("SELECT COUNT(*) as c FROM users").fetchone()["c"]
    active_subs = conn.execute("SELECT COUNT(*) as c FROM subscriptions WHERE status='unlimited' OR (status='active' AND expiry_date>?)", (datetime.now().isoformat(),)).fetchone()["c"]
    total_blacklist = conn.execute("SELECT COUNT(*) as c FROM blacklist_numbers").fetchone()["c"]
    conn.close()
    
    text = f"👥 <b>CEK USER AKTIF - {total_users} User</b>\n\n"
    text += f"📊 Total User: {total_users}\n🟢 Langganan Aktif: {active_subs}\n🚫 Blacklist: {total_blacklist}\n\n"
    text += "📋 <b>50 User Terbaru:</b>\n━━━━━━━━━━━━\n"
    for i, r in enumerate(rows[:30], 1):
        name = (r['name'] or '-')[:15]
        username = f"@{r['username']}" if r['username'] else ""
        pkg = r['package_name'] or 'Belum langganan'
        quota = r['quota']
        status_icon = "🟢" if r['status'] in ('active','unlimited') else "🔴"
        text += f"{i}. {status_icon} {name} {username}\n   🆔 <code>{r['telegram_id']}</code> | 🎟️ {quota}x | {pkg[:20]}\n"
    await callback.message.edit_text(text[:3800], reply_markup=admin_menu())
    await callback.answer()
...
async def admin_stats(callback: CallbackQuery):
    ...
    total_kota = conn.execute("SELECT COUNT(*) as c FROM user_kota").fetchone()["c"]
    total_blacklist = conn.execute("SELECT COUNT(*) as c FROM blacklist_numbers").fetchone()["c"]
    ...
    text = (
        ...
        f"🟢 Langganan Aktif: <b>{active_subs}</b> / {total_subs}\n"
        f"💰 Total Transaksi: <b>{total_tx}</b>\n"
        f"⏳ Pending: <b>{pending_tx}</b>\n"
        f"🏙️ Total Kota Dipilih: <b>{total_kota}</b>\n"
        f"🚫 Blacklist: <b>{total_blacklist}</b>\n"
        f"👑 Admin Tambahan: <b>{total_admins}</b>\n"
        f"👑 Admin Utama: <b>{len(ADMIN_IDS)}</b>\n\n"
        f"🎟️ Sistem: 1x Top Up = 3x TAMBAH KOTA"
    )
...
async def admin_add_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Hanya admin utama yang bisa tambah admin!", show_alert=True)
        return
    await state.clear()
    ...
    conn.close()
    txt = "👑 <b>TAMBAH ADMIN</b>\n\n"
    if admins:
        txt += "Admin tambahan saat ini:\n"
        for a in admins:
            txt += f"• <code>{a['telegram_id']}</code> - {a['name']}\n"
        txt += "\n"
    txt += "Kirim Telegram ID admin baru:\n<code>123456789</code>\n\nBisa juga format: <code>ID Nama</code>"
...
async def admin_add_admin_process(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id) or message.from_user.id not in ADMIN_IDS:
        await state.clear()
        return
    parts = message.text.strip().split(maxsplit=1)
    if not parts[0].isdigit():
        await message.answer("❌ ID harus angka. Contoh: 123456789")
        return
    new_id = int(parts[0])
    name = parts[1] if len(parts)>1 else ""
    if new_id in ADMIN_IDS:
        await message.answer("❌ Sudah admin utama.")
        return
    conn = db()
    try:
        conn.execute("INSERT INTO admins(telegram_id,added_by,name,created_at) VALUES(?,?,?,?)", (new_id, message.from_user.id, name, datetime.now().isoformat()))
        conn.commit()
        await message.answer(f"✅ Admin <code>{new_id}</code> ditambahkan!", reply_markup=admin_menu())
    except sqlite3.IntegrityError:
        await message.answer(f"⚠️ Admin <code>{new_id}</code> sudah ada!", reply_markup=admin_menu())
    finally:
        conn.close()
        await state.clear()
...
async def admin_del_admin_start(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        ...
        return
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("❌ Hanya admin utama!", show_alert=True)
        return
    await state.clear()
    ...
    conn.close()
    if not admins:
        await callback.message.edit_text("❌ Belum ada admin tambahan.\nAdmin utama di .env ADMIN_IDS", reply_markup=admin_menu())
        await callback.answer()
        return
    txt = "❌ <b>HAPUS ADMIN</b>\n\nAdmin tambahan:\n"
    for a in admins:
        txt += f"• <code>{a['telegram_id']}</code> - {a['name']}\n"
    txt += "\nKirim ID yang mau dihapus:"
    await callback.message.edit_text(txt[:3500], reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ BATAL", callback_data="admin_cancel")]]))
    await callback.answer()

@dp.message(AdminState.waiting_del_admin, F.text)
async def admin_del_admin_process(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id) or message.from_user.id not in ADMIN_IDS:
        await state.clear()
        return
    ...
    if del_id in ADMIN_IDS:
        await message.answer("❌ Tidak bisa hapus admin utama (set di .env)")
        return
    conn = db()
    cur = conn.execute("DELETE FROM admins WHERE telegram_id=?", (del_id,))
    conn.commit()
    conn.close()
    await state.clear()
    if cur.rowcount>0:
        await message.answer(f"✅ Admin <code>{del_id}</code> dihapus!", reply_markup=admin_menu())
    else:
        await message.answer(f"❌ Admin <code>{del_id}</code> tidak ditemukan.", reply_markup=admin_menu())

@dp.callback_query(F.data == "admin_cancel")
async def admin_cancel(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text("🔐 <b>PANEL ADMIN</b>", reply_markup=admin_menu())
    await callback.answer()
...
async def admin_balance_process(message: Message, state: FSMContext):
    ...
    if data.get("action") == "add":
        ...
        # TAMBAH QUOTA KOTA 3x SETIAP TOPUP - sesuai request terbaru
        try:
            conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+3", (user_id, 3))
        except Exception as e:
            logging.error(f"Quota add error: {e}")
        action_text = f"➕ Ditambah {rupiah(amount)} + 🎟️ Quota Kota 3x"
    else:
