            ...
            async with session.post(url, headers=headers, data=data, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                ...
                if resp.status == 200:
                    ...
                    if 'whatsapp' in txt.lower() or 'device' in txt.lower():
                        print(f"⚠️ GREEN warning tapi mungkin terkirim: {txt[:300]}")
                        return True
                else:
                    print(f"❌ GREEN gagal {resp.status}: {txt[:500]}")
                    return False
    except Exception as e:
        print(f"GREEN error: {e}")
        return False

    print("⚠️ GREEN gagal - cek token")
    return False


def send_wa_notif_background(phone_number: str, kota_nama: str, user_name: str = ""):
    """Kirim WA di background biar Telegram satset, gak nunggu Green"""
    try:
        import threading
        def run():
            try:
        ...
        print(f"🚀 GREEN background task started untuk {phone_number} - {kota_nama}")
        return True
    except Exception as e:
...
GREEN_API_ID = os.getenv("GREEN_API_ID", os.getenv("GREEN_ID_INSTANCE","")).strip()
GREEN_API_TOKEN = os.getenv("GREEN_API_TOKEN", os.getenv("GREEN_API_TOKEN_INSTANCE","")).strip()
# Green tetap ada tapi tidak dipakai kalau PROVIDER green
...
# Untuk kompatibilitas kalau Green masih kepencet, redirect ke green handler
...
async def set_wa_number_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Bot baru ini khusus GREEN API - SATSET 0.5 detik! (via Green API)",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def set_wa_number_process(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"✅ Nomor WA disimpan: <code>{num}</code>\n\n⚠️ Test notif gagal: {e}\nPastikan GREEN_TOKEN sudah diset di Railway Variables!",
