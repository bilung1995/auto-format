...
async def hasil_akun(callback: CallbackQuery):
    ...
    conn = db()
    rows = conn.execute("""
        SELECT a.id, a.email, a.password, a.raw_text, a.created_at, a.result_id,
...
async def search_akun_do(message: Message, state: FSMContext):
    ...
    conn = db()
    rows = conn.execute("SELECT id,email,password FROM format_accounts WHERE telegram_id=? AND (email LIKE ? OR password LIKE ?) ORDER BY id ASC LIMIT 20", (message.from_user.id, f"%{keyword}%", f"%{keyword}%")).fetchall()
    conn.close()
    await state.clear()
    if not rows:
...
async def admin_list_admin(callback: CallbackQuery):
    ...
    db_admins_text = "\n".join([f"• <code>{r['telegram_id']}</code> - oleh {r['added_by']}" for r in rows]) if rows else "-"
...
async def admin_tambah_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "➕ <b>TAMBAH ADMIN</b>\n\nKirim Telegram ID baru. Contoh: <code>123456789</code> atau <code>123, 456</code>",
        reply_markup=admin_menu()
    )
    await callback.answer()
...
async def admin_tambah_admin_process(message: Message, state: FSMContext):
    ...
    for p in raw:
        if p.isdigit():
            ...
            added.append(aid)
    await state.clear()
    await message.answer(f"✅ Tambah admin: {', '.join([f'<code>{a}</code>' for a in added])}" if added else "❌ Tidak ada yang ditambah.", reply_markup=admin_menu())
...
async def admin_hapus_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    rows=conn.execute("SELECT telegram_id FROM admins").fetchall()
    conn.close()
    list_text="\n".join([f"• <code>{r['telegram_id']}</code>" for r in rows]) if rows else "Tidak ada admin DB."
    await callback.message.edit_text(f"➖ <b>HAPUS ADMIN</b>\n\n{list_text}\n\nKirim ID yang mau dihapus.", reply_markup=admin_menu())
    await callback.answer()
...
async def admin_hapus_admin_process(message: Message, state: FSMContext):
    ...
    for p in raw:
        if p.isdigit():
            aid=int(p)
            if aid in ADMIN_IDS:
                await message.answer(f"❌ ID <code>{aid}</code> ENV tidak bisa dihapus via bot.")
                continue
            conn=db()
            ...
            if cur.rowcount>0:
                removed.append(aid)
    await state.clear()
    await message.answer(f"✅ Hapus: {', '.join([f'<code>{a}</code>' for a in removed])}" if removed else "❌ Tidak ada yang dihapus.", reply_markup=admin_menu())
