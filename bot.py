...
def parse_data_with_akun(raw_text: str) -> dict:
    ...
    # AKUN: email/password. Password boleh mengandung spasi.
    m = re.search(r'AKUN\s*:\s*\n\s*([^\s@]+@[^\s]+)\s*\n\s*([^\n]+)', text, re.I)
    if m:
        data['EMAIL'], data['PASSWORD'] = m.group(1).strip(), m.group(2).strip()
    else:
        ...
        if em:
            ...
            for line in lines:
                line = line.strip()
                if line and not '@' in line and not re.match(r'^(AKUN|SAHABAT)\b', line, re.I):
                    data['PASSWORD'] = line
                    break
    return data
