...
async def af_selected_fixed(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(amount=amount, package_code=f"af_{amount}", package_name=nama)
    await callback.message.edit_text(f"💳 <b>{nama}</b> {rupiah(amount)}\nKirim FOTO bukti TF di sini. Admin akan dapat tombol SETUJU/TOLAK.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ SUDAH BAYAR - KIRIM BUKTI", callback_data="payment_done")]]))
