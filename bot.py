...
# ===== FIX 2026 =====
KEYWORD_LOKASI_BLACKLIST = ["provinsi","kota","kabupaten","kab","kecamatan","kec","kelurahan","kel","desa","jakarta","bandung","surabaya","medan","bekasi","tangerang","bogor","depok","semarang","yogyakarta","jogja","bali","denpasar"]
...
async def green_webhook(request):
    ...
    try:
        ...
        try:
            ...
            print(f"kw err {e}")
        try:
            conn.execute("INSERT INTO wa_inbox_log(wa_group,wa_sender,message,parsed_kab,parsed_kec,created_at) VALUES(?,?,?,?,?,?)", (chat_name, sender_name, text_msg[:4000], kab or "", kec or "", datetime.now().isoformat()))
            conn.commit()
        except Exception as e:
...
@dp.message(PaymentState.waiting_proof, F.document)
async def payment_proof_doc(message: Message, state: FSMContext):
    # FIX bukti sebagai dokumen juga diterima
    # convert document to photo-like handling
    ...
    if amount <= 0:
        await state.clear()
        await message.answer("❌ Nominal tidak ditemukan.")
        return
    now = datetime.now().isoformat()
    conn = db()
    cur = conn.cursor()
    ...
    """, (message.from_user.id, amount, data.get("package_code"), data.get("package_name"), photo_file_id, now))
    tx_id = cur.lastrowid
    cur.execute("""
        INSERT INTO bukti_tf(
            telegram_id, username, full_name, file_id, file_type,
    ...
    """, (message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", photo_file_id, "document", f"{data.get('package_name') or 'TOP UP SALDO'} - {amount}", amount, "pending", now))
    bukti_id = cur.lastrowid
    conn.commit()
    conn.close()
    await state.clear()
    await message.answer(f"✅ <b>BUKTI TF #{tx_id} BERHASIL DIKIRIM!</b>\n\n💰 Nominal : {rupiah(amount)}\n⏳ Tunggu persetujuan admin.")
    await kirim_bukti_ke_admin(message.bot, bukti_id, message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", photo_file_id, "document", f"{data.get('package_name') or 'TOP UP SALDO'} - {rupiah(amount)}", amount, tx_id, data.get("package_name") or "TOP UP SALDO")

@dp.message(PaymentState.waiting_proof)
async def payment_wrong_proof(message: Message):
    await message.answer("📸 Silakan kirim FOTO bukti pembayaran. Jika kamu sudah ketik nominal, tekan ✅ SUDAH BAYAR dulu, baru kirim foto.")
...
async def process_payment(callback: CallbackQuery, tx_id: int, approve: bool):
    ...
    if approve:
        ...
        if pkg.startswith("kota_"):
            ...
            # FIX: top up saldo biasa juga dapat quota 3 KOTA (sesuai request user)
            conn.execute("UPDATE users SET balance=balance+? WHERE telegram_id=?", (tx["amount"], tx["telegram_id"]))
            try:
                conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+?", (tx["telegram_id"], 3, 3))
            except Exception as e:
                print(f"quota add err {e}")
        conn.commit()
        conn.close()
        exp_txt = ""
        if pkg:
...
async def format_search_receive(message: Message, state: FSMContext):
    data_kw = await state.get_data()
    search_type = data_kw.get("search_type")
    if search_type == "keyword_add":
        ...
        if is_keyword_lokasi(kw_input):
            await message.answer("❌ <b>Keyword tidak boleh mengandung lokasi!</b>\n\nJangan pakai: KOTA, KECAMATAN, PROVINSI, JAKARTA dll\n\n✅ Contoh: info penipu, info klaim, link dana kaget")
            return
        try:
            conn = db()
            ...
            conn.close()
            await state.clear()
            await message.answer(f"✅ Keyword {html.escape(kw_input)} berhasil disimpan!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Kembali ke KEYWORD", callback_data="keyword_menu")]]))
        except Exception as e:
...
async def kota_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    if is_fitur_terkunci(callback.from_user.id):
        await state.clear()
        await callback.message.edit_text(
            f"🏙️ <b>TAMBAH KOTA - PILIH PAKET</b>\n\n"
            f"📊 Sisa quota kamu: {quota} KOTA\n"
            f"💎 <b>Paket tersedia:</b>\n"
            f"⏰ 1 Minggu - Rp 40.000 (3x)\n"
            f"📅 1 Bulan - Rp 120.000 (3x)\n"
            f"📅 2 Bulan - Rp 220.000 (3x)\n"
            f"📅 6 Bulan - Rp 600.000 (3x)\n"
            f"♾️ Unlimited - Rp 2.000.000",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
