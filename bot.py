...
def get_all_admins():
    try:
        ...
        conn.close()
        db_admins = {r["telegram_id"] for r in rows}
        return ADMIN_IDS.union(db_admins)
    except:
        return ADMIN_IDS
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 CEK SALDO USER", callback_data="admin_check_balance")],
        [InlineKeyboardButton(text="➕ TAMBAH SALDO", callback_data="admin_add_balance")],
        [InlineKeyboardButton(text="➖ KURANGI SALDO", callback_data="admin_reduce_balance")],
...
async def admin_list_admin(callback: CallbackQuery):
    ...
    db_admins_text = ""
    if rows:
        db_admins_text = "\n".join([f"• <code>{r['telegram_id']}</code> - ditambah oleh {r['added_by']} pada {r['created_at'][:10]}" for r in rows])
    text = f"👥 <b>DAFTAR ADMIN</b>\n\n<b>Dari ENV:</b>\n{env_admins or '-'}\n\n<b>Dari DB:</b>\n{db_admins_text or '-'}"
...
async def admin_tambah_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "➕ <b>TAMBAH ADMIN</b>\n\nKirim Telegram ID admin baru.\nContoh: <code>123456789</code>\n\nBisa juga multiple ID dipisah koma: <code>123, 456, 789</code>",
...
async def admin_tambah_admin_process(message: Message, state: FSMContext):
    ...
    await state.clear()
    if added:
        await message.answer(f"✅ Berhasil tambah {len(added)} admin:\n" + "\n".join([f"<code>{a}</code>" for a in added]), reply_markup=admin_menu())
    else:
        await message.answer("❌ Tidak ada admin baru ditambahkan (sudah ada atau ID tidak valid).", reply_markup=admin_menu())
...
async def admin_hapus_admin_start(callback: CallbackQuery, state: FSMContext):
    ...
    rows = conn.execute("SELECT telegram_id FROM admins").fetchall()
    conn.close()
    list_text = "\n".join([f"• <code>{r['telegram_id']}</code>" for r in rows]) if rows else "Tidak ada admin tambahan di DB."
    await callback.message.edit_text(
        f"➖ <b>HAPUS ADMIN</b>\n\nDaftar admin DB:\n{list_text}\n\nKirim ID yang mau dihapus. Contoh: <code>123456789</code>",
        reply_markup=admin_menu()
    )
    await callback.answer()
...
async def admin_hapus_admin_process(message: Message, state: FSMContext):
    ...
    for part in raw:
        if part.isdigit():
            aid = int(part)
            if aid in ADMIN_IDS:
                await message.answer(f"❌ ID <code>{aid}</code> adalah admin ENV, tidak bisa dihapus via bot. Hapus dari Railway Variables.")
                continue
            conn = db()
    ...
    await state.clear()
    if removed:
        await message.answer(f"✅ Berhasil hapus {len(removed)} admin:\n" + "\n".join([f"<code>{a}</code>" for a in removed]), reply_markup=admin_menu())
    else:
        await message.answer("❌ Tidak ada admin yang dihapus.", reply_markup=admin_menu())
