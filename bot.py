...
async def green_webhook(request):
    ...
    try:
        ...
        print(f"📨 GROUP={chat_name} TEXT={text_msg[:2500]}")

        def is_valid_kota_name(s: str):
            if not s:
                return False
            s_up = s.upper().strip()
            blacklist = ["NIK","KPJ","SALDO","LAHIR","TUNGGAL","KERJA","JK","BPJS","JHT","RUPIAH"]
            if any(b in s_up for b in blacklist):
                return False
            if not re.match(r'^[A-Z ]{3,30}$', s_up):
                return False
            if re.match(r'^\d+$', s_up):
                return False
            if len(s_up.split()) > 4:
                return False
            return True
        ...
        def parse_kota_final(text):
            if not text:
                return None, None
            txt_raw = text.strip()
            lines = [l.strip() for l in txt_raw.split("\n") if l.strip() != ""]
            clean_lines = []
            for l in lines[:5]:
            ...
            if len(clean_lines) < 2:
                first = clean_lines[0] if clean_lines else ""
                if "," in first:
                    parts = [p.strip() for p in first.split(",") if p.strip()]
                    if len(parts) >= 2:
                        kab_c = parts[0].upper()
                        kec_c = parts[1].upper()
                        if is_valid_kota_name(kab_c) and is_valid_kota_name(kec_c):
                            print(f"✅ PARSE KOMA 2 KUNCI: KAB={kab_c} KEC={kec_c}")
                            return kab_c, kec_c
                print(f"❌ SKIP - HANYA 1 BARIS: {clean_lines}")
                return None, None

            tu = txt_raw.upper()
            m_kab = re.search(r'KAB\s*[:\-]?\s*([A-Z0-9 ]{3,30})', tu)
            m_kec = re.search(r'KEC\s*[:\-]?\s*([A-Z0-9 ]{3,30})', tu)
            if m_kab and m_kec:
                kab = re.sub(r'[^A-Z0-9 ]', '', m_kab.group(1).strip()).strip()
                kec = re.sub(r'[^A-Z0-9 ]', '', m_kec.group(1).strip()).strip()
                if is_valid_kota_name(kab) and is_valid_kota_name(kec):
                    print(f"✅ PARSE LABEL 2 KUNCI: KAB={kab} KEC={kec}")
                    return kab, kec

            first_raw = clean_lines[0]
            second_raw = clean_lines[1]
            first_clean = re.sub(r'^(KOTA|KABUPATEN|KAB)\s*[:\-]?\s*', '', first_raw, flags=re.I).strip().upper()
            second_clean = re.sub(r'^(KECAMATAN|KEC)\s*[:\-]?\s*', '', second_raw, flags=re.I).strip().upper()
            first_clean = re.sub(r'[^A-Z ]', ' ', first_clean).strip()
            first_clean = re.sub(r'\s+', ' ', first_clean)
            second_clean = re.sub(r'[^A-Z ]', ' ', second_clean).strip()
            second_clean = re.sub(r'\s+', ' ', second_clean)

            if is_valid_kota_name(first_clean) and is_valid_kota_name(second_clean):
                print(f"✅ PARSE 2 BARIS WAJIB 2 KUNCI: KAB={first_clean} KEC={second_clean}")
                return first_clean, second_clean
            else:
                print(f"❌ SKIP VALIDASI: first='{first_clean}' second='{second_clean}'")
                return None, None

        kab, kec = parse_kota_final(text_msg)

        if not kab or not kec:
            print(f"❌ FINAL PARSE GAGAL 2 KUNCI: KAB={kab} KEC={kec} -> SKIP NO ADMIN")
            return web.json_response({"ok": True})

        print(f"✅ FINAL PARSE 2 KUNCI: KAB={kab} KEC={kec} group={chat_name}")

        conn = db()
        targets = []
        try:
            rows = conn.execute("SELECT telegram_id FROM users WHERE upper(kab)=? AND upper(kec)=?", (kab.upper(), kec.upper())).fetchall()
            targets = [r["telegram_id"] for r in rows]
            if not targets:
        ...
        if not targets:
            print(f"⚠️ Tidak ada user untuk {kab}/{kec} - SKIP NO ADMIN")
            return web.json_response({"ok": True})

        header = f"📢 INFO JHT MASUK - {kab} / {kec}"
        footer = "\n\n--------------------------\nPERHATIAN :\nTetap hati-hati dan waspada dalam bertransaksi untuk lebih aman gunakan jasa rekber. Terimakasih\nSumber: https://t.me/seduluranjht_bot"
        forward_text = f"{header}\n\n{text_msg}\n{footer}"

        bot = request.app['bot']
        success = 0
        for uid in targets[:300]:
            try:
                await bot.send_message(chat_id=uid, text=forward_text[:4000])
                success += 1
                await asyncio.sleep(0.05)
            except Exception as e:
                print(f"❌ Gagal {uid}: {e}")

        print(f"✅ FORWARDED {success}/{len(targets)} untuk {kab}/{kec} - NO ADMIN")
...
async def payment_proof(message: Message, state: FSMContext):
    ...
    await message.answer(
        "✅ <b>BUKTI TF #{tx_id} BERHASIL DIKIRIM!</b>\n\n💰 {rupiah(amount)}\n📦 {data.get('package_name')}\n\n⏳ <b>TUNGGU PERSETUJUAN ADMIN</b>\nAdmin sudah menerima foto\n\n"
        f"🧾 Transaksi : #{tx_id}\n"
...
async def keyword_menu(callback: CallbackQuery):
    ...
    text = (
        ...
        "Fungsi:\n"
        "Ketika ada pesan di WA yang mengandung kata kunci yang kamu set, kamu akan dapat notifikasi langsung!\n\n"
        "📌 Contoh:\n"
...
async def keyword_add_save(message: Message, state: FSMContext):
    ...
    conn.close()
    await state.clear()
    await message.answer(f"✅ {added} keyword ditambahkan! Kamu akan dapat notifikasi jika ada pesan WA mengandung keyword tersebut.", reply_markup=main_menu(message.from_user.id))
