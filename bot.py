...
def find_users_from_wa_message(wa_text: str):
    """Cari user berdasarkan KAB/KOTA + KEC di pesan WA.
    Tidak mensyaratkan keduanya berurutan/berada di baris berbeda.
    """
    ...
    try:
        ...
        for r in rows:
            ...
            nkab, nkec = _norm_region_name(kab), _norm_region_name(kec)
            if not nkab or not nkec:
                continue
            # Cocokkan dua kunci di mana pun posisinya dalam pesan.
            kab_ok = re.search(r'(?<![A-Z0-9])' + re.escape(nkab) + r'(?![A-Z0-9])', normalized)
            kec_ok = re.search(r'(?<![A-Z0-9])' + re.escape(nkec) + r'(?![A-Z0-9])', normalized)
            if kab_ok and kec_ok:
                matches.append((int(r['telegram_id']), kab, kec))
        # Satu Telegram ID hanya menerima satu notifikasi.
        unique = {}
        for item in matches:
            unique[item[0]] = item
