...
# ========== FIX: AUTO FORMAT & CARI LAINNYA - SAME FLOW AS TAMBAH KOTA ==========
@dp.callback_query(F.data == "auto_format")
async def auto_format_menu_fixed(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "📝 <b>AUTO FORMAT JHT</b>\n\n"
        "Pilih paket langganan Auto Format:\n\n"
        "• 1 Minggu - Rp 40.000\n"
        "• 1 Bulan - Rp 100.000\n"
        "• 6 Bulan - Rp 250.000\n"
        "• 1 Tahun - Rp 400.000\n\n"
        "Setelah pilih paket, upload bukti transfer dan admin akan approve (sama seperti TAMBAH KOTA).",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="1 Minggu - Rp 40.000", callback_data="af_40000")],
            [InlineKeyboardButton(text="1 Bulan - Rp 100.000", callback_data="af_100000")],
            [InlineKeyboardButton(text="6 Bulan - Rp 250.000", callback_data="af_250000")],
            [InlineKeyboardButton(text="1 Tahun - Rp 400.000", callback_data="af_400000")],
...
async def auto_format_select_fixed(callback: CallbackQuery, state: FSMContext):
    amount = int(callback.data.replace("af_",""))
    paket_map = {
        40000: ("AUTO FORMAT 1 Minggu", "af_1w"),
        100000: ("AUTO FORMAT 1 Bulan", "af_1m"),
        250000: ("AUTO FORMAT 6 Bulan", "af_6m"),
        400000: ("AUTO FORMAT 1 Tahun", "af_1y"),
    }
    nama, kode = paket_map.get(amount, (f"AUTO FORMAT {amount}", f"af_{amount}"))
    await state.clear()
    await state.set_state(PaymentState.waiting_proof)
    await state.update_data(amount=amount, package_code=kode, package_name=nama)
    await callback.message.edit_text(
        f"💳 <b>TOP UP - {nama.upper()}</b>\n\n"
        f"📦 Paket: <b>{nama}</b>\n"
        f"💰 Harga: <b>{rupiah(amount)}</b>\n\n"
        "Silakan transfer ke:\n"
        "🏦 SEABANK 901040978290 A/N HAMBALI\n"
        "💰 DANA 083824101264 A/N HAMBALI\n\n"
        f"Setelah transfer {rupiah(amount)}, kirim FOTO bukti di sini.\n"
        "Admin akan dapat notifikasi + tombol SETUJU/TOLAK.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def cari_lainnya_menu_fixed(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "🔎 <b>CARI LAINNYA - SEARCH KOTA LAIN</b>\n\n"
        "Fitur untuk mencari history JHT di kota lain.\n\n"
        "Pilih paket:\n"
        "• 1 Minggu - Rp 30.000\n"
        "• 1 Bulan - Rp 80.000\n"
        "• 6 Bulan - Rp 200.000\n\n"
        "Setelah pilih paket, upload bukti dan admin akan approve (sama seperti TAMBAH KOTA & AUTO FORMAT).",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="1 Minggu - Rp 30.000", callback_data="cl_30000")],
            [InlineKeyboardButton(text="1 Bulan - Rp 80.000", callback_data="cl_80000")],
            [InlineKeyboardButton(text="6 Bulan - Rp 200.000", callback_data="cl_200000")],
            [InlineKeyboardButton(text="⬅️ Kembali", callback_data="back_main")]
        ])
    )
    await callback.answer()
...
async def cari_lainnya_select_fixed(callback: CallbackQuery, state: FSMContext):
    amount = int(callback.data.replace("cl_",""))
    paket_map = {
        30000: ("CARI LAINNYA 1 Minggu", "cari_1w"),
        80000: ("CARI LAINNYA 1 Bulan", "cari_1m"),
        200000: ("CARI LAINNYA 6 Bulan", "cari_6m"),
    }
    nama, kode = paket_map.get(amount, (f"CARI LAINNYA {amount}", f"cari_{amount}"))
    await state.clear()
    await state.set_state(PaymentState.waiting_proof)
    await state.update_data(amount=amount, package_code=kode, package_name=nama)
    await callback.message.edit_text(
...
# ========== FIX: PAYMENT PROOF - PASTI KIRIM KE ADMIN + PESAN TUNGGU ADMIN ==========
@dp.message(PaymentState.waiting_proof, F.photo)
async def payment_proof_fixed(message: Message, state: FSMContext):
    data = await state.get_data()
    amount = int(data.get("amount", 0))
    if amount <= 0:
        await state.clear()
        await message.answer("❌ Nominal tidak ditemukan. Ulangi dari menu.")
        return
    ...
    # PESAN KE USER - TUNGGU PERSETUJUAN ADMIN (INI YANG DIMINTA)
    await message.answer(
        ...
        f"Mohon tunggu 1-5 menit.",
        reply_markup=main_menu(message.from_user.id)
    )

    # NOTIFIKASI KE ADMIN - PASTI DAPAT + TOMBOL SETUJU/TOLAK
    caption = (
        f"💰 <b>BUKTI TF BARU MASUK #{tx_id}</b>\n\n"
        f"👤 Nama: {message.from_user.full_name}\n"
        f"🆔 ID: <code>{message.from_user.id}</code>\n"
        f"📱 Username: @{message.from_user.username or '-'}\n"
        f"💰 Nominal: <b>{rupiah(amount)}</b>\n"
        f"📦 Paket: {data.get('package_name') or 'TOP UP SALDO'}\n"
        f"🟡 Status: PENDING - BUTUH SETUJU/TOLAK"
    )
    ...
    print(f"📤 Mengirim notif bukti #{tx_id} ke {len(ADMIN_IDS)} admin...")
    for admin_id in ADMIN_IDS:
        try:
            await message.bot.send_photo(admin_id, photo.file_id, caption=caption, reply_markup=keyboard)
            print(f"✅ Notif #{tx_id} terkirim ke admin {admin_id}")
        except Exception as e:
            print(f"❌ Gagal kirim ke admin {admin_id}: {e}")
            import logging
            logging.exception("Gagal kirim admin")

@dp.message(PaymentState.waiting_proof)
async def payment_wrong_proof_fixed(message: Message):
    await message.answer("📸 Silakan kirim FOTO bukti pembayaran (bukan text/file).")
...
async def keyword_menu(callback: CallbackQuery):
    ...
    txt = (
        ...
        "Fungsi:\n"
        "Ketika ada pesan di WhatsApp yang mengandung kata kunci yang kamu set, kamu akan dapat notifikasi langsung!\n\n"
        "📌 Contoh:\n"
        "• aku lagi sebar dana kaget\n"
        "• info penipu\n"
        "• info JMO terkini\n\n"
...
@dp.callback_query(F.data == "keyword_list")
async def keyword_list(callback: CallbackQuery):
    conn = db()
    rows = conn.execute("SELECT id, keyword FROM keywords WHERE telegram_id=? ORDER BY created_at DESC", (callback.from_user.id,)).fetchall()
    conn.close()
    if not rows:
        txt = "📋 <b>KEYWORD SAYA</b>\n\nBelum ada keyword. Klik Tambah Keyword."
    else:
        txt = "📋 <b>KEYWORD SAYA</b>\n\nDaftar keyword yang dipantau:\n\n"
        for i,r in enumerate(rows,1):
            txt += f"{i}. <code>{html.escape(r['keyword'])}</code>\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ TAMBAH KEYWORD", callback_data="keyword_add"), InlineKeyboardButton(text="🗑️ HAPUS KEYWORD", callback_data="keyword_delete")],
        [InlineKeyboardButton(text="⬅️ Kembali", callback_data="keyword_menu")]
    ])
    await callback.message.edit_text(txt, reply_markup=kb)
    await callback.answer()
...
async def keyword_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "➕ <b>TAMBAH KEYWORD</b>\n\n"
        "Ketik kata kunci yang ingin dipantau di WA.\n\n"
        "Contoh:\n"
        "aku lagi sebar dana kaget\n"
        "info penipu\n"
        "info JMO terkini\n\n"
        "Kirim 1 per baris, bisa beberapa sekaligus.\n\n"
        "Ketik /cancel untuk batal.",
        reply_markup=back_main()
    )
    await callback.answer()

@dp.message(KeywordState.waiting_add, F.text)
async def keyword_add_save(message: Message, state: FSMContext):
    if message.text.strip() == "/cancel":
        await state.clear()
        await message.answer("Dibatalkan.", reply_markup=main_menu(message.from_user.id))
        return
    lines = [l.strip() for l in message.text.strip().split("\n") if l.strip()]
    conn = db()
    added = 0
    for kw in lines:
        if len(kw) < 3 or len(kw) > 100:
            continue
        exists = conn.execute("SELECT id FROM keywords WHERE telegram_id=? AND lower(keyword)=?", (message.from_user.id, kw.lower())).fetchone()
        if exists:
            continue
        conn.execute("INSERT INTO keywords (telegram_id, keyword, created_at) VALUES (?,?,?)", (message.from_user.id, kw, datetime.now().isoformat()))
        added += 1
    conn.commit()
    conn.close()
    await state.clear()
    await message.answer(f"✅ {added} keyword ditambahkan!\n\nKamu akan dapat notifikasi jika ada pesan WA mengandung keyword tersebut.", reply_markup=main_menu(message.from_user.id))
...
async def keyword_delete_menu(callback: CallbackQuery):
    ...
    for r in rows:
        label = r['keyword'][:25]
        kb.append([InlineKeyboardButton(text=f"🗑️ {label}", callback_data=f"kw_del_{r['id']}")])
    kb.append([InlineKeyboardButton(text="⬅️ Kembali", callback_data="keyword_menu")])
    await callback.message.edit_text("🗑️ <b>HAPUS KEYWORD</b>\nPilih yang mau dihapus:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()
...
async def green_webhook(request):
    try:
        ...
        print(f"📩 GREEN RAW: {str(data)[:4000]}")
    except Exception as e:
        tb = await request.text()
        print(f"RAW ERR {e}: {tb[:2000]}")
    ...
    try:
        ...
        def is_valid_kota(s: str):
            if not s: return False
            su = s.upper().strip()
            blacklist = ["NIK","KPJ","SALDO","LAHIR","TUNGGAL","KERJA","JK","BPJS","JHT","RUPIAH"]
            if any(b in su for b in blacklist): return False
        ...
        if not all_targets:
            print(f"❌ SKIP KAB={kab} KEC={kec} NO KW - NO ADMIN")
            conn.close()
            return web.json_response({"ok": True})
        ...
        conn.close()
        print(f"✅ FORWARDED kota={len(targets_kota)} kw={len(targets_keyword)} total={success} - FORMAT BARU + 2 TOMBOL")
...
# Cache untuk tombol SALIN
salin_cache_mem = {}

@dp.callback_query(F.data.startswith("salin_"))
async def salin_handler(callback: CallbackQuery):
    cache_id = callback.data.replace("salin_","")
    content = salin_cache_mem.get(cache_id)
    if not content:
        try:
            conn = db()
            row = conn.execute("SELECT content FROM salin_cache WHERE id=?", (cache_id,)).fetchone()
            conn.close()
            if row:
                content = row["content"]
        except:
            content = None
    if not content:
        await callback.answer("Pesan expired", show_alert=True)
        return
...
async def kirim_bukti_ke_admin(bot, bukti_id, user_id, username, full_name, file_id, file_type, caption_user, amount=0):
    if not ADMIN_IDS:
        return
    ...
    text_admin = (
        ...
        f"📸 Cek bukti di atas, lalu Setuju / Tolak."
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
    ...
    for admin_id in ADMIN_IDS:
        try:
            if file_type == "photo":
                await bot.send_photo(admin_id, file_id, caption=text_admin, reply_markup=kb)
            else:
                await bot.send_document(admin_id, file_id, caption=text_admin, reply_markup=kb)
            await asyncio.sleep(0.15)
        except Exception as e:
            logging.exception(f"Gagal kirim bukti TF ke admin {admin_id}: {e}")
...
async def admin_bukti_approve(callback: CallbackQuery):
    ...
    if not row or row["status"] != "pending":
        ...
        return
    conn.execute("UPDATE bukti_tf SET status='approved', decided_by=?, decided_at=? WHERE id=?", (callback.from_user.id, datetime.now().isoformat(), bukti_id))
    conn.execute("UPDATE transactions SET status='approved', processed_at=? WHERE proof_file_id=? AND status='pending'", (datetime.now().isoformat(), row["file_id"]))
    # kalau ada nominal, tambahkan ke balance user sebagai contoh
    if row["amount"] and row["amount"] > 0:
        conn.execute("UPDATE users SET balance=balance+? WHERE telegram_id=?", (row["amount"], row["telegram_id"]))
    conn.commit()
    conn.close()
    try:
        await callback.message.edit_caption(caption=(callback.message.caption or "") + f"\n\n✅ <b>DISETUJUI oleh {callback.from_user.id}</b>")
    except:
        try:
            ...
            pass
    try:
        await callback.bot.send_message(row["telegram_id"], f"✅ <b>BUKTI TF #{bukti_id} DISETUJUI!</b>\n\nTerima kasih, pembayaran kamu telah dikonfirmasi admin. Saldo / akses sudah aktif.", reply_markup=main_menu(row["telegram_id"]))
    except:
        pass
