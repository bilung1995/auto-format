...
async def green_webhook(request):
    """FINAL GREEN API WEBHOOK - WA Grup -> Telegram by Kota"""
    try:
    ...
    # Log raw untuk debug
    print(f"📩 GREEN WEBHOOK: {str(data)[:500]}")
    ...
    chat_name = sender_data.get('chatName','') or sender_data.get('senderName','')
    sender = sender_data.get('sender','') or data.get('sender','')
    sender_name = sender_data.get('senderName','') or ''
    
    # text bisa di beberapa tempat
    wa_text = ''
    if 'textMessageData' in message_data:
        wa_text = message_data['textMessageData'].get('textMessage','')
    elif 'extendedTextMessageData' in message_data:
        wa_text = message_data['extendedTextMessageData'].get('text','')
    elif 'quotedMessage' in message_data:
        wa_text = message_data['quotedMessage'].get('textMessage','') or str(message_data.get('quotedMessage',''))[:1000]
    else:
        wa_text = data.get('message','') or data.get('text','') or ''
    
    # Jika bukan grup, skip? Tapi tetap log. Kita hanya forward yang ada KAB/KEC
    is_group = chat_id.endswith('@g.us') or 'g.us' in chat_id or data.get('isGroup', False)
    wa_group = chat_name or chat_id or 'WA GROUP'
    
    if not wa_text:
        # Kadang Green kirim file, caption ada di fileMessageData
        if 'fileMessageData' in message_data:
            wa_text = message_data['fileMessageData'].get('caption','') or ''
        if 'imageMessageData' in message_data:
            wa_text = message_data['imageMessageData'].get('caption','') or ''

    if not wa_text:
        print("⚠️ Green webhook tanpa text, skip")
        return web.json_response({"status": False, "reason": "no text"})

    # Parse KAB/KEC
    kab, kec = parse_kota_kec_from_text_v2(wa_text)
    print(f"🔍 GREEN PARSE: KAB={kab} KEC={kec} from {wa_group}")

    # Simpan log
    try:
        conn = db()
        conn.execute("INSERT INTO wa_inbox_log(wa_group,wa_sender,message,parsed_kab,parsed_kec,created_at) VALUES(?,?,?,?,?,?)",
                     (wa_group, f"{sender_name} ({sender})", wa_text, kab or '', kec or '', datetime.now().isoformat()))
        db_commit_and_sync(conn)
        conn.close()
    except Exception as e:
        print(f"log fail {e}")
    ...
    bot = request.app['bot']
    forwarded = await forward_wa_to_telegram(bot, wa_text, wa_group, sender_name or sender, kab, kec)
    ...
    return web.json_response({
        ...
        "kec": kec,
        "forwarded": forwarded,
        "text_preview": wa_text[:200]
    })

# Untuk kompatibilitas kalau Fonnte masih kepencet, redirect ke green handler
async def fonnte_webhook(request):
    # Jika provider green, tetap coba parse sebagai green juga
    if WA_PROVIDER == "green":
        return await green_webhook(request)
    # fallback fonnte lama
    try:
        data = await request.json()
    except:
        try:
            ...
            data = {}
    wa_text = data.get('message') or data.get('text') or ''
    wa_group = data.get('group') or 'Fonnte Group'
    wa_sender = data.get('sender') or ''
    if not wa_text and isinstance(data.get('data'), dict):
        wa_text = data['data'].get('message','')
    if not wa_text:
        wa_text = str(data)[:3000]
    kab, kec = parse_kota_kec_from_text_v2(wa_text)
    try:
        conn = db()
        conn.execute("INSERT INTO wa_inbox_log(wa_group,wa_sender,message,parsed_kab,parsed_kec,created_at) VALUES(?,?,?,?,?,?)",
    ...
    return web.json_response({"status": True, "provider": "fonnte", "forwarded": forwarded, "kab": kab, "kec": kec})
...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="💳 KIRIM BUKTI TF", callback_data="kirim_bukti_tf"), InlineKeyboardButton(text="🔎 CARI LAINNYA", callback_data="kota_search_lain")],
