...
async def keyword_menu(callback: CallbackQuery):
    if is_banned(callback.from_user.id):
        await callback.answer("❌ Akun dibanned", show_alert=True)
        return
    # Ambil keywords user dari database atau default
    conn = db()
    try:
        # Buat tabel keywords kalau belum ada
        ...
        conn.commit()
        rows = conn.execute("SELECT keyword, kota, kec FROM keywords WHERE telegram_id=? ORDER BY created_at DESC LIMIT 20", (callback.from_user.id,)).fetchall()
    except:
        rows = []
    conn.close()

    txt = "🔑 <b>MENU KEYWORD</b>\n\n"
    txt += "Fitur ini untuk set kata kunci auto-detect JHT sesuai kota kamu.\n\n"
    if rows:
        txt += "<b>Keyword Aktif:</b>\n"
        for i,r in enumerate(rows,1):
            txt += f"{i}. {r['keyword']} - {r['kota'] or '-'} / {r['kec'] or '-'}\n"
    else:
        txt += "Belum ada keyword. Tambahkan keyword untuk filter JHT lebih akurat.\n"
