...
# Support koma, spasi, enter, semua format
ADMIN_IDS = {int(x) for x in re.findall(r"\d+", _raw_admin) if len(x) >= 5}
print(f"🔧 ADMIN_IDS RAW: '{_raw_admin}' -> PARSED: {ADMIN_IDS}")
...
# === WA PROVIDER ===
WA_PROVIDER = "green"
GREEN_API_ID = os.getenv("GREEN_API_ID", os.getenv("GREEN_ID_INSTANCE","")).strip()
GREEN_API_TOKEN = os.getenv("GREEN_API_TOKEN", os.getenv("GREEN_API_TOKEN_INSTANCE","")).strip()
FONNTE_TOKEN = os.getenv("FONNTE_TOKEN", "").strip()
FONNTE_DEVICE = os.getenv("FONNTE_DEVICE", "").strip()
WABLAS_TOKEN = ""
WABLAS_DOMAIN = ""
WABLAS_DEVICE = ""
STARSENDER_TOKEN = ""
...
# === ADMIN WA NOTIF ===
ADMIN_WA_NUMBERS_RAW = os.getenv("ADMIN_WA_NUMBERS", "").strip()  # contoh: 6281234567890,628987654321
ADMIN_WA_NUMBERS = [re.sub(r'[^0-9]', '', x) for x in re.split(r'[,
 ]+', ADMIN_WA_NUMBERS_RAW) if x.strip()]

def normalize_wa_number(phone: str) -> str:
...
async def send_wa_to_admins(text_msg: str):
    """Kirim WA notif ke semua admin via GREEN API - WAJIB"""
    if not GREEN_API_ID or not GREEN_API_TOKEN:
        print("⚠️ GREEN_API_ID/TOKEN belum diset - WA admin notif gagal")
        return False
    if not ADMIN_WA_NUMBERS:
        print("⚠️ ADMIN_WA_NUMBERS kosong - set di Railway! Contoh: 6281234567890")
        return False
    ok = 0
    for raw in ADMIN_WA_NUMBERS:
        phone = normalize_wa_number(raw)
        chat_id = f"{phone}@c.us"
        try:
            async with aiohttp.ClientSession() as session:
                url = f"https://api.green-api.com/waInstance{GREEN_API_ID}/sendMessage/{GREEN_API_TOKEN}"
                payload = {"chatId": chat_id, "message": text_msg}
                async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=20)) as resp:
                    txt_resp = await resp.text()
                    print(f"GREEN ADMIN NOTIF {resp.status} to {phone}: {txt_resp[:300]}")
                    if resp.status == 200:
                        ok += 1
        except Exception as e:
            print(f"GREEN admin notif error to {phone}: {e}")
    print(f"✅ WA admin terkirim ke {ok}/{len(ADMIN_WA_NUMBERS)} admin")
    return ok > 0

def send_wa_to_admins_background(text_msg: str):
    try:
        import threading
        def run():
            try:
                ...
                loop.run_until_complete(send_wa_to_admins(text_msg))
                loop.close()
            except Exception as e:
                print(f"Background WA admin error: {e}")
        threading.Thread(target=run, daemon=True).start()
        print(f"🚀 WA ADMIN background task started")
        return True
    except Exception as e:
        print(f"WA admin background gagal: {e}")
        return False
...
def parse_kode_input(text: str):
    """Parse input kode format seperti 'JPG - 001' atau '🤖 JPG - 001 🤖'"""
    text = text.strip()
    
    # Cek pola dengan angka di akhir: prefix - 001
    ...
    # Cek pola dengan suffix: prefix - 001 suffix
    m = re.search(r'^(.*?)(\d+)(.*?)$', text)
...
def generate_next_code(telegram_id: int) -> str:
    """Generate kode format berikutnya untuk user"""
    try:
        conn = db()
        row = conn.execute(
            "SELECT prefix, suffix, current_number, padding FROM format_codes WHERE telegram_id=? AND enabled=1",
            (telegram_id,)
        ).fetchone()
        ...
        num = row['current_number'] or 1
        padding = row['padding'] or 3
        
        # Update current number untuk next
        conn.execute(
            "UPDATE format_codes SET current_number=? WHERE telegram_id=?",
            (num + 1, telegram_id)
        )
        conn.commit()
        conn.close()
        
        kode = f"{prefix} {str(num).zfill(padding)} {suffix}".strip()
        return kode
    except Exception as e:
        print(f"generate_next_code error: {e}")
        return ""
...
async def send_wa_notif_kota(phone_number: str, kota_nama: str, user_name: str = ""):
    """Kirim notif WA via GREEN API - SATSET 0.5 DETIK - KHUSUS BOT BARU"""
    msg = f"""✅ *KOTA DIPILIH*
...
📍 SAHABAT JHT BOT BARU V2 - GREEN API SATSET"""

    if not GREEN_API_ID or not GREEN_API_TOKEN:
        print("⚠️ GREEN_API_ID/TOKEN belum diset di Railway Variables")
        return False
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                txt_resp = await resp.text()
                print(f"GREEN {resp.status}: {txt_resp[:400]}")
                if resp.status == 200:
                    print(f"✅ GREEN API SATSET ke {phone} - kota {kota_nama}")
                    return True
                else:
                    print(f"❌ GREEN gagal {resp.status}: {txt_resp[:500]}")
                    return False
    except Exception as e:
    ...
    try:
        ...
        def run():
            try:
                ...
                print(f"Background GREEN error: {e}")
        threading.Thread(target=run, daemon=True).start()
        print(f"🚀 GREEN background task started untuk untuk {phone_number} - {kota_nama}")
        return True
    except Exception as e:
...
def init_database():
    ...
    CREATE TABLE IF NOT EXISTS bukti_tf (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        telegram_id INTEGER NOT NULL,
        username TEXT,
        full_name TEXT,
...
def has_cari_lainnya_access(user_id):
    try:
        ...
        if not sub:
            return False
        pkg = (sub.get("package_code") or "").lower()
...
# ====== FIX TAMBAHAN 2026: QUOTA LOCK & KEYWORD ANTI LOKASI ======
KEYWORD_LOKASI_BLACKLIST = [
    "provinsi","kota","kabupaten","kab","kecamatan","kec","kelurahan","kel","desa",
    "jakarta","bandung","surabaya","medan","bekasi","tangerang","bogor","depok",
    "semarang","yogyakarta","jogja","bali","denpasar","malang","sidoarjo"
]

def is_keyword_lokasi(text: str) -> bool:
    low = text.lower().strip()
    if len(low) < 3:
        return True
    for blk in KEYWORD_LOKASI_BLACKLIST:
        if blk in low:
            return True
    if re.search(r'\b(kota|kab|kec|kel|desa|prov|kecamatan|kabupaten)\b', low):
        return True
    return False

def get_sisa_quota(telegram_id: int) -> int:
    try:
        conn = db()
        row = conn.execute("SELECT quota FROM kota_quota WHERE telegram_id=?", (telegram_id,)).fetchone()
        conn.close()
        return int(row["quota"]) if row and row["quota"] else 0
    except:
        return 0

def is_fitur_terkunci(telegram_id: int) -> bool:
    try:
        if telegram_id in ADMIN_IDS or is_admin(telegram_id):
            return False
        sub = get_subscription(telegram_id)
        if sub and sub.get("status") == "unlimited":
            return False
        # jika ada paket cari_ atau kota_ aktif, anggap tidak terkunci juga
        if sub and sub.get("status") == "active" and sub.get("expiry_date"):
            try:
                if datetime.now() < datetime.fromisoformat(sub["expiry_date"]):
                    # paket aktif masih boleh
                    return False
            except:
                pass
        return get_sisa_quota(telegram_id) <= 0
    except:
        return True
...
def parse_kota_kec_from_text(text: str):
    ...
    if not kab or not kec:
        m_kab = re.search(r'KAB\s*[:\-]\s*([^\n\r,]+)', text, re.IGNORECASE)
        m_kec = re.search(r'KEC\s*[:\-]\s*([^\n\r,]+)', text, re.IGNORECASE)
        if m_kab:
...
FOOTER_PERINGATAN = (
    "━━━━━━━━━━━━━━━━━━━\n"
    "⚠️ REKBER HANYA @Hambali1995\n"
    "💰 SEABANK 901040978290 / DANA 083824101264 A/N HAMBALI\n"
    "🚫 HATI-HATI PENIPUAN MENGATAS NAMAKAN ADMIN"
)

def parse_kota_kec_from_text_v2(text: str):
    if not text:
        return None, None
    kab = None
    kec = None
    for pat in [r'(?:KAB|KABUPATEN|KOTA)\s*[:\-]\s*([^\n\r,;]+)', r'📍\s*KAB\s*:\s*([^\n]+)']:
        m = re.search(pat, text, re.IGNORECASE)
        if m:
            val = m.group(1).strip().split('\n')[0].strip()
            if val and len(val) < 40 and 'RP' not in val.upper():
                kab = val.upper()
                break
    for pat in [r'(?:KEC|KECAMATAN)\s*[:\-]\s*([^\n\r,;]+)', r'📍\s*KEC\s*:\s*([^\n]+)']:
        m = re.search(pat, text, re.IGNORECASE)
        if m:
            val = m.group(1).strip().split('\n')[0].strip()
    ...
    if not kab or not kec:
        lines = [l.strip() for l in text.splitlines() if l.strip()]
        for i, line in enumerate(lines):
            if re.match(r'^\d{3,5}$', line.strip()):
                if i+1 < len(lines) and not kab:
                    cand = lines[i+1].strip().upper()
                    if 3 < len(cand) < 35 and ':' not in cand:
                        kab = cand
                if i+2 < len(lines) and not kec:
                    cand = lines[i+2].strip().upper()
                    if 3 < len(cand) < 40 and ':' not in cand:
                        kec = cand
    if not kab or not kec:
        try:
            conn = db()
            rows = conn.execute("SELECT DISTINCT kab, kec FROM user_kota WHERE kab!='' AND kec!=''").fetchall()
            conn.close()
            low = text.lower()
            for r in rows:
                db_kab = (r['kab'] or '').strip()
                db_kec = (r['kec'] or '').strip()
                if db_kab and db_kec and db_kab.lower() in low and db_kec.lower() in low:
                    return db_kab.upper(), db_kec.upper()
        except:
            pass
    if kab:
        kab = re.sub(r'[^A-Z0-9 ]', '', kab).strip()
...
def parse_kota_kec_from_text(text: str):
    return parse_kota_kec_from_text_v2(text)
...
def find_users_by_kota_kec(kab: str, kec: str):
    ...
    try:
        conn = db()
        kab_clean = kab.strip().upper()
        kec_clean = kec.strip().upper()
        rows = conn.execute("SELECT DISTINCT telegram_id FROM user_kota WHERE UPPER(TRIM(kab))=UPPER(TRIM(?)) AND UPPER(TRIM(kec))=UPPER(TRIM(?))", (kab_clean, kec_clean)).fetchall()
        if not rows:
            rows = conn.execute("SELECT DISTINCT telegram_id FROM user_kota WHERE kab LIKE ? AND kec LIKE ?", (f"%{kab_clean}%", f"%{kec_clean}%")).fetchall()
        valid = []
        for r in rows:
            tid = r['telegram_id']
            if is_admin(tid):
                valid.append(tid)
                continue
            sub = conn.execute("SELECT status, expiry_date FROM subscriptions WHERE telegram_id=?", (tid,)).fetchone()
            if not sub:
                valid.append(tid)
                continue
            if sub['status'] == 'unlimited':
                ...
                try:
                    if sub['expiry_date'] and datetime.fromisoformat(sub['expiry_date']) > datetime.now():
                        valid.append(tid)
        ...
        print(f"find_users error: {e}")
        return []

async def forward_wa_to_telegram(bot, wa_text: str, wa_group: str, wa_sender: str, parsed_kab: str, parsed_kec: str):
    if not parsed_kab or not parsed_kec:
        parsed_kab, parsed_kec = parse_kota_kec_from_text_v2(wa_text)
    print(f"FINAL PARSE: KAB={parsed_kab} KEC={parsed_kec} group={wa_group}")
    if not parsed_kab or not parsed_kec:
        try:
            conn = db()
            admin_rows = conn.execute("SELECT telegram_id FROM admins").fetchall()
            conn.close()
            admin_ids = [r['telegram_id'] for r in admin_rows] + list(ADMIN_IDS)
            debug = f"WA MASUK TAPI GAGAL PARSE KOTA\nGroup: {wa_group}\nSender: {wa_sender}\nIsi:\n{wa_text[:1000]}"
            for aid in set(admin_ids):
                try:
                    await bot.send_message(aid, debug[:4000])
                except:
                    pass
        except:
            pass
    ...
    if not user_ids:
        print(f"Tidak ada user match {parsed_kab}-{parsed_kec}")
        return 0
    try:
        conn = db()
        bl_rows = conn.execute("SELECT number FROM blacklist_numbers").fetchall()
        conn.close()
        for bl in bl_rows:
            if bl['number'] and bl['number'] in wa_text:
                print(f"Blocked by blacklist {bl['number']}")
                return 0
    except:
        pass
    clean_text = wa_text.strip()
    if "REKBER HANYA" not in clean_text:
        clean_text = f"{clean_text}\n\n{FOOTER_PERINGATAN}"
    final_text = f"📢 <b>INFO JHT MASUK - {parsed_kab} / {parsed_kec}</b>\n🏢 Grup: {html.escape(wa_group or '-')}\n━━━━━━━━━━━━━━━━━━━\n{clean_text}"
    final_text = final_text.replace("https//", "https://")
    count = 0
    for tid in user_ids:
        try:
            conn = db()
            tmpl_row = conn.execute("SELECT template FROM format_settings WHERE telegram_id=?", (tid,)).fetchone()
            conn.close()
            send_text = final_text
            if tmpl_row and tmpl_row['template']:
                try:
                    formatted = apply_template(tmpl_row['template'], clean_text)
                    if formatted and len(formatted) > 20:
                        send_text = f"📢 <b>INFO JHT [{parsed_kab}/{parsed_kec}]</b>\n━━━━━━━━━━━━━━━━━━━\n{formatted}\n\n{FOOTER_PERINGATAN}"
                except:
                    pass
            await bot.send_message(tid, send_text[:4000])
            count += 1
            await asyncio.sleep(0.05)
        except Exception as e:
            print(f"Gagal ke {tid}: {e}")
    print(f"Total forwarded {count} untuk {parsed_kab}/{parsed_kec}")
    return count




# ==================== GREEN API CONFIG - FINAL ====================


# Fonnte tetap ada tapi tidak dipakai kalau PROVIDER green
WA_PROVIDER = os.getenv("WA_PROVIDER", "green").lower()

async def send_green_message(chat_id: str, message: str):
    """Kirim WA via Green API (untuk notif kalau perlu)"""
    if not GREEN_API_ID or not GREEN_API_TOKEN:
        return False
    try:
        url = f"https://api.green-api.com/waInstance{GREEN_API_ID}/sendMessage/{GREEN_API_TOKEN}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                txt = await resp.text()
                print(f"Green send {resp.status}: {txt[:200]}")
                return resp.status == 200
    except Exception as e:
        print(f"Green send error: {e}")
        return False
...
@dp.callback_query(F.data.startswith("af_"))
async def af_selected_fixed(callback: CallbackQuery, state: FSMContext):
    mapping = {
        "af_3m": (20000, "AUTO FORMAT 3 BULAN", "af_3m"),
        "af_6m": (50000, "AUTO FORMAT 6 BULAN", "af_6m"),
        "af_1y": (80000, "AUTO FORMAT 1 TAHUN", "af_1y"),
        "af_unlimited": (200000, "AUTO FORMAT UNLIMITED", "af_unlimited"),
    }
    if callback.data in mapping:
        amount, nama, code = mapping[callback.data]
    else:
        try:
            amount = int(callback.data.replace("af_",""))
            nama = f"AUTO FORMAT {amount}"
            code = f"af_{amount}"
        except:
            amount = 20000
            nama = "AUTO FORMAT"
    ...
    await state.update_data(amount=amount, package_code=code, package_name=nama)
    await callback.message.edit_text(f"💳 <b>{nama}</b> {rupiah(amount)}\n\nSilakan transfer dulu, lalu kirim FOTO bukti TF. Admin akan dapat tombol SETUJU/TOLAK.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ SUDAH BAYAR - KIRIM BUKTI", callback_data="payment_done")]]))
    await callback.answer()

@dp.callback_query(F.data.startswith("cl_"))
async def cl_selected_fixed(callback: CallbackQuery, state: FSMContext):
    try:
        amount = int(callback.data.replace("cl_",""))
    except:
        amount = 15000
    nama = f"CARI LAINNYA {amount}"
    await state.clear()
    await state.set_state(PaymentState.waiting_proof)
    await state.update_data(amount=amount, package_code=f"cari_{amount}", package_name=nama)
    await callback.message.edit_text(f"💳 <b>{nama}</b> {rupiah(amount)}\n\nSilakan transfer dulu, lalu kirim FOTO bukti TF. Admin akan dapat tombol SETUJU/TOLAK.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ SUDAH BAYAR - KIRIM BUKTI", callback_data="payment_done")]]))
    await callback.answer()
...
async def green_webhook(request):
    try:
    ...
    try:
        ...
        chat_name = sender.get("chatName","") or "Unknown Group"
        sender_name = sender.get("senderName","") or sender.get("senderContactName","") or "Unknown"
        sender_id_raw = sender.get("sender","") or sender.get("senderId","") or ""
        phone_raw_digits = ""
        phone_display = "Tidak tersedia"
        if sender_id_raw:
            m = re.search(r'(\d+)', sender_id_raw)
            if m:
                digits = m.group(1)
                phone_raw_digits = digits
                phone_display = "0" + digits[2:] if digits.startswith("62") else digits
        text_msg = ""
        if isinstance(md, dict):
            if "textMessageData" in md:
                text_msg = md["textMessageData"].get("textMessage","")
            if not text_msg and "extendedTextMessageData" in md:
                text_msg = md["extendedTextMessageData"].get("text","")
        text_msg = text_msg or ""
        def is_valid_kota(s):
            if not s: return False
            su = s.upper().strip()
            if any(b in su for b in ["NIK","KPJ","SALDO","LAHIR","TUNGGAL","KERJA","JK"]): return False
            if not re.match(r'^[A-Z ]{3,30}$', su): return False
            if len(su.split())>4: return False
            return True
        def parse_kota_final(text):
            if not text: return None, None
            # === FIX 2026-05: support KOTA JAKARTA single line ===
            raw_upper = text.upper().strip()
            # langsung cek pola KOTA JAKARTA, KOTA: JAKARTA, JAKARTA
            m_single = re.search(r'(?:KOTA|KABUPATEN|KAB)\s*[:\-]?\s*([A-Z ]{3,30})', raw_upper)
            if m_single:
                cand = re.sub(r'[^A-Z ]', ' ', m_single.group(1)).strip()
                cand = re.sub(r'\s+', ' ', cand)
                if is_valid_kota(cand):
                    # return KOTA as kab, kec = KOTA juga biar match LIKE
                    return cand, cand
            # Jika cuma satu kata kota tanpa label, misal "JAKARTA"
            lines = [l.strip() for l in text.split("\n") if l.strip()!=""]
            if len(lines)==1:
                one = re.sub(r'[^A-Z ]', ' ', lines[0].upper()).strip()
                one = re.sub(r'\s+', ' ', one)
                if is_valid_kota(one) and len(one.split())<=2:
                    return one, one
            clean = []
            for l in lines[:5]:
                cl = re.sub(r'[^\w\s,:\-]', ' ', l).strip()
                cl = re.sub(r'\s+', ' ', cl)
            ...
            if len(clean)<2:
                ...
                if "," in first:
                    ...
                    if len(parts)>=2 and is_valid_kota(parts[0]) and is_valid_kota(parts[1]):
                        return parts[0].upper(), parts[1].upper()
                # fallback single still
                if first and is_valid_kota(first.upper()):
                    return first.upper(), first.upper()
                return None, None
            first = re.sub(r'^(KOTA|KABUPATEN|KAB)\s*[:\-]?\s*', '', clean[0], flags=re.I).strip().upper()
            second = re.sub(r'^(KECAMATAN|KEC)\s*[:\-]?\s*', '', clean[1], flags=re.I).strip().upper()
            first = re.sub(r'[^A-Z ]', ' ', first).strip()
            second = re.sub(r'[^A-Z ]', ' ', second).strip()
            first = re.sub(r'\s+', ' ', first)
            second = re.sub(r'\s+', ' ', second)
            if is_valid_kota(first) and is_valid_kota(second):
                return first, second
            # jika second tidak valid tapi first valid (KOTA JAKARTA only)
            if is_valid_kota(first):
                return first, first
            return None, None
        kab, kec = parse_kota_final(text_msg)
        conn = db()
        targets_kota = []
        targets_keyword = []
        matched = {}
        if kab and kec:
            rows = conn.execute("SELECT telegram_id FROM users WHERE upper(kab)=? AND upper(kec)=?", (kab.upper(), kec.upper())).fetchall()
            targets_kota = [r["telegram_id"] for r in rows]
            if not targets_kota:
                rows = conn.execute("SELECT telegram_id FROM users WHERE upper(kab) LIKE ? AND upper(kec) LIKE ?", (f"%{kab.upper()}%", f"%{kec.upper()}%")).fetchall()
                targets_kota = [r["telegram_id"] for r in rows]
        try:
            all_kw = conn.execute("SELECT telegram_id, keyword FROM keywords").fetchall()
            low = text_msg.lower()
            for r in all_kw:
                kw = (r["keyword"] or "").lower().strip()
                if len(kw)>=3 and kw in low:
                    tid = r["telegram_id"]
                    if tid not in targets_keyword:
                        targets_keyword.append(tid)
                    if tid not in matched:
                        matched[tid]=[]
                    matched[tid].append(r["keyword"])
        except Exception as e:
            print(f"kw err {e}")
        # FIX: simpan ke wa_inbox_log untuk fitur CARI LAINNYA
        try:
            conn.execute("INSERT INTO wa_inbox_log(wa_group,wa_sender,message,parsed_kab,parsed_kec,created_at) VALUES(?,?,?,?,?,?)",
                         (chat_name, sender_name, text_msg[:4000], kab or "", kec or "", datetime.now().isoformat()))
            conn.commit()
        except Exception as e:
            print(f"log wa err {e}")
        all_targets = list(set(targets_kota + targets_keyword))
        if not all_targets:
            conn.close()
            return web.json_response({"ok": True, "saved": True, "targets": 0})
        bot = request.app['bot']
        success = 0
        import uuid
        for uid in all_targets:
            try:
                user_kws = matched.get(uid, [])
                if uid in targets_keyword and user_kws:
                    label = f"📍KEYWORD MASUK - {', '.join(user_kws[:2])}"
                else:
                    label = f"📍KOTA MASUK - {kab} / {kec}" if kab and kec else "📍INFO JHT MASUK"
                cache_id = str(uuid.uuid4())[:8]
                salin_cache_mem[cache_id] = text_msg
                try:
                    conn.execute("INSERT OR REPLACE INTO salin_cache (id, content, created_at) VALUES (?,?,?)", (cache_id, text_msg[:4000], datetime.now().isoformat()))
                    conn.commit()
                except:
                    pass
                digits_wa = phone_raw_digits
                if digits_wa.startswith("0"):
                    digits_wa = "62" + digits_wa[1:]
                if digits_wa and not digits_wa.startswith("62") and len(digits_wa)>=10:
                    digits_wa = "62" + digits_wa.lstrip("0")
                forward_new = (
                    f"{label}\n"
                    f"🛡️GRUP: {chat_name}\n"
                    f"🧑‍💻PENGIRIM: {sender_name}\n"
                    f"📱NO HP: {phone_display}\n"
                    f"\n"
                    f"{text_msg}\n"
                    f"\n"
                    f"━━━━━━━━━━━━━━━━\n"
                ...
                kb_btns = []
                kb_btns.append([InlineKeyboardButton(text="📋 SALIN", callback_data=f"salin_{cache_id}")])
                if digits_wa and len(digits_wa)>=10:
                    kb_btns.append([InlineKeyboardButton(text="💬 CHAT PENGIRIM", url=f"https://wa.me/{digits_wa}")])
                else:
                    kb_btns.append([InlineKeyboardButton(text="💬 CHAT PENGIRIM", callback_data="noop")])
                kb_markup = InlineKeyboardMarkup(inline_keyboard=kb_btns)
                await bot.send_message(chat_id=uid, text=forward_new[:4000], reply_markup=kb_markup)
                success+=1
                await asyncio.sleep(0.05)
            except Exception as e:
                print(f"err fwd {uid}: {e}")
        conn.close()
        return web.json_response({"ok": True, "forwarded": success})
    except Exception as e:
...
async def fonnte_webhook(request):
    return await green_webhook(request)
...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🔑 KEYWORD", callback_data="keyword_menu"), InlineKeyboardButton(text="🔎 CARI LAINNYA", callback_data="kota_search_lain")],
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💳 BUKTI TF PENDING", callback_data="admin_bukti_pending")],
...
def get_all_admin_ids():
    try:
        ...
        try:
            ...
            db_ids = set()
        conn.close()
        all_ids = set(ADMIN_IDS) | db_ids
        # filter valid int >0
        all_ids = {int(x) for x in all_ids if str(x).isdigit() and int(x) > 0}
        print(f"DEBUG get_all_admin_ids: ENV={ADMIN_IDS} DB={db_ids} FINAL={all_ids}")
        return list(all_ids)
    except Exception as e:
        print(f"get_all_admin_ids error: {e}")
        import traceback; traceback.print_exc()
        return list(ADMIN_IDS)

async def notify_admins_bukti(bot, file_id, file_type, caption, kb):
    admin_ids = get_all_admin_ids()
    print(f"🔔 NOTIFY ADMIN - Target: {admin_ids} | file_type: {file_type}")
    if not admin_ids:
        print("❌ CRITICAL: Tidak ada ADMIN_IDS di ENV maupun DB! Set ADMIN_IDS di Railway Variables!")
        # fallback: coba kirim ke semua ADMIN_IDS dari ENV yang mungkin string
        try:
            # jangan return, tetap coba log
            pass
        except:
            pass
        # jika tetap kosong, return tapi beri tahu user via log
    sukses = 0
    gagal = 0
    for admin_id in admin_ids:
        try:
            ...
            print(f"✅ Berhasil kirim bukti ke admin {admin_id}")
            sukses += 1
            await asyncio.sleep(0.2)
        except Exception as e:
            print(f"❌ Gagal kirim bukti ke admin {admin_id}: {e}")
            import traceback
            traceback.print_exc()
            gagal += 1
    print(f"📊 Hasil kirim: {sukses} sukses, {gagal} gagal")
    # Jika semua gagal, coba kirim text fallback
    if sukses == 0:
        for admin_id in admin_ids:
            try:
                await bot.send_message(admin_id, f"⚠️ Ada bukti TF baru tapi foto gagal diteruskan.\n\n{caption}\n\nFile ID: {file_id}")
            except:
                pass


async def kirim_bukti_ke_admin(bot, bukti_id, user_id, username, full_name, file_id, file_type, caption_user, amount=0, tx_id=0, package_name=""):
    safe_name = html.escape(full_name or "-")
    safe_uname = html.escape(username or "-")
    safe_cap = html.escape(caption_user or "-")
    pkg = package_name or "-"
    text_admin = (
        f"💳 <b>BUKTI TF BARU #{bukti_id}</b>\n\n"
        f"👤 Nama: {safe_name}\n"
        f"🆔 ID: <code>{user_id}</code>\n"
        f"📛 Username: @{safe_uname}\n"
        f"💰 Nominal: {rupiah(amount) if amount else '-'} \n"
        f"📦 Paket: {html.escape(pkg)}\n"
        f"🧾 Tx: #{tx_id if tx_id else bukti_id}\n"
        f"📝 Caption: {safe_cap}\n"
        f"⏰ {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}\n\n"
        f"📸 Cek foto bukti di atas, lalu tekan SETUJU / TOLAK."
    )
    # Untuk transaksi TOP UP utama, tombol admin langsung memproses transaksi
    # sehingga paket/saldo ikut aktif dan status bukti ikut tersinkron.
    approve_cb = f"approve_{tx_id}" if tx_id else f"bukti_approve_{bukti_id}"
    reject_cb = f"reject_{tx_id}" if tx_id else f"bukti_reject_{bukti_id}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ SETUJU", callback_data=approve_cb),
            InlineKeyboardButton(text="❌ TOLAK", callback_data=reject_cb)
        ],
        [InlineKeyboardButton(text="👁️ LIHAT", callback_data=f"bukti_view_{bukti_id}")]
    ])
    await notify_admins_bukti(bot, file_id, file_type, text_admin, kb)
    
    # === NOTIF WA WAJIB KE ADMIN - BIAR GAK KETINGGALAN ===
    wa_admin_msg = f"""🚨 *BUKTI TF BARU MASUK - BUTUH APPROVE!*

💳 Bukti #{bukti_id} | Tx #{tx_id if tx_id else '-'}
👤 {full_name} (@{username}) ID:{user_id}
💰 {rupiah(amount) if amount else '-'} | 📦 {package_name}
⏰ {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}

⚠️ *WAJIB CEK TELEGRAM SEKARANG!*
Buka bot admin dan tekan SETUJU/TOLAK.

Caption: {caption_user[:200]}"""

    # Kirim WA background biar Telegram satset
    send_wa_to_admins_background(wa_admin_msg)



salin_cache_mem = {}

@dp.callback_query(F.data.startswith("salin_"))
async def salin_handler(callback: CallbackQuery):
    cid = callback.data.replace("salin_","")
    content = salin_cache_mem.get(cid)
    if not content:
        try:
            conn = db()
            row = conn.execute("SELECT content FROM salin_cache WHERE id=?", (cid,)).fetchone()
            conn.close()
    ...
    if not content:
        ...
        return
    await callback.message.answer(f"📋 <b>SALIN PESAN:</b>\n\n<code>{html.escape(content[:3500])}</code>")
...
async def payment_proof(message: Message, state: FSMContext):
    ...
    # Fallback jika amount hilang (state expired) coba ambil dari caption atau default 0
    if amount <= 0:
        try:
            # coba parse dari caption jika ada angka
            cap = message.caption or ""
            import re as re2
            m = re2.search(r"(\d{4,})", cap)
            if m:
                amount = int(m.group(1))
        except:
            pass
    if amount <= 0:
        # tetap izinkan bukti masuk dengan nominal 0, admin bisa lihat
        amount = 0
        await message.answer("⚠️ Nominal tidak terbaca, tapi bukti tetap dikirim ke admin. Admin akan cek manual.")
    # lanjut
    ...
    # Simpan transaksi utama - STATUS PENDING WAJIB APPROVE ADMIN, TIDAK AUTO!
    cur.execute("""
    ...
    """, (
        ...
        data.get("package_name"), photo.file_id, now
    ))
    tx_id = cur.lastrowid

    # Simpan bukti transfer yang terhubung ke transaksi.
    cur.execute("""
        INSERT INTO bukti_tf(
            telegram_id, username, full_name, file_id, file_type,
            caption, amount, status, created_at
        ) VALUES(?,?,?,?,?,?,?,?,?)
    """, (
        ...
        photo.file_id,
        "photo",
        f"{data.get('package_name') or 'TOP UP SALDO'} - {amount}",
        amount,
        "pending",
        now
    ))
    bukti_id = cur.lastrowid
    ...
    await message.answer(
        f"✅ <b>BUKTI TF #{tx_id} BERHASIL DIKIRIM!</b>\n\n"
        f"🧾 Transaksi : #{tx_id}\n"
        f"💰 Nominal : {rupiah(amount)}\n"
        f"📦 Paket : {data.get('package_name') or 'TOP UP SALDO'}\n\n"
        f"⏳ <b>TUNGGU PERSETUJUAN ADMIN</b>\n"
        f"Admin sudah menerima foto bukti kamu dan akan segera mengecek.\n"
        f"Kamu akan dapat notifikasi jika disetujui / ditolak."
    )

    # Kirim foto + tombol SETUJU/TOLAK ke SEMUA admin.
    await kirim_bukti_ke_admin(
        message.bot,
        bukti_id,
        message.from_user.id,
        message.from_user.username or "",
        ...
        photo.file_id,
        "photo",
        f"{data.get('package_name') or 'TOP UP SALDO'} - {rupiah(amount)}",
        amount,
        tx_id,
        data.get("package_name") or "TOP UP SALDO"
    )
...
async def process_payment(callback: CallbackQuery, tx_id: int, approve: bool):
    ...
    if approve:
        ...
        # Sinkronkan record bukti transfer agar menu BUKTI TF PENDING ikut berubah.
        try:
            conn.execute(
                "UPDATE bukti_tf SET status='approved', decided_at=?, decided_by=? "
                "WHERE file_id=? AND status='pending'",
                (now, callback.from_user.id, tx["proof_file_id"])
            )
        except Exception as e:
            print(f"⚠️ Gagal sinkron status bukti TF approved: {e}")
        expiry = None
        ...
        conn.commit()
        conn.close()
        # === FIX: after approve, send detailed notif ===
        exp_txt = ""
        if pkg:
            if expiry:
        ...
        try:
            sisa_q = get_sisa_quota(tx["telegram_id"])
            await callback.bot.send_message(
                tx["telegram_id"],
                f"✅ <b>TOP UP DISETUJUI ADMIN</b>\n\n"
                f"💰 Nominal: {rupiah(tx['amount'])}\n"
                f"📦 Paket: {tx['package_name'] or 'TOP UP SALDO'}\n"
                f"🏙️ Quota +3 KOTA\n"
                f"📊 Sisa quota: {sisa_q} KOTA\n"
                f"{exp_txt}\n\n"
                f"Fitur TAMBAH KOTA, CARI LAINNYA, AUTO FORMAT sudah terbuka!",
                reply_markup=main_menu(tx["telegram_id"])
            )
        except Exception as e:
            print(f"notif user after approve err {e}")
        ...
        try:
            conn.execute(
                "UPDATE bukti_tf SET status='rejected', decided_at=?, decided_by=? "
                "WHERE file_id=? AND status='pending'",
                (now, callback.from_user.id, tx["proof_file_id"])
            )
        except Exception as e:
            print(f"⚠️ Gagal sinkron status bukti TF rejected: {e}")
        conn.commit()
        conn.close()
...
async def auto_format(callback: CallbackQuery, state: FSMContext):
    try:
        # === FIX: cek quota lock juga untuk auto format ===
        if is_fitur_terkunci(callback.from_user.id) and not has_auto_format_access(callback.from_user.id):
            sisa = get_sisa_quota(callback.from_user.id)
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💳 TOP UP SEKARANG", callback_data="topup")],
                [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="back_main")]
            ])
            await callback.message.edit_text(
                f"🔒 <b>AUTO FORMAT TERKUNCI</b>\n\nHarus TOP UP dulu ka. 1x Top Up = 3 KOTA. Sisa: {sisa} KOTA",
                reply_markup=kb
            )
            await callback.answer("🔒 Harus Top Up!", show_alert=True)
            return
        await state.clear()
        nl = chr(10)
        try:
...
async def format_excel_receive(message: Message, state: FSMContext):
    if not has_auto_format_access(message.from_user.id):
        await message.answer("🔒 AUTO FORMAT masih terkunci.")
        return
...
async def format_setting(callback: CallbackQuery, state: FSMContext):
    ...
    conn = db()
    row = conn.execute("SELECT template FROM format_settings WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
    conn.close()
    
    current = row["template"] if row else DEFAULT_TEMPLATE
    preview = apply_template(current, "JAKARTA\nPENJARINGAN\nBEBAS\n14000\nPEREMPUAN 1992\n2021\n22* 23*\n02-03-2025\nSAKTI MULYA")
    
    await callback.message.edit_text(
        "⚙️ <b>SET TEMPLATE FORMAT</b>\n\n"
        "Kirim template baru. Bot akan mengikuti 100% format yang kamu kirim.\n\n"
        "📌 <b>Template saat ini:</b>\n"
        f"<pre>{html.escape(current[:500])}</pre>\n\n"
        "📌 <b>Preview:</b>\n"
        f"<pre>{html.escape(preview[:500])}</pre>\n\n"
        "Kirim template baru (bisa pake placeholder):\n"
        "<code>{KAB}</code> <code>{KEC}</code> <code>{KEL}</code>\n"
        "<code>{SALDO}</code> <code>{KELAMIN}</code> <code>{KPJ}</code>\n"
        "<code>{SENSOR}</code> <code>{IT}</code> <code>{PT}</code>\n\n"
        "Atau kirim contoh format tanpa placeholder.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔢 SET KODE", callback_data="set_kode_format")],
            [InlineKeyboardButton(text="🗑️ HAPUS KODE", callback_data="delete_kode_format")],
...
async def format_results(callback: CallbackQuery):
    ...
    conn = db()
    rows = conn.execute(
        "SELECT id, result_text, created_at FROM format_results WHERE telegram_id=? ORDER BY id ASC LIMIT 20",
...
async def copy_result(callback: CallbackQuery):
    ...
    conn.close()
    if row:
        await callback.message.bot.send_message(callback.from_user.id, f"📋 <b>SALIN:</b>\n<pre>{html.escape(row['result_text'][:4000])}</pre>")
...
async def edit_result_start(callback: CallbackQuery, state: FSMContext):
    rid = int(callback.data.split("_")[-1])
    await state.set_state(FormatState.waiting_edit)
    ...
    if row:
        await callback.message.bot.send_message(
            callback.from_user.id,
            f"✏️ <b>EDIT HASIL ID {rid}</b>\n\nKirim data baru (sama seperti buat format):",
            reply_markup=back_main()
        )
    await callback.answer()


@dp.message(FormatState.waiting_edit, F.text)
async def edit_result_save(message: Message, state: FSMContext):
    data = await state.get_data()
    rid = data.get("edit_result_id")
    if not rid:
        await message.answer("❌ ID tidak ditemukan.")
        await state.clear()
        return
    
    conn = db()
    row = conn.execute("SELECT template FROM format_settings WHERE telegram_id=?", (message.from_user.id,)).fetchone()
    template = row["template"] if row else DEFAULT_TEMPLATE
    kode = generate_next_code(message.from_user.id)
    result = apply_template(template, message.text, kode_header=kode)
    
    conn.execute("UPDATE format_results SET result_text=? WHERE id=? AND telegram_id=?", (result, rid, message.from_user.id))
    conn.commit()
    conn.close()
    await state.clear()
...
async def format_search_receive(message: Message, state: FSMContext):
    ...
    # === FIX KEYWORD ANTI LOKASI ===
    if search_type == "keyword_add":
        kw_input = message.text.strip()
        if kw_input.lower() in ["/cancel", "cancel", "/batal", "batal"]:
            await state.clear()
            await message.answer("❌ Dibatalkan.", reply_markup=back_main())
            return
        if is_keyword_lokasi(kw_input):
            await message.answer(
                "❌ <b>Keyword tidak boleh mengandung lokasi!</b>\n\n"
                "Jangan pakai kata: PROVINSI, KOTA, KECAMATAN, KABUPATEN, JAKARTA, BANDUNG dll\n\n"
                "✅ Contoh BENAR:\n"
                "• info penipu\n"
                "• info klaim\n"
                "• link dana kaget\n\n"
                "Silakan ketik ulang keyword yang valid:"
            )
            return
        # Simpan keyword valid
        try:
            conn = db()
            conn.execute("CREATE TABLE IF NOT EXISTS keywords (id INTEGER PRIMARY KEY AUTOINCREMENT, telegram_id INTEGER, keyword TEXT, created_at TEXT)")
            conn.execute("INSERT INTO keywords(telegram_id,keyword,created_at) VALUES(?,?,?)", (message.from_user.id, kw_input.lower(), datetime.now().isoformat()))
            conn.commit()
            conn.close()
            await state.clear()
            await message.answer(
                f"✅ Keyword <b>{html.escape(kw_input)}</b> berhasil disimpan!\n\n"
                f"Nanti setiap ada pesan WA dari grup yang mengandung kata '{html.escape(kw_input)}' bakal langsung di-forward ke kamu.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Kembali ke KEYWORD", callback_data="keyword_menu")]])
            )
        except Exception as e:
            await message.answer(f"❌ Gagal simpan: {e}")
        return
...
async def hasil_akun(callback: CallbackQuery):
    if not has_auto_format_access(callback.from_user.id):
        ...
        return
    conn = db()
    rows = conn.execute("SELECT id,email,password,created_at, result_id FROM format_accounts WHERE telegram_id=? ORDER BY id ASC LIMIT 20", (callback.from_user.id,)).fetchall()
    conn.close()
    if not rows:
        await callback.message.edit_text(
...
async def edit_akun_save(message: Message, state: FSMContext):
    ...
    if "@" not in email:
        ...
        return
    conn = db()
    conn.execute("UPDATE format_accounts SET email=?, password=?, raw_text=? WHERE id=? AND telegram_id=?", (email, password, f"{email}\n{password}", aid, message.from_user.id))
    conn.commit()
    conn.close()
    await state.clear()
...
async def hapus_akun_all(callback: CallbackQuery):
    conn = db()
    conn.execute("DELETE FROM format_accounts WHERE telegram_id=?", (callback.from_user.id,))
    conn.commit()
    conn.close()
...
async def clear_history(callback: CallbackQuery):
    ...
    conn.commit()
    conn.close()
    await callback.message.edit_text("🗑️ Semua riwayat dihapus.", reply_markup=auto_menu())
    await callback.answer()




@dp.callback_query(F.data == "keyword_menu")
async def keyword_menu(callback: CallbackQuery):
    conn = db()
    try:
        ...
        conn.commit()
        rows = conn.execute("SELECT COUNT(*) as c FROM keywords WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
        count = rows["c"] if rows else 0
    except:
        count = 0
    conn.close()
    text = (
        "📢 <b>MENU KEYWORD - NOTIFIKASI CUSTOM</b>\n\n"
        "Fungsi: Ketika ada pesan WA mengandung kata kunci yang kamu set, kamu akan dapat notifikasi!\n\n"
        "📌 Contoh: aku lagi sebar dana kaget, info penipu\n\n"
        f"📊 Keyword aktif: {count}\n"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📋 KEYWORD SAYA", callback_data="keyword_list")],
        [InlineKeyboardButton(text="➕ TAMBAH KEYWORD", callback_data="keyword_add")],
        [InlineKeyboardButton(text="🗑️ HAPUS KEYWORD", callback_data="keyword_delete")],
        [InlineKeyboardButton(text="⬅️ Kembali", callback_data="back_main")]
    ])
...
@dp.callback_query(F.data == "keyword_add")
async def keyword_add_start(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(FormatState.waiting_search)
    await state.update_data(search_type="keyword_add")
    await callback.message.edit_text(
        "🔑 <b>TAMBAH KEYWORD</b>\n\nKetik kata kunci bebas (BUKAN lokasi)!\n\n✅ Contoh BENAR:\n<code>info penipu</code>\n<code>info klaim</code>\n<code>link dana kaget</code>\n<code>info pencairan</code>\n\n❌ JANGAN pakai: KOTA, KECAMATAN, PROVINSI, JAKARTA dll\n\nKetik /cancel untuk batal.",
        reply_markup=back_main()
    )
    await callback.answer()

@dp.callback_query(F.data == "keyword_delete")
async def keyword_delete_menu(callback: CallbackQuery):
    conn = db()
    rows = conn.execute("SELECT id, keyword FROM keywords WHERE telegram_id=?", (callback.from_user.id,)).fetchall()
    conn.close()
    if not rows:
        await callback.answer("Belum ada keyword", show_alert=True)
        return
    kb_rows = []
    for r in rows:
        kb_rows.append([InlineKeyboardButton(text=f"🗑️ {r['keyword']}", callback_data=f"kw_del_{r['id']}")])
    kb_rows.append([InlineKeyboardButton(text="⬅️ Kembali", callback_data="keyword_menu")])
    await callback.message.edit_text("Pilih keyword yang mau dihapus:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows))
    await callback.answer()

@dp.callback_query(F.data.startswith("kw_del_"))
async def keyword_delete_exec(callback: CallbackQuery):
    kid = callback.data.replace("kw_del_","")
    conn = db()
    conn.execute("DELETE FROM keywords WHERE id=? AND telegram_id=?", (kid, callback.from_user.id))
    conn.commit()
    conn.close()
    await callback.answer("✅ Keyword dihapus")
    # refresh menu
    await keyword_menu(callback)
...
async def kota_add_start(callback: CallbackQuery, state: FSMContext):
    # === FIX: cek quota lock 1 topup = 3 kota ===
    if is_fitur_terkunci(callback.from_user.id):
        sisa = get_sisa_quota(callback.from_user.id)
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💳 TOP UP SEKARANG", callback_data="topup")],
            [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="back_main")]
        ...
        await callback.message.edit_text(
            f"🔒 <b>FITUR TERKUNCI - TAMBAH KOTA</b>\n\n"
            f"Harus TOP UP dulu ka.\n"
            f"1x Top Up = 3 KOTA\n"
            f"📊 Sisa quota kamu: {sisa} KOTA\n\n"
            f"Setelah top up & di-approve admin, fitur kebuka lagi.",
            reply_markup=kb
        )
        await callback.answer("🔒 Harus Top Up dulu!", show_alert=True)
        return
    ...
    if quota > 0:
        await callback.message.edit_text(
            "💎 <b>TAMBAH KOTA - KUOTA MASIH ADA</b>\n\n"
            f"🎟️ Sisa Kuota kamu: <b>{quota}x</b> lagi\n"
            f"💰 Saldo: {rupiah(balance)}\n\n"
            "✅ Kamu sudah Top Up & disetujui Admin. Kuota masih ada, bisa langsung lanjut pilih provinsi!\n\n"
            "📦 <b>Mau tambah kuota lagi? DAFTAR PAKET:</b>\n"
            ...
            f"👇 Sisa kuota kamu {quota}x - Klik tombol hijau untuk lanjut TAMBAH KOTA:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="➡️ LANJUT PILIH PROVINSI", callback_data="kota_add_provinsi")],
...
async def paket_kota_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
            [InlineKeyboardButton(text="⬅️ GANTI PAKET", callback_data="kota_add")],
            [InlineKeyboardButton(text="⬅️ KEMBALI KE MENU UTAMA", callback_data="back_main")]
        ])
    )
...
async def kota_add_provinsi(callback: CallbackQuery, state: FSMContext):
    quota = get_sisa_quota(callback.from_user.id)
    if quota <= 0 and callback.from_user.id not in ADMIN_IDS and not is_admin(callback.from_user.id):
        await callback.answer("❌ Kuota habis, silakan pilih paket dulu", show_alert=True)
...
async def provinsi_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ KEMBALI KE PROVINSI", callback_data="kota_add_provinsi")]])
    )
    regencies = await fetch_wilayah(f"regencies/{prov_id}.json")
    if not regencies:
        await callback.message.edit_text("❌ Gagal memuat.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="kota_add_provinsi")]]))
        return
    buttons = []
    for reg in regencies:
        buttons.append([InlineKeyboardButton(text=reg["name"], callback_data=f"kab_{reg['id']}_{reg['name']}")])
    buttons.append([InlineKeyboardButton(text="⬅️ KEMBALI KE PROVINSI", callback_data="kota_add_provinsi")])
    await callback.message.edit_text(
...
async def kabupaten_selected(callback: CallbackQuery, state: FSMContext):
    ...
    if not districts:
        await callback.message.edit_text("❌ Gagal memuat kecamatan.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="kota_add_provinsi")]]))
        return
    await state.update_data(kab_id=kab_id, kab_name=kab_name, selected_kec=[], districts_cache=districts)
    buttons = []
    for dist in districts[:60]:
        buttons.append([InlineKeyboardButton(text=f"⬜ {dist['name']}", callback_data=f"kec_toggle_{dist['id']}_{dist['name']}")])
...
async def kecamatan_save(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if user_wa and user_wa['wa_number']:
            ...
            send_wa_notif_background(user_wa['wa_number'], kota_full, callback.from_user.full_name)
    except Exception as e:
    ...
    try:
        ...
        if not has_access:
            await callback.message.edit_text(
                ...
                "Pilih paket di bawah untuk lanjut Top Up. Admin harus menyetujui dulu baru bisa digunakan:",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await callback.message.edit_text(
            ...
            "📍 <b>MASUKAN NAMA KOTA</b>" + nl +
            "💡 Contoh: <code>BANDUNG</code>" + nl + nl +
            "✍️ Ketik kota yang mau dicari" + nl +
            "Bot akan cari di history WA yang dishare pengirim!" + nl + nl +
...
@dp.callback_query(F.data.startswith("paket_cari_"))
async def paket_cari_selected(callback: CallbackQuery, state: FSMContext):
    try:
        paket_map = {
            "paket_cari_1w": ("1 Minggu", 15000, "cari_1w"),
            "paket_cari_1m": ("1 Bulan", 50000, "cari_1m"),
            "paket_cari_2m": ("2 Bulan", 80000, "cari_2m"),
            "paket_cari_6m": ("6 Bulan", 250000, "cari_6m"),
        }
        data = paket_map.get(callback.data)
        if not data:
            await callback.answer("Paket tidak ditemukan", show_alert=True)
        ...
        await state.clear()
        await state.set_state(PaymentState.waiting_proof)
        await state.update_data(amount=harga, package_code=kode, package_name=f"CARI DATA LAINNYA {nama_paket}")
        nl = chr(10)
        await callback.message.edit_text(
            f"💳 TOP UP - PAKET {nama_paket.upper()}" + nl + nl +
            f"📦 Paket: CARI DATA LAINNYA {nama_paket}" + nl +
            f"💰 Harga: {rupiah(harga)}" + nl + nl +
            "Silakan transfer ke rekening yang tertera di menu TOP UP, lalu upload bukti pembayaran." + nl + nl +
            "Admin akan menyetujui setelah cek bukti.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📤 UPLOAD BUKTI - TOP UP", callback_data="topup")],
                [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="kota_search_lain")]
            ])
        )
        await callback.answer()
    except Exception as e:
        print(f"paket_cari crash: {e}")
        await callback.answer("Error")


@dp.message(KotaState.waiting_cari_lainnya, F.text)
async def kota_search_lain_receive(message: Message, state: FSMContext):
    query = message.text.strip()
    ...
    # FIX: cari juga di history WA grup
    try:
        wa_rows = conn.execute("SELECT * FROM wa_inbox_log WHERE message LIKE ? OR parsed_kab LIKE ? OR parsed_kec LIKE ? OR wa_group LIKE ? ORDER BY created_at DESC LIMIT 20", (like, like, like, like)).fetchall()
    except:
        wa_rows = []
    conn.close()
    await state.clear()
    total = len(format_rows) + len(history_rows) + len(kota_rows) + len(wa_rows)
    if total == 0:
...
async def blacklist_search_check(message: Message, state: FSMContext):
    ...
    if not data.get("is_blacklist_search"):
        # Ini bukan blacklist search, tapi kota search - skip
        return
...
@dp.message(Command("cekadmin"))
async def cek_admin_debug(message: Message):
    admin_ids_env = list(ADMIN_IDS)
    admin_ids_all = get_all_admin_ids()
    txt = f"🔍 <b>DEBUG ADMIN</b>\n\nENV ADMIN_IDS: {admin_ids_env}\nALL ADMIN (ENV+DB): {admin_ids_all}\n\nID Kamu: <code>{message.from_user.id}</code>\nIs Admin? {is_admin(message.from_user.id)}"
    await message.answer(txt)

@dp.message(Command("testnotif"))
async def test_notif_admin(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        await message.answer("Bukan admin")
        return
    await state.clear()
    await state.set_state(BuktiState.waiting_bukti)
    await message.answer("Kirim foto test, nanti akan diteruskan ke semua admin sebagai simulasi bukti TF")
...
async def set_wa_number_process(message: Message, state: FSMContext):
    ...
    try:
        await send_wa_notif_kota(num, test_kota, message.from_user.full_name)
        await message.answer(
...
# ==================== FITUR BARU: KIRIM BUKTI TF LANGSUNG ====================



@dp.callback_query(F.data == "kirim_bukti_tf")
async def kirim_bukti_tf_start(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(BuktiState.waiting_bukti)
    await callback.message.edit_text(
        "💳 <b>KIRIM BUKTI TRANSFER</b>\n\n"
        "Silakan kirim <b>foto bukti transfer</b> kamu sekarang (JPG/PNG).\n\n"
        "• Pastikan foto jelas, tidak buram\n"
        "• Boleh tambahkan caption nominal\n"
        "• Setelah kirim, admin akan langsung dapat notif dengan tombol SETUJU/TOLAK",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ BATAL", callback_data="back_main")]
        ])
    )
...
@dp.message(BuktiState.waiting_bukti, F.photo | F.document)
async def bukti_tf_received(message: Message, state: FSMContext):
    file_id = message.photo[-1].file_id if message.photo else message.document.file_id
    file_type = "photo" if message.photo else "document"
    caption_user = message.caption or ""
    # coba parse nominal dari caption
    nominal = 0
    try:
        import re
        nums = re.findall(r'\d+', caption_user.replace('.', '').replace(',', ''))
        if nums:
            # ambil angka terbesar sebagai nominal
            nominal = int(max(nums, key=lambda x: int(x)))
            if nominal < 1000:
                nominal = 0
    except:
        nominal = 0
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO bukti_tf(telegram_id, username, full_name, file_id, file_type, caption, amount, status, created_at) VALUES(?,?,?,?,?,?,?,?,?)",
        (message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", file_id, file_type, caption_user, nominal, "pending", datetime.now().isoformat())
    )
    bukti_id = cur.lastrowid
    # juga masukkan ke transactions agar konsisten dengan sistem lama
    cur.execute(
        "INSERT INTO transactions(telegram_id, amount, payment_method, proof_file_id, status, created_at) VALUES(?,?,?,?,?,?)",
        (message.from_user.id, nominal, "SEABANK/DANA", file_id, "pending", datetime.now().isoformat())
    )
    conn.commit()
    conn.close()
    ...
    await state.clear()
    await message.answer(
        f"✅ <b>Bukti TF #{bukti_id} berhasil dikirim!</b>\n\n"
        "Admin sudah menerima foto bukti kamu dan akan segera mengecek.\n"
        "Kamu akan dapat notifikasi jika disetujui / ditolak.",
        reply_markup=main_menu(message.from_user.id)
    )
    await kirim_bukti_ke_admin(message.bot, bukti_id, message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", file_id, file_type, caption_user, nominal)

    # WA admin backup jika kirim_bukti_ke_admin gagal manggil (double safety)
    # sudah dihandle di dalam kirim_bukti_ke_admin

# --- Admin Lihat Pending Bukti TF ---
@dp.callback_query(F.data == "admin_bukti_pending")
async def admin_bukti_pending_list(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Bukan Admin", show_alert=True)
        return
    conn = db()
    rows = conn.execute("SELECT * FROM bukti_tf WHERE status='pending' ORDER BY id DESC LIMIT 15").fetchall()
    conn.close()
    if not rows:
        await callback.message.edit_text("✅ Tidak ada bukti TF pending.", reply_markup=admin_menu())
        await callback.answer()
        return
    text_list = f"💳 <b>BUKTI TF PENDING ({len(rows)})</b>\n\n"
    kb = []
    for r in rows:
        text_list += f"#{r['id']} - {r['full_name']} (@{r['username']}) - {rupiah(r['amount'])} - {r['created_at'][:16]}\n"
        kb.append([
            InlineKeyboardButton(text=f"Lihat #{r['id']}", callback_data=f"bukti_view_{r['id']}"),
            InlineKeyboardButton(text=f"✅ #{r['id']}", callback_data=f"bukti_approve_{r['id']}"),
            InlineKeyboardButton(text=f"❌ #{r['id']}", callback_data=f"bukti_reject_{r['id']}")
        ])
    kb.append([InlineKeyboardButton(text="⬅️ Kembali", callback_data="admin_panel")])
    await callback.message.edit_text(text_list, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()

@dp.callback_query(F.data.startswith("bukti_view_"))
async def admin_bukti_view(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Bukan Admin", show_alert=True)
        return
    try:
        bukti_id = int(callback.data.split("_")[-1])
    except:
        await callback.answer("ID invalid")
        return
    conn = db()
    row = conn.execute("SELECT * FROM bukti_tf WHERE id=?", (bukti_id,)).fetchone()
    conn.close()
    if not row:
        await callback.answer("Tidak ditemukan", show_alert=True)
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ SETUJU", callback_data=f"bukti_approve_{bukti_id}"), InlineKeyboardButton(text="❌ TOLAK", callback_data=f"bukti_reject_{bukti_id}")],
        [InlineKeyboardButton(text="⬅️ Kembali", callback_data="admin_bukti_pending")]
    ])
    try:
        if row["file_type"] == "photo":
            await callback.bot.send_photo(callback.from_user.id, row["file_id"], caption=f"💳 Bukti #{bukti_id} - {row['full_name']} - {rupiah(row['amount'])}", reply_markup=kb)
        else:
            await callback.bot.send_document(callback.from_user.id, row["file_id"], caption=f"💳 Bukti #{bukti_id}", reply_markup=kb)
    except Exception as e:
        print(e)
    await callback.answer()

@dp.callback_query(F.data.startswith("bukti_approve_"))
async def admin_bukti_approve(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Bukan Admin", show_alert=True)
        return
    ...
    if row["status"] != "pending":
        conn.close()
        await callback.answer("Sudah diproses.", show_alert=True)
        return

    # Cari transaksi utama berdasarkan file_id bukti.
    tx = conn.execute(
        "SELECT id FROM transactions WHERE proof_file_id=? ORDER BY id DESC LIMIT 1",
        (row["file_id"],)
    ).fetchone()
    conn.close()

    if tx:
        # Gunakan satu jalur approval agar saldo/paket dan status bukti konsisten.
        await process_payment(callback, tx["id"], True)
        return

    # Fallback untuk bukti lama yang belum memiliki transaksi utama.
    conn = db()
    now = datetime.now().isoformat()
    conn.execute(
        "UPDATE bukti_tf SET status='approved', decided_by=?, decided_at=? WHERE id=?",
        (callback.from_user.id, now, bukti_id)
    )
    ...
    try:
        await callback.bot.send_message(
            row["telegram_id"],
            f"✅ <b>BUKTI TF #{bukti_id} DISETUJUI!</b>\n\n"
            "Pembayaran kamu telah dikonfirmasi admin. Saldo sudah ditambahkan.",
            reply_markup=main_menu(row["telegram_id"])
        )
    except:
        pass
...
@dp.callback_query(F.data.startswith("bukti_reject_"))
async def admin_bukti_reject_start(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Bukan Admin", show_alert=True)
        return
    try:
    ...
    if not row:
        ...
        return
    await state.clear()
    await state.set_state(BuktiState.waiting_reject_reason)
    await state.update_data(reject_bukti_id=bukti_id, reject_user_id=row["telegram_id"])
    await callback.message.answer(f"❌ Kamu akan menolak bukti #{bukti_id}.\n\nKirim alasan penolakan (akan diteruskan ke user).\nKetik /cancel untuk batal.")
    await callback.answer()

@dp.message(BuktiState.waiting_reject_reason, F.text)
async def admin_bukti_reject_reason(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        await state.clear()
        return
    if message.text.strip() == "/cancel":
        await state.clear()
        await message.answer("Dibatalkan.", reply_markup=admin_menu())
        return
    data = await state.get_data()
    bukti_id = data.get("reject_bukti_id")
    user_id = data.get("reject_user_id")
    reason = message.text.strip()
    conn = db()
    conn.execute("UPDATE bukti_tf SET status='rejected', decided_by=?, decided_at=?, reject_reason=? WHERE id=?", (message.from_user.id, datetime.now().isoformat(), reason, bukti_id))
    conn.execute("UPDATE transactions SET status='rejected', processed_at=? WHERE proof_file_id IN (SELECT file_id FROM bukti_tf WHERE id=?)", (datetime.now().isoformat(), bukti_id))
    conn.commit()
    conn.close()
    await state.clear()
    try:
        await message.bot.send_message(user_id, f"❌ <b>BUKTI TF #{bukti_id} DITOLAK</b>\n\nAlasan: {html.escape(reason)}\n\nSilakan kirim ulang bukti yang jelas.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💳 Kirim Ulang", callback_data="kirim_bukti_tf")]]))
    except:
        pass
    await message.answer(f"✅ Bukti #{bukti_id} ditolak: {reason}", reply_markup=admin_menu())
...
async def admin_user_aktif(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        ...
        return
    conn = db()
    # semua user yang pernah ada
    rows = conn.execute("SELECT telegram_id, name, username, created_at, balance FROM users ORDER BY created_at DESC LIMIT 50").fetchall()
    count_total = conn.execute("SELECT COUNT(*) as c FROM users").fetchone()["c"]
    count_active_sub = conn.execute("SELECT COUNT(*) as c FROM subscriptions WHERE status IN ('active','unlimited') AND (expiry_date IS NULL OR expiry_date > ?)", (datetime.now().isoformat(),)).fetchone()["c"]
    conn.close()
    
    txt = f"👥 <b>CEK USER AKTIF</b>\n\n📊 Total User: {count_total}\n✅ Langganan Aktif: {count_active_sub}\n\n<b>50 User Terbaru:</b>\n"
    for i,r in enumerate(rows,1):
        txt += f"{i}. {r['name'] or '-'} (@{r['username'] or '-'}) - ID:{r['telegram_id']} - Saldo:{rupiah(r['balance'] or 0)}\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 STATS LENGKAP", callback_data="admin_stats")],
        [InlineKeyboardButton(text="⬅️ Kembali", callback_data="admin_panel")]
    ])
    await callback.message.edit_text(txt[:4000], reply_markup=kb)
    await callback.answer()
...
async def admin_stats(callback: CallbackQuery):
    ...
    total_pending = conn.execute("SELECT COUNT(*) as c FROM transactions WHERE status='pending'").fetchone()["c"]
    total_approved = conn.execute("SELECT COUNT(*) as c FROM transactions WHERE status='approved'").fetchone()["c"]
    total_bukti_pending = conn.execute("SELECT COUNT(*) as c FROM bukti_tf WHERE status='pending'").fetchone()["c"] if conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='bukti_tf'").fetchone() else 0
    sum_balance = conn.execute("SELECT SUM(balance) as s FROM users").fetchone()["s"] or 0
    total_subs = conn.execute("SELECT COUNT(*) as c FROM subscriptions WHERE status='active' OR status='unlimited'").fetchone()["c"]
    conn.close()
    
    txt = (
        f"📊 <b>STATUS USER / STATS</b>\n\n"
        f"👥 Total User: <b>{total_users}</b>\n"
        f"✅ Subs Aktif: <b>{total_subs}</b>\n"
        f"💰 Total Saldo Semua User: <b>{rupiah(sum_balance)}</b>\n\n"
        f"⏳ Transaksi Pending: <b>{total_pending}</b>\n"
        f"✅ Transaksi Approved: <b>{total_approved}</b>\n"
        f"💳 Bukti TF Pending: <b>{total_bukti_pending}</b>\n\n"
        f"⏰ {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👥 CEK USER AKTIF", callback_data="admin_user_aktif")],
        [InlineKeyboardButton(text="💳 BUKTI TF PENDING", callback_data="admin_bukti_pending")],
        [InlineKeyboardButton(text="⬅️ Kembali", callback_data="admin_panel")]
    ])
...
async def main():
    ...
    try:
        ...
        raise
    logging.info("🤖 JMO BOT V2 - TOP UP WAJIB APPROVE ADMIN + FILTER 2 KUNCI + FOOTER REKBER")
    bot = Bot(token=BOT_TOKEN, default=DefaultBotProperties(parse_mode=ParseMode.HTML))
    app = web.Application()
    app['bot'] = bot
    app.router.add_post('/webhook/green', green_webhook)
    app.router.add_post('/webhook/fonnte', green_webhook)
    app.router.add_post('/webhook', green_webhook)
    app.router.add_post('/whatsapp-webhook', green_webhook)
    app.router.add_post('/whatsapp_webhook', green_webhook)
    app.router.add_post('/', green_webhook)
