...
def _parse_telegram_ids(*values, allow_negative=False):
    result = set()
    for value in values:
        raw = str(value or "")
...
ADMIN_IDS = _parse_telegram_ids(*_raw_admin_values, allow_negative=False)
# Chat tujuan tambahan: bisa user admin (positif) atau grup/supergroup (biasanya negatif).
ADMIN_NOTIFY_IDS = _parse_telegram_ids(
    os.getenv("ADMIN_NOTIFY_IDS", ""),
    os.getenv("ADMIN_CHAT_ID", ""),
    allow_negative=True,
)
print(f"🔧 ADMIN USER IDS: {sorted(ADMIN_IDS)}")
print(f"🔔 ADMIN NOTIFY CHAT IDS: {sorted(ADMIN_NOTIFY_IDS)}")
...
async def cekadmin_command(message: Message):
    if not is_admin(message.from_user.id):
        await message.answer("❌ Bukan Admin")
        return
    ids = get_all_admin_ids()
    notify_ids = get_all_admin_notify_ids()
    await message.answer(
        "🔎 <b>CEK NOTIFIKASI ADMIN</b>\n\n"
        f"👤 Admin user ID: <code>{', '.join(map(str, ids)) or 'KOSONG'}</code>\n"
        f"🔔 Target notifikasi: <code>{', '.join(map(str, notify_ids)) or 'KOSONG'}</code>\n\n"
        "Jika target kosong, isi ADMIN_IDS atau ADMIN_ID di Railway Variables."
    )
...
def get_all_admin_ids():
    """ID user admin yang berhak menekan tombol approve/reject."""
    ids = set(ADMIN_IDS)
    try:
        conn = db()
        try:
            rows = conn.execute("SELECT telegram_id FROM admins").fetchall()
            ids.update(int(r["telegram_id"]) for r in rows if r["telegram_id"] is not None)
        except Exception as e:
            print(f"⚠️ Tidak bisa membaca tabel admins: {e}")
        finally:
            conn.close()
    except Exception as e:
        print(f"⚠️ get_all_admin_ids error: {e}")
    return sorted(x for x in ids if x > 0)


def get_all_admin_notify_ids():
    """Semua chat yang menerima notifikasi bukti pembayaran."""
...
async def notify_admins_bukti(bot, file_id, file_type, caption, kb):
    ...
    print(f"🔔 NOTIF BUKTI TF -> target={targets}")
    if not targets:
        print("❌ TIDAK ADA TARGET ADMIN. Isi ADMIN_IDS/ADMIN_ID atau ADMIN_NOTIFY_IDS/ADMIN_CHAT_ID di Railway.")
        return False
    ...
    for chat_id in targets:
        try:
            if file_type == "photo":
                await bot.send_photo(chat_id=chat_id, photo=file_id, caption=caption, reply_markup=kb)
            else:
                await bot.send_document(chat_id=chat_id, document=file_id, caption=caption, reply_markup=kb)
            success += 1
            print(f"✅ NOTIF BUKTI TF TERKIRIM ke {chat_id}")
            continue
        except Exception as e:
            print(f"❌ MEDIA GAGAL ke {chat_id}: {type(e).__name__}: {e}")

        # Foto gagal? Admin tetap harus menerima notifikasi teks + tombol.
        try:
            await bot.send_message(
                ...
                text=(
                    ...
                    f"{caption}\n\n"
                    "📸 Foto tidak dapat diteruskan otomatis. "
                    "Buka <b>BUKTI TF PENDING</b> untuk melihat bukti."
                ),
                reply_markup=kb,
            )
            success += 1
            print(f"✅ FALLBACK TEKS TERKIRIM ke {chat_id}")
        except Exception as e2:
            print(f"❌ NOTIF TOTAL GAGAL ke {chat_id}: {type(e2).__name__}: {e2}")
...
async def kirim_bukti_ke_admin(bot, bukti_id, user_id, username, full_name, file_id, file_type, caption_user, amount=0, tx_id=0, package_name=""):
    ...
    return await notify_admins_bukti(bot, file_id, file_type, text_admin, kb)
...
async def payment_proof(message: Message, state: FSMContext):
    ...
    # Kirim foto + tombol SETUJU/TOLAK ke SEMUA admin SEBELUM
    # menyatakan ke user bahwa bukti sudah diteruskan.
    admin_notified = await kirim_bukti_ke_admin(
        message.bot,
        bukti_id,
        message.from_user.id,
        message.from_user.username or "",
    ...
    if admin_notified:
        admin_status = "Admin sudah menerima notifikasi bukti dan tombol SETUJU/TOLAK."
    else:
        admin_status = (
            "⚠️ Notifikasi admin belum terkirim. Pastikan ADMIN_IDS/ADMIN_ID "
            "di Railway Variables benar dan admin sudah membuka bot."
        )

    await message.answer(
        "✅📸 <b>Bukti terkirim ke Admin!</b>\n\n"
        "⏳ Menunggu persetujuan (max 1x24 jam)\n"
        "🔔 Nanti ada notifikasi otomatis!"
    )
...
async def bukti_tf_received(message: Message, state: FSMContext):
    ...
    admin_status = (
        "Admin sudah menerima notifikasi bukti dan tombol SETUJU/TOLAK."
        if admin_notified else
        "⚠️ Notifikasi admin belum terkirim. Pastikan ADMIN_IDS/ADMIN_ID di Railway Variables benar."
    )
    await message.answer(
        "✅📸 <b>Bukti terkirim ke Admin!</b>\n\n"
        "⏳ Menunggu persetujuan (max 1x24 jam)\n"
