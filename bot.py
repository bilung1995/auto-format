...
def db():
    ...
    # === PERMANEN FIX: Pastikan pakai Railway Volume /data kalau ada ===
    # Railway Volume harus di-mount di /data di dashboard Railway
    try:
        DB_PATH.parent.mkdir(parents=True, exist_ok=True)
        # test write
    ...
    # Restore PASADATA hanya sekali dan hanya jika URL memang diset - untuk permanen
    if not _pasadata_synced and PASADATA_URL_BARU:
        try:
            if not DB_PATH.exists() or DB_PATH.stat().st_size < 100:
                print(f"🔄 DB kecil/hilang, restore dari PASADATA BARU...")
                sync_from_pasadata_baru()
            else:
                print(f"✅ DB sudah ada: {DB_PATH} ({DB_PATH.stat().st_size} bytes) - tidak perlu restore")
        except Exception as e:
    ...
    try:
        ...
        # WAL mode biar tahan crash & lebih permanen
        try:
...
def has_auto_format_access(user_id):
    # Admin selalu boleh akses AUTO FORMAT
    if user_id in ADMIN_IDS or is_admin(user_id):
        return True
    sub = get_subscription(user_id)
    if not sub:
        return False
    # Cek paket AUTO FORMAT: 3M, 6M, 1Y, UNLIMITED atau status unlimited
...
async def auto_format(callback: CallbackQuery, state: FSMContext):
    ...
    if has_auto_format_access(callback.from_user.id):
        ...
        await callback.message.edit_text(
            "🔒 <b>AUTO FORMAT TERKUNCI</b>\n\n"
            "Untuk membuka AUTO FORMAT, silakan pilih paket dulu:\n\n"
            "🟡 3 Bulan — <b>Rp20.000</b>\n"
...
async def process_payment(callback: CallbackQuery, tx_id: int, approve: bool):
    ...
    if approve:
        ...
        expiry = None
        # --- FIX: KHUSUS PAKET KOTA = TAMBAH KUOTA 3x ---
        if tx["package_code"] and str(tx["package_code"]).startswith("kota_"):
            # mapping paket kota ke durasi expiry (untuk info)
            pkg_code = tx["package_code"]
            if pkg_code == "kota_1w":
            ...
            sub_status = "active" if pkg_code != "kota_unlimited" else "unlimited"
            ...
            # TAMBAH KUOTA 3x - INI KUNCINYA
            quota_to_add = 9999 if pkg_code == "kota_unlimited" else 3
            conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+?", (tx["telegram_id"], quota_to_add, quota_to_add))

        elif tx["package_code"]:
            if tx["package_code"] == "3M":
                expiry, sub_status = add_months(datetime.now(), 3), "active"
            elif tx["package_code"] == "6M":
        ...
        if tx["package_code"]:
            if expiry:
                expiry_text = f"\n📅 Berakhir : {expiry.strftime('%d-%m-%Y')}"
            else:
                # hanya unlimited yang SELAMANYA
                if tx["package_code"] == "kota_unlimited":
                    expiry_text = "\n📅 Berakhir : SELAMANYA ♾️"
                else:
                    expiry_text = f"\n📅 Berakhir : {expiry.strftime('%d-%m-%Y') if expiry else '-'}"
...
async def status(callback: CallbackQuery):
    ...
    user = conn.execute("SELECT * FROM users WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
    sub = conn.execute("SELECT * FROM subscriptions WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
    # ambil kota untuk info tambahan
    kota_rows = conn.execute("SELECT COUNT(*) as cnt FROM user_kota WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
    quota_row = conn.execute("SELECT * FROM kota_quota WHERE telegram_id=?", (callback.from_user.id,)).fetchone()
    conn.close()

    balance = user["balance"] if user else 0
    total_kota = kota_rows["cnt"] if kota_rows else 0
    quota = quota_row["quota"] if quota_row else 0
    
    if not sub:
        text = f"📊 <b>STATUS USER</b>\n\n💰 Saldo : <b>{rupiah(balance)}</b>\n📦 Langganan : Belum ada\n📊 Status : 🔴 Tidak aktif\n🎟️ Kuota Kota: 0x\n📍 Kota Dipilih: {total_kota}"
    else:
        paket_name = sub["package_name"] or "-"
        kode = (sub["package_code"] or "").lower()
        is_unlimited = kode == "kota_unlimited" or sub["status"] == "unlimited"
        
        if is_unlimited:
            text = (
                f"📊 <b>STATUS USER</b>\n\n"
                f"💰 Saldo : <b>{rupiah(balance)}</b>\n"
                f"📦 Langganan : <b>{paket_name}</b>\n"
                f"📅 Berakhir : <b>SELAMANYA ♾️</b>\n"
                f"📊 Status : 🟢 AKTIF ✅\n"
                f"🎟️ Sisa Kuota: <b>{quota if quota<9000 else 'Unlimited'} kali</b>\n"
                f"📍 Total Kota Dipilih: <b>{total_kota} kota</b>"
            )
        else:
            try:
                exp = datetime.fromisoformat(sub["expiry_date"]) if sub["expiry_date"] else None
            except Exception:
                exp = None
            ...
            if exp:
                ...
                sisa_text = f"{sisa} hari lagi" if sisa >=0 else "habis"
                if sisa == 0:
                    sisa_text = "habis hari ini"
                
                text = (
                    f"📊 <b>STATUS USER</b>\n\n"
                    f"💰 Saldo : <b>{rupiah(balance)}</b>\n"
                    f"📦 Langganan : <b>{paket_name}</b>\n"
                    f"📅 Berakhir : <b>{exp.strftime('%d-%m-%Y')}</b> ({sisa_text})\n"
                    f"📊 Status : {'🟢 AKTIF' if active else '🔴 EXPIRED'}\n"
                    f"🎟️ Sisa Kuota: <b>{quota} kali</b>\n"
                    f"📍 Total Kota Dipilih: <b>{total_kota} kota</b>\n\n"
                    f"📍 Kota sesuai yang kamu pilih di menu KOTA SAYA"
                )
            else:
                text = (
                    f"📊 <b>STATUS USER</b>\n\n"
...
async def format_manual_receive(message: Message, state: FSMContext):
    ...
    # Tombol sesuai request: SALIN, EDIT, SIMPAN, HAPUS
    await message.answer(
        f"<b>✅ FORMAT OTOMATIS JADI:</b>\n\n<pre>{html.escape(result[:3800])}</pre>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
...
def parse_kode_input(text: str):
    """Parse kode seperti 'JPG - 001', 'As 001', 'ABC-001', '🤖 JPG - 001 🤖'"""
    import re
    if not text:
        return None
    t = text.strip()
    # hapus emoji untuk parsing tapi simpan suffix/prefix emoji kalau ada
    # pola: [emoji] PREFIX - NUMBER [emoji]  atau PREFIX NUMBER
    # contoh: As 001 -> prefix=As, num=1, padding=3
    # JPG - 001 -> prefix=JPG, num=1, padding=3
    
    # Bersihkan karakter aneh di awal/akhir tapi simpan huruf
    # Coba regex fleksibel
    # Grup 1: prefix (huruf, angka, spasi, emoji), Grup 2: angka, Grup 3: suffix opsional
    patterns = [
        r'^\s*(.+?)\s*[-–—]\s*(\d+)\s*(.*?)\s*$',  # JPG - 001 atau As - 001 + suffix
        r'^\s*([A-Za-z]+)\s+(\d+)\s*(.*?)\s*$',     # As 001
        r'^\s*([A-Za-z]+)(\d+)\s*$',                 # As001
        r'^\s*(.+?)\s+(\d+)\s*$',                   # fallback prefix + number
    ]
    for pat in patterns:
        m = re.match(pat, t)
        if m:
            ...
            suffix = m.group(3).strip() if len(m.groups()) >=3 else ""
            # prefix harus mengandung huruf
            if not re.search(r'[A-Za-z]', prefix):
                continue
            # hapus emoji berlebihan dari prefix untuk penyimpanan bersih tapi biarkan
            # prefix uppercase untuk konsistensi
            try:
                num = int(num_str)
            except:
                continue
            ...
            if padding > 5:
                padding = len(num_str)
            return (prefix.upper(), num, padding, suffix)
    return None

def generate_next_code(telegram_id: int):
    """Ambil kode berikutnya dari DB, auto increment"""
    try:
        conn = db()
        row = conn.execute("SELECT * FROM format_codes WHERE telegram_id=? AND enabled=1", (telegram_id,)).fetchone()
        if not row:
            conn.close()
            return None
        prefix = row["prefix"]
        ...
        # increment untuk next
        conn.execute("UPDATE format_codes SET current_number=current_number+1, updated_at=? WHERE telegram_id=?", (datetime.now().isoformat(), telegram_id))
        conn.commit()
        conn.close()
        # kembalikan dengan underline garis bawah sesuai request: tengah + garis
        centered = code_str.center(27)
        # Garis bawah setelah kode - sesuai request "kasih garis bawah setelah set kode sudah di ketik"
        underline = "━━━━━━━━━━━━━━━━━━━"
        return f"{centered}\n{underline}"
    except Exception as e:
        try:
...
def set_kode_format(callback: CallbackQuery, state: FSMContext):
    if not has_auto_format_access(callback.from_user.id):
...
async def receive_kode_format(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"<pre>{centered_demo}\n{garis}\n[FORMAT KAMU]</pre>\n\n"
        f"Garis bawah sudah ditambahkan di bawah kode.\n"
...
Menampilkan {len(rows)} akun di bawah, tiap format sudah ada kode + garis bawah:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
        ])
    )
    ...
    for i, row in enumerate(rows, 1):
        ...
        # Pastikan kode + garis bawah ada
        if "━━━━━━━━" not in fmt_text:
            # coba ambil kode terbaru tanpa increment (hanya preview)
            kode_preview = ""
            try:
                conn3 = db()
                code_row = conn3.execute("SELECT prefix, suffix, current_number, padding FROM format_codes WHERE telegram_id=? AND enabled=1", (callback.from_user.id,)).fetchone()
                conn3.close()
                if code_row:
                    code_str = f"{code_row['prefix']} {str(code_row['current_number']-1).zfill(code_row['padding'])} {code_row['suffix'] or ''}".strip()
                    kode_preview = f"{code_str.center(27)}
━━━━━━━━━━━━━━━━━━━
"
            except:
                pass
            if kode_preview:
                fmt_text = f"{kode_preview}{fmt_text}"
            else:
                fmt_text = f"{fmt_text}
━━━━━━━━━━━━━━━━━━━"
        
        # Gabung format + akun
        full_display = f"{fmt_text.strip()}

📧 AKUN:
{email}
{pwd}"
        if len(full_display) > 3800:
            full_display = full_display[:3800] + "..."
        ...
        try:
            await callback.message.bot.send_message(
                callback.from_user.id,
                f"<b>{i}. ID {rid}</b>
<pre>{html.escape(full_display)}</pre>",
                reply_markup=kb
            )
            await asyncio.sleep(0.2)
        except Exception as e:
            logging.warning(f"Gagal kirim akun {rid}: {e}")
...
async def kecamatan_save(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id not in ADMIN_IDS and not is_admin(callback.from_user.id):
        # 1x save = 1 kuota (walau pilih banyak kecamatan), sesuai request 1 top up = 3x tambah kota
...
async def kota_list(callback: CallbackQuery):
    ...
    # AUTO-FIX: kalau user sudah punya paket kota tapi kuota masih 0 (karena transaksi lama), kasih 3
    if sub and quota == 0 and (sub["package_code"] or "").startswith("kota_") and sub["package_code"] != "kota_unlimited":
        try:
            conn2 = db()
            conn2.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+3", (callback.from_user.id, 3))
            conn2.commit()
            conn2.close()
            quota = 3
        except Exception:
            pass
    if sub and sub["package_code"] == "kota_unlimited":
        quota = 9999
    balance = user["balance"] if user else 0
    ...
    if sub:
        ...
        # ADMIN unlimited atau paket kota_unlimited = SELAMANYA
        is_kota_unlimited = kode == "kota_unlimited"
        is_admin_user = callback.from_user.id in ADMIN_IDS or is_admin(callback.from_user.id)
        
        if sub["status"] == "unlimited" or is_kota_unlimited or (is_admin_user and not exp_dt):
            aktif_str = start_dt.strftime("%d-%m-%Y") if start_dt else datetime.now().strftime("%d-%m-%Y")
            if is_kota_unlimited or sub["status"] == "unlimited" or is_admin_user:
                expired_str = "SELAMANYA ♾️"
                status_langganan = "🟢 UNLIMITED AKTIF"
            else:
                expired_str = exp_dt.strftime("%d-%m-%Y") if exp_dt else "SELAMANYA ♾️"
                status_langganan = "🟢 UNLIMITED AKTIF"
        elif exp_dt:
            ...
            if sisa_hari < 0:
                ...
                if sisa_hari == 0:
                    status_langganan = f"🟢 AKTIF (habis hari ini)"
                else:
                    status_langganan = f"🟢 AKTIF ({sisa_hari} hari lagi)"
        else:
            aktif_str = "-"
            expired_str = "-"
...
async def admin_user_aktif(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        ...
        return
    conn = db()
    # Ambil 50 user terbaru dengan info lengkap: kota, kuota, format count
    rows = conn.execute("""
        SELECT u.telegram_id, u.name, u.username, u.balance,
               s.package_code, s.package_name, s.expiry_date, s.status,
               (SELECT COUNT(*) FROM user_kota WHERE telegram_id=u.telegram_id) as total_kota,
               (SELECT quota FROM kota_quota WHERE telegram_id=u.telegram_id) as sisa_kuota,
               (SELECT COUNT(*) FROM format_results WHERE telegram_id=u.telegram_id) as total_format
        FROM users u
        LEFT JOIN subscriptions s ON s.telegram_id=u.telegram_id
        ORDER BY u.telegram_id DESC LIMIT 50
    """).fetchall()
    conn.close()

    if not rows:
        await callback.message.edit_text("👥 <b>CEK USER AKTIF - KOSONG</b>\n\nBelum ada user.", reply_markup=admin_menu())
        await callback.answer()
        return

    # Kirim summary dulu
    text_summary = f"👥 <b>CEK USER AKTIF - {len(rows)} USER TERBARU</b>\n\n"
    for i, r in enumerate(rows, 1):
        # status
        if r['status'] == 'unlimited' or (r['package_code'] or '') == 'kota_unlimited':
            status_icon = "🟢 UNLIMITED"
            exp_str = "SELAMANYA"
        elif r['expiry_date']:
            try:
                exp_dt = datetime.fromisoformat(r['expiry_date'])
                is_active = datetime.now() < exp_dt
                sisa = (exp_dt - datetime.now()).days
                status_icon = f"{'🟢 AKTIF' if is_active else '🔴 EXPIRED'} ({sisa}hri)"
                exp_str = exp_dt.strftime("%d-%m-%Y")
            except:
                status_icon = r['status'] or '-'
                exp_str = (r['expiry_date'][:10] if r['expiry_date'] else '-')
        else:
            status_icon = "🔴 BELUM LANGGANAN"
            exp_str = "-"
        
        paket = r['package_name'] or '-'
        username = f"@{r['username']}" if r['username'] else '-'
        total_kota = r['total_kota'] or 0
        sisa_kuota = r['sisa_kuota'] if r['sisa_kuota'] is not None else 0
        total_format = r['total_format'] or 0
        
        text_summary += f"{i}. <b>{html.escape(r['name'] or 'Tanpa Nama')}</b>\n"
        text_summary += f"   🆔 <code>{r['telegram_id']}</code> | {username}\n"
        text_summary += f"   📦 {paket} | {status_icon}\n"
        text_summary += f"   📅 Exp: {exp_str} | 💰 {rupiah(r['balance'] or 0)}\n"
        text_summary += f"   🏙️ Kota: {total_kota} | 🎟️ Kuota: {sisa_kuota}x | 📝 Format: {total_format}\n\n"
    
    # Telegram limit 4096, split if needed
    if len(text_summary) > 3800:
        # kirim 2 pesan
        part1 = text_summary[:3800]
        part2 = text_summary[3800:]
        await callback.message.edit_text(part1, reply_markup=admin_menu())
        # kirim sisanya sebagai pesan baru
        try:
            await callback.message.bot.send_message(callback.from_user.id, part2[:3800])
        except:
            pass
    else:
        await callback.message.edit_text(text_summary, reply_markup=admin_menu())
    await callback.answer()
...
async def admin_stats(callback: CallbackQuery):
    ...
    active_subs = conn.execute("SELECT COUNT(*) as cnt FROM subscriptions WHERE status='active' AND expiry_date>? OR status='unlimited'", (datetime.now().isoformat(),)).fetchone()["cnt"]
    pending_tx = conn.execute("SELECT COUNT(*) as cnt FROM transactions WHERE status='pending'").fetchone()["cnt"]
    total_kota = conn.execute("SELECT COUNT(*) as cnt FROM user_kota").fetchone()["cnt"]
    total_format = conn.execute("SELECT COUNT(*) as cnt FROM format_results").fetchone()["cnt"]
    total_trans = conn.execute("SELECT COUNT(*) as cnt FROM transactions WHERE status='approved'").fetchone()["cnt"]
    total_saldo = conn.execute("SELECT SUM(balance) as tot FROM users").fetchone()["tot"] or 0
    
    # Paket breakdown
    paket_rows = conn.execute("SELECT package_name, COUNT(*) as cnt FROM subscriptions GROUP BY package_name").fetchall()
    conn.close()

    text = f"""📊 <b>STATS USER - DASHBOARD ADMIN</b>

👥 Total User: <b>{total_users}</b>
🟢 Langganan Aktif: <b>{active_subs}</b>
⏳ Transaksi Pending: <b>{pending_tx}</b>
✅ Transaksi Approved: <b>{total_trans}</b>
💰 Total Saldo User: <b>{rupiah(total_saldo)}</b>

🏙️ Total Kota Dipilih: <b>{total_kota}</b>
📝 Total Format Dibuat: <b>{total_format}</b>

📦 <b>Breakdown Paket:</b>
"""
    for pr in paket_rows:
        text += f"• {pr['package_name'] or 'Tanpa Paket'}: {pr['cnt']} user\n"
    
    text += f"\n📅 Data diambil: {datetime.now().strftime('%d-%m-%Y %H:%M')}"
