...
def parse_kota_kec_from_text(text: str):
    ...
    kab = None
    kec = None
    # Pola 1: format lama 4 digit kode pos di baris sendiri, baris selanjutnya KAB, selanjutnya KEC
        ...
        if re.match(r'^\d{4}$', line):
            ...
            if i+2 < len(lines):
                ...
                if maybe_kec and ':' not in maybe_kec and 'RP' not in maybe_kec and len(maybe_kec) < 40:
                    kec = maybe_kec
            break
    # Pola 2: cari dengan regex KAB : ... KEC : ...
    ...
    # Pola 3: cek apakah ada KAB dan KEC di DB yang muncul di text (paling robust untuk grup)
    if not kab or not kec:
        low = text.lower()
        try:
            conn = db()
    ...
    # Jika masih belum ketemu, coba ambil 2 kata pertama yang mirip kota
    if kab and kec:
