...
async def paket_cari_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"💳 <b>TOP UP - PAKET {nama_paket.upper()}</b>\n\n"
        f"📦 Paket: <b>CARI DATA LAINNYA {nama_paket}</b>\n"
        f"💰 Harga: <b>{rupiah(harga)}</b>\n\n"
        "Silakan transfer ke rekening yang tertera di menu TOP UP, lalu upload bukti pembayaran.\n\n"
        "Admin akan menyetujui setelah cek bukti.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📤 UPLOAD BUKTI", callback_data="topup")],
...
async def kota_search_lain_start(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    nl = chr(10)
    if not has_cari_lainnya_access(callback.from_user.id):
        await callback.message.edit_text(
