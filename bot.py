...
# Support koma, spasi, enter, dan ADMIN_ID tunggal.
# Jangan membatasi panjang ID Telegram agar konfigurasi tetap fleksibel.
_raw_admin_extra = " ".join([
    os.getenv("ADMIN_ID", ""),
    os.getenv("ADMIN_TELEGRAM_ID", ""),
])
ADMIN_IDS = {
    int(x) for x in re.findall(r"\d+", f"{_raw_admin} {_raw_admin_extra}")
    if int(x) > 0
}
print(f"🔧 ADMIN_IDS RAW: '{_raw_admin} {_raw_admin_extra}' -> PARSED: {ADMIN_IDS}")
...
async def notify_admins_bukti(bot, file_id, file_type, caption, kb):
    ...
    if not admin_ids:
        print("❌ CRITICAL: Tidak ada ADMIN_IDS di ENV maupun DB.")
        return False
    ...
    for admin_id in admin_ids:
        ...
        # Jalur utama: kirim foto/dokumen beserta tombol approve/tolak.
        try:
            if file_type == "photo":
            ...
            delivered = True
            sukses += 1
            print(f"✅ Berhasil kirim bukti + tombol ke admin {admin_id}")
        except Exception as e:
            print(f"❌ Gagal kirim media ke admin {admin_id}: {e}")

        # Fallback PER ADMIN: walaupun foto gagal, admin tetap mendapat
        # notifikasi teks + tombol approve/tolak.
        if not delivered:
            try:
                await bot.send_message(
                    ...
                    "⚠️ <b>BUKTI TF BARU</b>\n\n"
                    f"{caption}\n\n"
                    "📸 Foto bukti gagal diteruskan otomatis. "
                    "Gunakan menu <b>PANEL ADMIN → BUKTI TF PENDING</b> "
                    "untuk melihat bukti dan memprosesnya.",
                    reply_markup=kb
                )
                delivered = True
                sukses += 1
                print(f"✅ Fallback notifikasi teks berhasil ke admin {admin_id}")
            except Exception as e2:
                print(f"❌ Fallback notifikasi ke admin {admin_id} juga gagal: {e2}")
    ...
    print(f"📊 Hasil kirim notifikasi admin: {sukses} berhasil, {gagal} gagal")
    return sukses > 0
...
async def payment_proof(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ <b>BUKTI TF #{tx_id} BERHASIL DISIMPAN!</b>\n\n"
        f"🧾 Transaksi : #{tx_id}\n"
...
async def bukti_tf_received(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ <b>Bukti TF #{bukti_id} berhasil disimpan!</b>\n\n"
        f"{admin_status}\n"
