...
async def forward_wa_to_telegram(bot, wa_text: str, wa_group: str, wa_sender: str, parsed_kab: str, parsed_kec: str):
    ...
    if not parsed_kab or not parsed_kec:
        print(f"⚠️ Parse gagal, text: {wa_text[:200]}")
        try:
            conn = db()
            ...
            if admin_ids:
                debug_text = f"⚠️ WA MASUK TAPI PARSE GAGAL{nl}{nl}Group: {wa_group}{nl}Sender: {wa_sender}{nl}Kab: {parsed_kab} Kec: {parsed_kec}{nl}{nl}Isi:{nl}{wa_text[:1000]}"
                for aid in set(admin_ids):
                    try:
                        await bot.send_message(aid, debug_text[:4000])
                    except:
                        pass
        except:
            pass
    ...
    if not user_ids:
        print(f"⏭️ No match {parsed_kab}+{parsed_kec} - cek user_kota table")
        try:
            conn = db()
            admin_rows = conn.execute("SELECT telegram_id FROM admins").fetchall()
            conn.close()
            admin_ids = [r['telegram_id'] for r in admin_rows] + list(ADMIN_IDS)
            debug_text = f"📩 WA MASUK - TIDAK ADA USER YANG MATCH{nl}{nl}KAB: {parsed_kab}{nl}KEC: {parsed_kec}{nl}Group: {wa_group}{nl}{nl}Isi:{nl}{wa_text[:1000]}"
            for aid in set(admin_ids):
                try:
                    await bot.send_message(aid, debug_text[:4000])
                except:
    ...
    if "PERHATIAN" not in clean_text:
        clean_text = f"{clean_text}{nl}{nl}{FOOTER_PERINGATAN}"
    final_text = f"📢 INFO MASUK KEYWORD {nl}{nl}{clean_text}"
    ...
    for tid in user_ids:
        try:
            ...
            count += 1
            print(f"✅ Forwarded ke {tid} - {parsed_kab} {parsed_kec}")
        except Exception as e:
...
async def fonnte_webhook(request):
    ...
    wa_text = data.get('message') or data.get('text') or data.get('msg') or data.get('message_text') or ''
    wa_group = data.get('group') or data.get('group_name') or data.get('name') or data.get('group_id') or 'ZOLDYCK STORE'
    wa_sender = data.get('sender') or data.get('from') or data.get('pushname') or data.get('author') or data.get('sender_name') or ''
    if not wa_text and isinstance(data.get('data'), dict):
        wa_text = data['data'].get('message','') or data['data'].get('text','')
    if not wa_text:
        wa_text = str(data)[:3000]
    print(f"📩 Webhook WA masuk: group={wa_group} sender={wa_sender} text={wa_text[:200]}")
    kab, kec = parse_kota_kec_from_text(wa_text)
    print(f"🔍 Parsed: KAB={kab} KEC={kec}")
    try:
        conn = db()
    ...
    return web.json_response({"status": True, "forwarded": forwarded, "kab": kab, "kec": kec, "parsed_kab": kab, "parsed_kec": kec, "admin_notif": True, "footer_added": True})
