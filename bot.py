...
async def green_webhook(request):
    ...
    try:
        ...
        def parse_kota_final(text):
            ...
            # === FIX 2026: WAJIB 2 KUNCI KOTA + KECAMATAN untuk TAMBAH KOTA ===
            # Contoh valid:
            # SERANG
            # LEBAK WANGI  -> MASUK
            # SERANG doang -> TIDAK MASUK
            # LEBAK WANGI doang -> TIDAK MASUK
            ...
            # Harus minimal 2 baris
            if len(clean)<2:
                # cek koma: "SERANG, LEBAK WANGI"
                ...
                return None, None
            # Ambil 2 baris pertama yang valid
            ...
            # WAJIB kedua-duanya valid kota/kecamatan
            if is_valid_kota(first) and is_valid_kota(second):
                return first, second
            # Jika baris 1 gagal, coba baris 2 & 3 (skip header)
            if len(clean)>=3:
                alt1 = re.sub(r'^(KOTA|KABUPATEN|KAB)\s*[:\-]?\s*', '', clean[1], flags=re.I).strip().upper()
                alt2 = re.sub(r'^(KECAMATAN|KEC)\s*[:\-]?\s*', '', clean[2], flags=re.I).strip().upper()
                alt1 = re.sub(r'[^A-Z ]', ' ', alt1).strip()
                alt2 = re.sub(r'[^A-Z ]', ' ', alt2).strip()
                alt1 = re.sub(r'\s+', ' ', alt1)
                alt2 = re.sub(r'\s+', ' ', alt2)
                if is_valid_kota(alt1) and is_valid_kota(alt2):
                    return alt1, alt2
            return None, None
