...
def main_menu(user_id=None):
    ...
    # FULL 2 KOTAK - SEMUA MUNCUL TERMASUK PANEL ADMIN
    keyboard = [
        [KeyboardButton(text="👤 PROFIL USER"), KeyboardButton(text="💳 TOP UP")],
        [KeyboardButton(text="📊 CEK STATUS"), KeyboardButton(text="💡 SOLUSI JMO")],
        [KeyboardButton(text="📝 AUTO FORMAT"), KeyboardButton(text="📞 HUBUNGI ADMIN")],
        [KeyboardButton(text="🔐 PANEL ADMIN"), KeyboardButton(text="ℹ️ INFO BOT")],
    ]
...
def main_menu_inline(user_id=None):
    ...
    rows = [
        [InlineKeyboardButton(text="👤 PROFIL USER", callback_data="profile"), InlineKeyboardButton(text="💳 TOP UP", callback_data="topup")],
        [InlineKeyboardButton(text="📊 CEK STATUS", callback_data="status"), InlineKeyboardButton(text="💡 SOLUSI JMO", callback_data="solusi_jmo")],
        [InlineKeyboardButton(text="📝 AUTO FORMAT", callback_data="auto_format"), InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
        [InlineKeyboardButton(text="🔐 PANEL ADMIN", callback_data="admin_panel"), InlineKeyboardButton(text="ℹ️ INFO BOT", callback_data="info_bot")],
    ]
...
def auto_menu():
    # 2 kolom, Top Up hijau, lainnya biru muda kayak foto
    return InlineKeyboardMarkup(inline_keyboard=[
        [
...
# === HANDLER UNTUK KOTAK FULL LAYAR - HARUS DI ATAS ===
@dp.message(F.text == "👤 PROFIL USER")
async def handle_profil_user(message: Message):
    await message.answer("👤 **PROFIL USER**\nID: {}\nNama: {}\nSaldo: cek /saldo".format(message.from_user.id, message.from_user.full_name), parse_mode="Markdown", reply_markup=main_menu_inline())

@dp.message(F.text == "💳 TOP UP")
async def handle_topup(message: Message):
    await message.answer("💳 **TOP UP**\nSilakan pilih nominal atau hubungi admin", reply_markup=main_menu_inline())

@dp.message(F.text == "📊 CEK STATUS")
async def handle_cek_status(message: Message):
    await message.answer("📊 **CEK STATUS**\nKirim ID transaksi atau /cekstatus [ID]")

@dp.message(F.text == "💡 SOLUSI JMO")
async def handle_solusi_jmo(message: Message):
    await message.answer("💡 **SOLUSI JMO**\nPilih masalah JMO kamu", reply_markup=main_menu_inline())

@dp.message(F.text == "📝 AUTO FORMAT")
async def handle_auto_format(message: Message):
    await message.answer("📝 **AUTO FORMAT**\nKirim data format otomatis")

@dp.message(F.text == "📞 HUBUNGI ADMIN")
async def handle_hubungi_admin(message: Message):
    await message.answer("📞 **HUBUNGI ADMIN**\nHubungi @admin untuk bantuan")

@dp.message(F.text == "🔐 PANEL ADMIN")
async def handle_panel_admin(message: Message):
    if is_admin(message.from_user.id):
        await message.answer("🔐 **PANEL ADMIN**", reply_markup=main_menu_inline())
    else:
        await message.answer("🔐 **PANEL ADMIN** - Akses admin saja")

@dp.message(F.text == "ℹ️ INFO BOT")
async def handle_info_bot(message: Message):
    await message.answer("ℹ️ **SAHABAT JHT BOT**\nVersi 2.1 - Layanan JHT")

@dp.message(F.text == "🏠 MENU UTAMA")
async def handle_menu_utama(message: Message):
