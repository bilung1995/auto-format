...
def generate_next_code(user_id):
    ...
    if suffix:
        ...
        p = prefix.strip()
        if p.endswith('-'):
            code_str = f"{p} {str(num).zfill(padding)}".strip()
        else:
            if p.upper().startswith('JPG') and '-' not in p and len(p) <= 5:
                code_str = f"{p} - {str(num).zfill(padding)}".strip()
            else:
                code_str = f"{p} {str(num).zfill(padding)}".strip()
    # increment
...
def apply_template(template, raw_text, kode_header=None):
    """Template DINAMIS - ngikutin SETTING FORMAT user."""
    # 1. Parsing input user (urutan baris)
    lines = [l.strip() for l in str(raw_text).strip().splitlines()]
    ...
    kab, kec, kel, saldo, kelamin, kpj, sensor, it, pt = cleaned[:9]
    ...
    # 2. Jika template pakai placeholder {KAB} dll -> ganti langsung
    ...
    # 3. Jika template custom tanpa placeholder (kayak AMIN EXPRESS) -> deteksi baris
    out_lines = []
    pin_count = 0
    for line in template.splitlines():
        ...
        if "📍" in line:
            pin_count += 1
            if pin_count == 1:
                new_line = re.sub(r"📍\s*.*", f"📍 {data['KAB']}", line)
            elif pin_count == 2:
                new_line = re.sub(r"📍\s*.*", f"📍 {data['KEC']}", line)
            elif pin_count == 3:
                new_line = re.sub(r"📍\s*.*", f"📍 {data['KEL']}", line)
            out_lines.append(new_line)
            continue

        if "TAHUN" in upper or "KELAMIN" in upper:
            if ":" in line:
                new_line = re.sub(r":\s*.*", f": {data['KELAMIN']}", line)
        elif "SALDO" in upper:
            if ":" in line:
                if "RP" in upper:
                    new_line = re.sub(r":\s*.*", f": Rp. {data['SALDO']}", line)
                else:
                    new_line = re.sub(r":\s*.*", f": {data['SALDO']}", line)
        elif "KPJ" in upper:
            if ":" in line:
                new_line = re.sub(r":\s*.*", f": {data['KPJ']}", line)
        elif "SENSOR" in upper or "TUNGGAL" in upper or "GANDA" in upper:
            if ":" in line:
                new_line = re.sub(r":\s*.*", f": {data['SENSOR']}", line)
        elif ("IUT" in upper or re.search(r"\bIT\b", upper)) and ":" in line:
    ...
    if pin_count == 0 and len([l for l in template.splitlines() if l.strip()]) < 5:
        base = f"""📍 KAB : {data['KAB']}
📍 KEC : {data['KEC']}
📍 KEL : {data['KEL']}
...
async def format_setting_receive(message: Message, state: FSMContext):
    ...
    # Simpan langsung sebagai template custom - jangan diubah, biar apply_template yang pinter ngikutin
    # Tapi kalau user kirim contoh jadi (tanpa {KAB}), kita tetap simpan apa adanya, nanti apply_template akan auto-detect
    conn = db()
    conn.execute("""
    ...
    """, (message.from_user.id, raw, datetime.now().isoformat()))
    conn.commit()
    conn.close()
    await state.clear()
    # Test preview dengan data dummy
    dummy_input = "JAKARTA\nPENJARINGAN\nBEBAS\n14000\nPEREMPUAN 1992\n2021\n22* 23*\n02-03-2025\nSAKTI MULYA"
    preview = apply_template(raw, dummy_input)
    await message.answer(
...
async def format_results(callback: CallbackQuery):
    ...
    conn = db()
    rows = conn.execute(
        "SELECT id, result_text, created_at FROM format_results WHERE telegram_id=? ORDER BY id DESC LIMIT 10",
        (callback.from_user.id,)
    ).fetchall()
    conn.close()

    # Hapus pesan list lama dulu
    try:
    ...
    if not rows:
        await callback.message.bot.send_message(
            callback.from_user.id,
            "📄 <b>HASIL FORMAT - KOSONG</b>\n\nBelum ada hasil format. Buat dulu di menu BUAT FORMAT.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    await callback.message.bot.send_message(
        callback.from_user.id,
        f"📄 <b>HASIL FORMAT - {len(rows)} DATA TERBARU</b>\n\nMenampilkan {len(rows)} format terbaru. Setiap format ada tombol SALIN & HAPUS di bawahnya.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔎 CARI HASIL FORMAT", callback_data="format_search")],
            [InlineKeyboardButton(text="🗑️ HAPUS SEMUA", callback_data="clear_results")],
            [InlineKeyboardButton(text="⬅️ KEMBALI KE MENU AUTO FORMAT", callback_data="auto_format")]
        ])
    )

    # Kirim semua format satu per satu lengkap dengan tombol salin di sampingnya
    for row in rows:
        rid = row["id"]
        txt = row["result_text"]
        # Format tanggal
        try:
            dt = datetime.fromisoformat(row["created_at"]).strftime("%d-%m %H:%M")
        except:
            dt = row["created_at"][:16]
        await callback.message.bot.send_message(
            callback.from_user.id,
    ...
    await callback.answer(f"✅ Menampilkan {len(rows)} format")
...
async def send_copy(callback, text):
    await callback.message.bot.send_message(
        callback.from_user.id,
        f"<pre>{html.escape(text[:3800])}</pre>\n✅ <b>Siap disalin - tap tahan untuk copy</b>"
    )
