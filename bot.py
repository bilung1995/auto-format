...
async def af_selected_fixed(callback: CallbackQuery, state: FSMContext):
    # FIX: support af_3m, af_6m, af_1y, af_unlimited + af_20000 legacy
    mapping = {
    ...
    await callback.message.edit_text(f"💳 <b>{nama}</b> {rupiah(amount)}\n\nSilakan transfer dulu, lalu kirim FOTO bukti TF di sini. Admin akan dapat tombol SETUJU/TOLAK.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ SUDAH BAYAR - KIRIM BUKTI", callback_data="payment_done")]]))
...
async def payment_proof(message: Message, state: FSMContext):
    ...
    # === FIX: kirim ke SEMUA admin termasuk DB ===
    await notify_admins_bukti(message.bot, photo.file_id, "photo", caption, keyboard)
    # simpan juga ke bukti_tf agar ada tombol approve seragam
    try:
        conn2 = db()
        conn2.execute("INSERT INTO bukti_tf(telegram_id, username, full_name, file_id, file_type, caption, amount, status, created_at) VALUES(?,?,?,?,?,?,?,?,?)", (message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", photo.file_id, "photo", f"{data.get('package_name')} - {amount}", amount, "pending", datetime.now().isoformat()))
        conn2.commit()
        conn2.close()
    except Exception as e:
        print(f"gagal insert bukti_tf mirror: {e}")
...
async def kirim_bukti_ke_admin(bot, bukti_id, user_id, username, full_name, file_id, file_type, caption_user, amount=0, tx_id=0, package_name=""):
    # Notifikasi lengkap ke SEMUA admin
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ SETUJU", callback_data=f"bukti_approve_{bukti_id}"),
            InlineKeyboardButton(text="❌ TOLAK", callback_data=f"bukti_reject_{bukti_id}")
        ],
        [
...
async def bukti_tf_received(message: Message, state: FSMContext):
    ...
    # ambil package info dari state sebelumnya jika ada di transaksi terakhir
    try:
        conn_tmp = db()
        last_tx = conn_tmp.execute("SELECT package_name, id FROM transactions WHERE proof_file_id=? ORDER BY id DESC LIMIT 1", (file_id,)).fetchone()
        conn_tmp.close()
        pkg_name_last = last_tx["package_name"] if last_tx else ""
        tx_id_last = last_tx["id"] if last_tx else bukti_id
    except:
        pkg_name_last = ""
        tx_id_last = bukti_id
    await kirim_bukti_ke_admin(message.bot, bukti_id, message.from_user.id, message.from_user.username or "", message.from_user.full_name or "", file_id, file_type, caption_user, nominal, tx_id_last, pkg_name_last)
...
async def admin_bukti_approve(callback: CallbackQuery):
    ...
    if not row or row["status"] != "pending":
        conn.close()
        await callback.answer(f"Sudah diproses / tidak ada", show_alert=True)
        return
    # cari transaksi terkait untuk dapat package_code
    tx = conn.execute("SELECT * FROM transactions WHERE proof_file_id=? ORDER BY id DESC LIMIT 1", (row["file_id"],)).fetchone()
    amount = row["amount"] or (tx["amount"] if tx else 0)
    pkg_code = tx["package_code"] if tx and tx["package_code"] else ""
    pkg_name = tx["package_name"] if tx and tx["package_name"] else ""
    now_iso = datetime.now().isoformat()
    conn.execute("UPDATE bukti_tf SET status='approved', decided_by=?, decided_at=? WHERE id=?", (callback.from_user.id, now_iso, bukti_id))
    conn.execute("UPDATE transactions SET status='approved', processed_at=? WHERE proof_file_id=? AND status='pending'", (now_iso, row["file_id"]))
    # LOGIC APPROVAL BERDASARKAN PAKET - TAMBAH KOTA / AUTO FORMAT / CARI LAINNYA
    from datetime import timedelta
    try:
        if pkg_code.startswith("kota_"):
            if pkg_code == "kota_1w":
                expiry = datetime.now() + timedelta(days=7)
            elif pkg_code == "kota_1m":
                expiry = add_months(datetime.now(), 1)
            elif pkg_code == "kota_2m":
                expiry = add_months(datetime.now(), 2)
            elif pkg_code == "kota_6m":
                expiry = add_months(datetime.now(), 6)
            else:
                expiry = None
            status = "unlimited" if pkg_code == "kota_unlimited" else "active"
            quota = 9999 if pkg_code == "kota_unlimited" else 3
            conn.execute("INSERT INTO subscriptions(telegram_id,package_code,package_name,price,start_date,expiry_date,status) VALUES(?,?,?,?,?,?,?) ON CONFLICT(telegram_id) DO UPDATE SET package_code=excluded.package_code, package_name=excluded.package_name, price=excluded.price, start_date=excluded.start_date, expiry_date=excluded.expiry_date, status=excluded.status", (row["telegram_id"], pkg_code, pkg_name or f"TAMBAH KOTA {quota}x", amount, now_iso, expiry.isoformat() if expiry else None, status))
            conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+?", (row["telegram_id"], quota, quota))
        elif pkg_code.startswith("cari_") or pkg_code.startswith("af_"):
            # AUTO FORMAT dan CARI LAINNYA sama-sama pakai subscriptions
            if pkg_code in ("cari_1w", "af_1w"):
                expiry = datetime.now() + timedelta(days=7)
            elif pkg_code in ("cari_1m", "af_3m"):
                expiry = add_months(datetime.now(), 3 if pkg_code.startswith("af_") else 1)
            elif pkg_code in ("cari_2m", "af_6m"):
                expiry = add_months(datetime.now(), 6 if pkg_code.startswith("af_") else 2)
            elif pkg_code in ("cari_6m", "af_1y"):
                expiry = add_months(datetime.now(), 12 if pkg_code.startswith("af_") else 6)
            else:
                expiry = None
            status = "unlimited" if "unlimited" in pkg_code else "active"
            conn.execute("INSERT INTO subscriptions(telegram_id,package_code,package_name,price,start_date,expiry_date,status) VALUES(?,?,?,?,?,?,?) ON CONFLICT(telegram_id) DO UPDATE SET package_code=excluded.package_code, package_name=excluded.package_name, price=excluded.price, start_date=excluded.start_date, expiry_date=excluded.expiry_date, status=excluded.status", (row["telegram_id"], pkg_code, pkg_name or pkg_code, amount, now_iso, expiry.isoformat() if expiry else None, status))
        else:
            # TOP UP biasa
            if amount and amount > 0:
                conn.execute("UPDATE users SET balance=balance+? WHERE telegram_id=?", (amount, row["telegram_id"]))
                conn.execute("INSERT INTO kota_quota(telegram_id,quota,total_used) VALUES(?,?,0) ON CONFLICT(telegram_id) DO UPDATE SET quota=quota+2", (row["telegram_id"], 2))
    except Exception as e:
        print(f"approve logic error: {e}")
    conn.commit()
    conn.close()
    try:
        await callback.message.edit_caption(caption=(callback.message.caption or "") + f"\n\n✅ <b>DISETUJUI oleh {callback.from_user.id}</b> - {pkg_name}")
    except:
        try:
            ...
            pass
    try:
        await callback.bot.send_message(row["telegram_id"], f"✅ <b>BUKTI TF #{bukti_id} DISETUJUI!</b>\n\n📦 Paket: {pkg_name or 'TOP UP'}\n💰 Nominal: {rupiah(amount)}\n\nTerima kasih, akses / quota sudah aktif! Silakan klik:\n• TAMBAH KOTA\n• AUTO FORMAT\n• CARI LAINNYA", reply_markup=main_menu(row["telegram_id"]))
    except:
        pass
    await callback.answer("✅ Disetujui & akses dibuka!")
