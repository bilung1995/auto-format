...
def main_menu(user_id=None):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="💳 TOP UP SALDO", callback_data="topup")],
        [InlineKeyboardButton(text="📊 CEK STATUS LAYANAN", callback_data="status")],
        [InlineKeyboardButton(text="🛠️ SOLUSI MASALAH JMO", callback_data="solusi_jmo")],
        [InlineKeyboardButton(text="📝 AUTO FORMAT DATA", callback_data="auto_format")],
        [InlineKeyboardButton(text="📞 HUBUNGI ADMIN", callback_data="contact_admin")],
    ]

    if is_admin(user_id):
        rows.append([InlineKeyboardButton(text="🔐 PANEL ADMINISTRATOR", callback_data="admin_panel")])
        rows.append([InlineKeyboardButton(text="ℹ️ INFO BOT SABABAT JHT", callback_data="info_bot")])
...
def auto_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 BUAT FORMAT DATA BARU", callback_data="format_create")],
        [InlineKeyboardButton(text="📄 LIHAT HASIL FORMAT TERKINI", callback_data="format_results")],
        [InlineKeyboardButton(text="⚙️ SETTING TEMPLATE FORMAT", callback_data="format_setting")],
        [InlineKeyboardButton(text="🕘 RIWAYAT HASIL FORMAT", callback_data="format_history")],
        [InlineKeyboardButton(text="⬅️ KEMBALI KE MENU UTAMA", callback_data="back_main")]
    ])
...
# ==================== FUNGSI UTAMA PEMILAH DATA OTOMATIS ====================
def apply_template(template, raw_text):
    """Convert user input (even messy/unordered) into the configured format."""
    import re

    # 1. Ubah input user menjadi UPPERCASE agar pencarian mudah
    text = str(raw_text).upper()

    # 2. Inisialisasi data kosong
    data = {
        "KAB": "-", "KEC": "-", "KEL": "-", 
        "SALDO": "0", "KELAMIN": "-", 
        "KPJ": "-", "SENSOR": "-", "IT": "-", "PT": "-"
    ...
    # 3. LOGIKA PENCARIAN DATA (Menggunakan Regex - Tidak peduli urutan)

    # --- Cari SALDO (ambil angka nominal dengan 4 digit atau lebih, termasuk titik/koma)
    match_saldo = re.search(r'(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?|\d{6,})', text)
    if match_saldo:
        clean_saldo = match_saldo.group(0).replace(".", "").replace(",", "")
        if clean_saldo.isdigit() and int(clean_saldo) > 0:
            data["SALDO"] = f"{int(clean_saldo):,}".replace(",", ".")

    # --- Cari KELAMIN & TAHUN LAHIR
    match_gender = re.search(r'(PRIA|WANITA|LAKI-LAKI|PEREMPUAN)', text)
    match_year = re.search(r'\b(19\d{2}|20\d{2})\b', text)
    kelamin_parts = []
    if match_gender:
        kelamin_parts.append(match_gender.group(0))
    if match_year:
        kelamin_parts.append(match_year.group(0))
    data["KELAMIN"] = " ".join(kelamin_parts) if kelamin_parts else "-"

    # --- Cari KPJ (4-5 digit angka, atau angka setelah kata KPJ)
    match_kpj = re.search(r'KPJ\s*(\d+)', text)
    if match_kpj:
        data["KPJ"] = match_kpj.group(1)
    else:
        match_kpj_alt = re.search(r'\b\d{4,5}\b', text)
        if match_kpj_alt:
            data["KPJ"] = match_kpj_alt.group(0)

    # --- Cari SENSOR (Tahun ke-2 yang muncul di kalimat, atau angka setelah kata SENSOR)
    match_sensor = re.search(r'SENSOR\s*(\d+)', text)
    if match_sensor:
        data["SENSOR"] = match_sensor.group(1)
    else:
        all_years = re.findall(r'\b(19\d{2}|20\d{2})\b', text)
        if len(all_years) >= 2:
            data["SENSOR"] = all_years[1]  # Ambil tahun kedua

    # --- Cari IT
    match_it = re.search(r'IT\s*[:=]\s*([\w\-]+)', text) or re.search(r'\bIT\s+([\w\-]+)', text)
    if match_it:
        data["IT"] = match_it.group(1)

    # --- Cari KAB / KEC / KEL
    # Prioritas: Jika ada label "KAB :", ambil itu. Jika tidak, cari kata kunci.
    match_kab = re.search(r'KAB\s*[:=]\s*([A-Z\s]+)', text)
    if match_kab:
        data["KAB"] = match_kab.group(1).strip()
    else:
        # Tambahkan kata kunci kabupaten di sini (bisa diperluas)
        match_kab_alt = re.search(r'\b(SERANG|TANGERANG|BOGOR|BEKASI|JAKARTA|BANDUNG)\b', text)
        if match_kab_alt:
            data["KAB"] = match_kab_alt.group(0)

    match_kec = re.search(r'KEC\s*[:=]\s*([A-Z\s]+)', text)
    if match_kec:
        data["KEC"] = match_kec.group(1).strip()
    else:
        # Contoh kecamatan (bisa disesuaikan)
        match_kec_alt = re.search(r'\b(LEBAK WANGI|CIPONDOH|PAMULANG|CILEDUG|TANGERANG)\b', text)
        if match_kec_alt:
            data["KEC"] = match_kec_alt.group(0)

    match_kel = re.search(r'KEL\s*[:=]\s*([A-Z\s]+)', text)
    if match_kel:
        data["KEL"] = match_kel.group(1).strip()
    else:
        # Contoh kelurahan
        match_kel_alt = re.search(r'\b(ANGSANA|BABAKAN|CIPOCOK|KELAPA DUA)\b', text)
        if match_kel_alt:
            data["KEL"] = match_kel_alt.group(0)

    # --- Cari PT
    match_pt = re.search(r'PT\s*[:=]\s*([A-Z\s]+)', text) or re.search(r'\bPT\s+([A-Z]+)', text)
    if match_pt:
        data["PT"] = match_pt.group(1).strip()

    # 4. Masukkan data ke dalam TEMPLATE (Pastikan semua sudah uppercase)
    result = template
    for key, value in data.items():
        # Ganti format template {KAB}, [KAB], atau KAB : 
        result = re.sub(rf'\{{\s*{key}\s*\}}', value, result)
        result = re.sub(rf'\[\s*{key}\s*\]', value, result)
        result = re.sub(rf'(?i)({key}\s*:\s*)[^\n]*', r'\g<1>' + value, result)
...
async def format_manual_receive(message: Message, state: FSMContext):
    ...
    # 1. Gunakan fungsi apply_template yang baru (data dipilah otomatis)
    # 2. Karena di dalam apply_template sudah di-UPPERCASE semua, hasilnya otomatis kapital
...
async def format_results(callback: CallbackQuery):
    ...
    buttons += [
        ...
        [InlineKeyboardButton(text="🗑️ HAPUS SEMUA HASIL FORMAT", callback_data="clear_results")],
        [InlineKeyboardButton(text="⬅️ KEMBALI KE MENU AUTO FORMAT", callback_data="auto_format")]
    ]
...
async def view_result(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📋 SALIN HASIL FORMAT", callback_data=f"copy_result_{rid}")],
            [InlineKeyboardButton(text="📋 SALIN SEMUA HASIL", callback_data=f"copy_all_{rid}")],
            [InlineKeyboardButton(text="✏️ EDIT HASIL FORMAT", callback_data=f"edit_result_{rid}")],
            [InlineKeyboardButton(text="🗑️ HAPUS HASIL FORMAT", callback_data=f"delete_result_{rid}")],
            [InlineKeyboardButton(text="💾 SIMPAN KE HISTORY", callback_data=f"save_result_{rid}")],
...
async def format_search_receive(message: Message, state: FSMContext):
    ...
    buttons.append([InlineKeyboardButton(text="⬅️ KEMBALI KE MENU HASIL FORMAT", callback_data="format_results")])
