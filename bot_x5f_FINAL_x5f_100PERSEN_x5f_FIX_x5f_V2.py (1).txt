        ...
        for r in rows:
            ...
            def contains(needle, hay):
                ...
                if needle in hay:
                    return True
                import re
                short = re.sub(r"^(kab\.?|kota|kabupaten)\s+", "", needle).strip()
                if short and short in hay:
                    return True
                return False
            kab_match = contains(kab_l, wa_lower)
            if not kec or kec.upper() == "SEMUA KECAMATAN":
                kec_match = True
            else:
                kec_match = contains(kec_l, wa_lower)
            if kab_match and kec_match:
                matched.append(r['telegram_id'])
        return list(set(matched))
    except Exception as e:
        logging.exception(f"Error find_users: {e}")
        return []
...
async def forward_wa_to_telegram(bot, wa_text: str, wa_group: str, wa_sender: str, wa_number: str = "", parsed_kab: str = "", parsed_kec: str = ""):
    ...
    matched_ids = find_users_from_wa_message(wa_text)
    matched = []
    if matched_ids:
        # find_users_from_wa_message sekarang return list telegram_id saja
        # Kita butuh kab/kec untuk log, ambil dari DB user_kota yang match
        conn = db()
        for tid in matched_ids:
            # ambil 1 kota yang match untuk ditampilkan di log
            row = conn.execute("SELECT kab,kec FROM user_kota WHERE telegram_id=? LIMIT 1", (tid,)).fetchone()
            if row:
                matched.append((tid, row['kab'], row['kec']))
            else:
                matched.append((tid, "-", "-"))
        conn.close()
