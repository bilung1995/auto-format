...
async def format_results(callback: CallbackQuery):
    ...
    conn = db()
    rows = conn.execute(
        "SELECT id, result_text, created_at FROM format_results WHERE telegram_id=? ORDER BY id DESC LIMIT 1000",
        (callback.from_user.id,)
    ).fetchall()
    conn.close()
    if not rows:
    ...
    await callback.message.edit_text(
        f"📄 <b>HASIL FORMAT SEMUA - {len(rows)} DATA</b>\n\nMenampilkan SEMUA {len(rows)} format (tanpa limit), tiap format ada tombolnya sendiri:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def hasil_akun(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"📧 <b>HASIL FORMAT+AKUN - {len(rows)} DATA - TANPA LIMIT</b>\n\nMenampilkan SEMUA {len(rows)} akun (tanpa limit):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
        ])
    )
...
async def search_akun_do(message: Message, state: FSMContext):
    ...
    conn = db()
    rows = conn.execute("SELECT id,email,password FROM format_accounts WHERE telegram_id=? AND (email LIKE ? OR password LIKE ?) ORDER BY id DESC LIMIT 100000", (message.from_user.id, f"%{keyword}%", f"%{keyword}%")).fetchall()
    conn.close()
    await state.clear()
    if not rows:
...
async def format_history(callback: CallbackQuery):
    ...
    rows = conn.execute("""
        ...
        WHERE telegram_id=? ORDER BY id DESC LIMIT 100000
    """, (callback.from_user.id,)).fetchall()
    conn.close()
