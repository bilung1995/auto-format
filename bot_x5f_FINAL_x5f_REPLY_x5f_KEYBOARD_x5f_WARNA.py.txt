...
def main_menu(user_id=None):
    ...
    # REPLY KEYBOARD = WARNA BIRU HIJAU ABU KAYAK AQUA VIVA
    # Ini yang bikin kotak berwarna, bukan inline
    keyboard = [
        [
            KeyboardButton(text="🔓 Buka Sensor"),
            KeyboardButton(text="💰 Cek Saldo Satuan")
        ],
        [
            KeyboardButton(text="📋 Cek Saldo Masal"),
            KeyboardButton(text="📧 Lupa Akun")
        ],
        [
            KeyboardButton(text="🚪 Logout Paksa"),
            KeyboardButton(text="✉️ Rombak Tanpa OTP")
        ],
        [
            KeyboardButton(text="💳 Top Up"),  # Ini bakal hijau di theme Aqua Viva
            KeyboardButton(text="🛒 Beli Credit")
        ],
        [
...
def main_menu_inline(user_id=None):
    # Fallback inline untuk di dalam chat (jika butuh)
    ...
    rows = [
        [
            ...
            InlineKeyboardButton(text="🟡 TOP UP", web_app=WebAppInfo(url="https://golden-pastelito-feec0f.netlify.app"))
        ],
        [
        ...
        [
            InlineKeyboardButton(text="✨ MENU EMAS", web_app=WebAppInfo(url="https://golden-pastelito-feec0f.netlify.app")),
...
def auto_menu():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="📧 AKUN+EMAIL", callback_data="hasil_akun"),
...
async def format_manual_receive(message: Message, state: FSMContext):
    ...
    if edit_id:
        ...
        if akun.get('EMAIL'):
            cur.execute("""INSERT INTO format_accounts(telegram_id,email,password,raw_text,result_id,created_at) VALUES(?,?,?,?,?,?)""", (message.from_user.id, akun['EMAIL'], akun['PASSWORD'], f"{akun['EMAIL']}\n{akun['PASSWORD']}", result_id, datetime.now().isoformat()))
...
async def format_results(callback: CallbackQuery):
    ...
    if not rows:
        ...
        await callback.answer()
        return
    text_msg = f"📄 <b>HASIL FORMAT TERKINI - {len(rows)} TERBARU</b> (cepat)\n\n"
    kb_rows = []
    for i, row in enumerate(rows, 1):
        ...
        if len(txt) > 200:
            preview += "..."
        text_msg += f"<b>{i}. ID {rid}</b>\n<pre>{html.escape(preview)}</pre>\n"
        kb_rows.append([
            InlineKeyboardButton(text=f"📋{i}", callback_data=f"copy_result_{rid}"),
            InlineKeyboardButton(text=f"✏️{i}", callback_data=f"edit_result_{rid}"),
            InlineKeyboardButton(text=f"🗑️{i}", callback_data=f"delete_result_{rid}")
        ])
    kb_rows.append([InlineKeyboardButton(text="🔎 CARI HASIL FORMAT", callback_data="format_search")])
    kb_rows.append([InlineKeyboardButton(text="📧 HASIL AKUN+EMAIL", callback_data="hasil_akun")])
    kb_rows.append([InlineKeyboardButton(text="🗑️ HAPUS SEMUA", callback_data="clear_results")])
    kb_rows.append([InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")])
    await callback.message.edit_text(text_msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows))
    await callback.answer()
...
async def hasil_akun(callback: CallbackQuery):
    if not has_auto_format_access(callback.from_user.id):
        ...
        return
    conn = db()
    rows = conn.execute("SELECT id,email,password,created_at FROM format_accounts WHERE telegram_id=? ORDER BY id DESC LIMIT 10", (callback.from_user.id,)).fetchall()
    conn.close()
    if not rows:
        await callback.message.edit_text(
            "📧 <b>HASIL AKUN+EMAIL - KOSONG</b>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📝 BUAT FORMAT BARU", callback_data="format_create")],
                [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
            ])
        ...
        await callback.answer()
        return
    text_msg = f"📧 <b>HASIL AKUN+EMAIL - {len(rows)} TERBARU</b> (cepat)\n\n"
    kb_rows = []
    for i, row in enumerate(rows, 1):
        rid = row["id"]
        text_msg += f"<b>{i}. ID {rid}</b> - <code>{html.escape(row['email'])}</code>\n"
        kb_rows.append([
            InlineKeyboardButton(text=f"📋{i}", callback_data=f"copy_akun_{rid}"),
            InlineKeyboardButton(text=f"✏️{i}", callback_data=f"edit_akun_{rid}"),
            InlineKeyboardButton(text=f"🗑️{i}", callback_data=f"del_akun_{rid}")
        ])
    kb_rows.append([InlineKeyboardButton(text="🔎 CARI AKUN", callback_data="search_akun")])
    kb_rows.append([InlineKeyboardButton(text="📄 HASIL FORMAT", callback_data="format_results")])
    kb_rows.append([InlineKeyboardButton(text="🗑️ HAPUS SEMUA", callback_data="hapus_akun_all")])
    kb_rows.append([InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")])
    await callback.message.edit_text(text_msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows))
    await callback.answer()
...
@dp.message(F.text == "🔓 Buka Sensor")
async def h_buka_sensor(message: Message):
    await message.answer("🔓 Fitur Buka Sensor - ketik data yang mau dibuka")

@dp.message(F.text == "💰 Cek Saldo Satuan")
async def h_cek_satuan(message: Message):
    await message.answer("💰 Cek Saldo Satuan - kirim format akun", reply_markup=main_menu_inline(message.from_user.id))

@dp.message(F.text == "📋 Cek Saldo Masal")
async def h_cek_masal(message: Message):
    await message.answer("📋 Cek Saldo Masal - upload file")

@dp.message(F.text == "📧 Lupa Akun")
async def h_lupa_akun(message: Message):
    await message.answer("📧 Lupa Akun - kirim email terdaftar")

@dp.message(F.text == "🚪 Logout Paksa")
async def h_logout(message: Message):
    await message.answer("🚪 Logout Paksa - kirim ID akun")

@dp.message(F.text == "✉️ Rombak Tanpa OTP")
async def h_rombak(message: Message):
    await message.answer("✉️ Rombak Tanpa OTP - kirim data")

@dp.message(F.text == "💳 Top Up")
async def h_topup_reply(message: Message):
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo
    await message.answer(
        "💳 TOP UP - Pilih nominal:\nKlik tombol di bawah untuk buka menu emas:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✨ BUKA TOP UP EMAS", web_app=WebAppInfo(url="https://golden-pastelito-feec0f.netlify.app"))],
            [InlineKeyboardButton(text="⬅️ Kembali", callback_data="back_main")]
        ])
    )

@dp.message(F.text == "🛒 Beli Credit")
async def h_beli_credit(message: Message):
    await message.answer("🛒 Beli Credit - hubungi admin")

@dp.message(F.text == "📊 Status")
async def h_status_reply(message: Message):
    await message.answer("📊 Cek Status - kirim ID transaksi", reply_markup=main_menu_inline(message.from_user.id))

@dp.message(F.text == "💡 Bantuan")
async def h_bantuan_reply(message: Message):
    await message.answer("💡 Bantuan - hubungi @admin")

@dp.message(F.text == "🏠 Menu Utama")
async def h_menu_utama_reply(message: Message):
    await message.answer("🏠 Menu Utama SAHABAT JHT", reply_markup=main_menu(message.from_user.id))

@dp.message(F.text == "🔐 Panel Admin")
async def h_admin_reply(message: Message):
    if is_admin(message.from_user.id):
        await message.answer("🔐 Panel Admin", reply_markup=main_menu_inline(message.from_user.id))
