...
def auto_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✨ BUAT FORMAT", callback_data="format_create"), InlineKeyboardButton(text="📝 MANUAL", callback_data="format_manual")],
...
async def bantuan_handler(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        "3. Masuk AUTO FORMAT > BUAT FORMAT\n"
...
async def format_results(callback: CallbackQuery):
    ...
    conn = db()
    rows = conn.execute(
        "SELECT id, result_text, created_at FROM format_results WHERE telegram_id=? ORDER BY id ASC",
    ...
    await callback.message.edit_text(
        f"📄 <b>HASIL FORMAT - {len(rows)} DATA</b>\n\nSemua hasil format ditampilkan di bawah, tiap hasil ada tombolnya sendiri:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def hasil_akun(callback: CallbackQuery):
    if not has_auto_format_access(callback.from_user.id):
        ...
        return
    conn = db()
    rows = conn.execute("SELECT id,email,password,created_at, result_id FROM format_accounts WHERE telegram_id=? ORDER BY id ASC", (callback.from_user.id,)).fetchall()
    conn.close()
    if not rows:
        await callback.message.edit_text(
    ...
    await callback.message.edit_text(
        f"📧 <b>AKUN - {len(rows)} DATA</b>\n\nSemua akun ditampilkan satu per satu:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ KEMBALI", callback_data="auto_format")]
        ])
    )
