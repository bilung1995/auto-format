...
async def forward_wa_to_telegram(bot, wa_text: str, wa_group: str, wa_sender: str, parsed_kab: str, parsed_kec: str):
    nl = chr(10)
    # Jika kab/kec tidak ketemu, tetap log dan coba forward ke semua user yang punya kota untuk testing
    if not parsed_kab or not parsed_kec:
        print(f"⚠️ Parse gagal, text: {wa_text[:200]}")
        # Untuk testing: kalau parse gagal, forward ke admin biar ketahuan
        try:
            conn = db()
    ...
    if not user_ids:
        ...
        # Kirim ke admin untuk debug biar tau kenapa tidak match
        try:
            conn = db()
            admin_rows = conn.execute("SELECT telegram_id FROM admins").fetchall()
            conn.close()
...
async def wa_logs_handler(request):
    try:
        conn = db()
        rows = conn.execute("SELECT wa_group, wa_sender, message, parsed_kab, parsed_kec, created_at FROM wa_inbox_log ORDER BY id DESC LIMIT 20").fetchall()
        conn.close()
        ...
        for r in rows:
            ...
            html_logs += f"<tr><td>{r['created_at']}</td><td>{r['wa_group']}</td><td>{r['wa_sender']}</td><td>{r['parsed_kab']}</td><td>{r['parsed_kec']}</td><td>{msg}</td></tr>"
        html_logs += "</table>"
        return web.Response(text=html_logs, content_type="text/html")
    except Exception as e:
        return web.Response(text=f"Error: {e}")
