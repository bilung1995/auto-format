...
def find_users_from_wa_message(wa_text: str):
    """Cari user HANYA jika KOTA/KABUPATEN DAN KECAMATAN sama-sama ada."""
    text = str(wa_text or '').upper()
    if not text.strip():
        return []

    # Normalisasi pesan, tetapi tetap mempertahankan nama daerah.
    ...
    def name_matches(name: str, pattern) -> bool:
        # 1) cocok normal dengan word-boundary.
        if pattern and pattern.search(normalized):
            return True
        # 2) fallback: cocok walau nama ditulis digabung tanpa spasi
        #    di pesan WA (mis. "SUKAJAYA" vs DB "SUKA JAYA").
        n = _norm_region_name(name).replace(' ', '')
        return bool(n) and n in normalized_nospace
    ...
    try:
        ...
        matches = []
        near_misses = []  # DEBUG: kab cocok tapi kec tidak (atau sebaliknya)
        for r in rows:
            kab = str(r['kab'] or '').strip()
            kec = str(r['kec'] or '').strip()
            kab_re = region_pattern(kab)
            kec_re = region_pattern(kec)
            kab_ok = name_matches(kab, kab_re)
            kec_ok = name_matches(kec, kec_re)

            # WAJIB dua-duanya cocok.
            if kab_ok and kec_ok:
                matches.append((int(r['telegram_id']), kab, kec))
            elif kab_ok != kec_ok:
                near_misses.append((r['telegram_id'], kab, kec, kab_ok, kec_ok))

        # DEBUG: kalau tidak ada yang match sama sekali, tampilkan kandidat
        # terdekat supaya ketahuan bagian mana yang tidak cocok (kab/kec).
        if not matches and near_misses:
            print(f"🔎 DEBUG near-miss (pesan ternormalisasi): {normalized[:200]!r}")
            for tid, kab, kec, kab_ok, kec_ok in near_misses[:10]:
                print(
                    f"   ↳ user={tid} kab='{kab}'({'✅' if kab_ok else '❌'}) "
                    f"kec='{kec}'({'✅' if kec_ok else '❌'})"
                )

        # Satu Telegram ID hanya menerima satu notifikasi untuk satu pesan.
        unique = {}
        for item in matches:
            unique[item[0]] = item
