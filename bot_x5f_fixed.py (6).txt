...
async def admin_list_admin(callback: CallbackQuery):
    ...
    db_admins_text = ""
    if rows:
        db_admins_text = "\n".join([f"• <code>{r['telegram_id']}</code> - oleh {r['added_by']} pada {r['created_at'][:10]}" for r in rows])
...
async def admin_tambah_admin_process(message: Message, state: FSMContext):
    ...
    if added:
        ...
        await message.answer("❌ Tidak ada admin baru ditambahkan.", reply_markup=admin_menu())
...
async def admin_hapus_admin_process(message: Message, state: FSMContext):
    ...
    for part in raw:
        if part.isdigit():
            aid = int(part)
            if aid in ADMIN_IDS:
                await message.answer(f"❌ ID <code>{aid}</code> adalah admin ENV, tidak bisa dihapus via bot.")
                continue
            conn = db()
